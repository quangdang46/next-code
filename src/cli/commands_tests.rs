...
fn configured_auth_test_targets_only_include_configured_supported_providers() {
    ...
    let _openrouter = SavedEnv::capture(&["OPENROUTER_API_KEY"]);
...
async fn auth_test_choice_plan_discovers_model_for_hosted_custom_compat_endpoint_with_api_key() {
    ...
    // 127.0.0.2 is loopback (connectable on Windows) but is not treated as
    ...
    // (0.0.0.0 binds on all interfaces but is not a valid connect target on Windows.)
    ...
    crate::env::set_var("NO_PROXY", "127.0.0.2,127.0.0.1,localhost");
    crate::env::set_var("no_proxy", "127.0.0.2,127.0.0.1,localhost");
    crate::env::remove_var("NEXT_CODE_OPENAI_COMPAT_DEFAULT_MODEL");
    crate::env::remove_var("NEXT_CODE_OPENAI_COMPAT_LOCAL_ENABLED");
    crate::provider_catalog::apply_openai_compatible_profile_env(None);
