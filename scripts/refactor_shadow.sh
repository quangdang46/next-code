...
Usage:
  ...
  scripts/refactor_shadow.sh serve [-- <next-code serve args>]
  scripts/refactor_shadow.sh run [-- <next-code args>]
  scripts/refactor_shadow.sh connect [-- <next-code connect args>]
...
ensure_binary() {
  if [[ ! -x "$ref_bin" ]]; then
    die "next-code binary not found or not executable: $ref_bin (run 'scripts/refactor_shadow.sh build')"
  fi
}
...
cmd_env() {
  cat <<EOF_OUT
NEXT_CODE_REF_HOME=$ref_home
NEXT_CODE_REF_SOCKET=$ref_socket
NEXT_CODE_REF_PROFILE=$ref_profile
