...
maybe_enable_sccache() {
  ...
  local force_sccache="${NEXT_CODE_SCCACHE:-auto}"
...
validate_feature_profile() {
  local profile="${NEXT_CODE_DEV_FEATURE_PROFILE:-default}"
  case "$profile" in
...
selfdev_low_memory_default_needed() {
  ...
  # On small no-swap machines, earlyoom can terminate the root next-code rustc
...
#
# After the monolith was split into the next-code-base/app-core/tui/cli crate DAG,
# the largest single rustc unit is next-code-base, which has grown back to a
...
select_build_jobs() {
  ...
  local override="${NEXT_CODE_BUILD_JOBS:-${CARGO_BUILD_JOBS:-}}"
  if [[ -n "$override" ]]; then
  ...
  mib_per_job="${NEXT_CODE_BUILD_MIB_PER_JOB:-$mib_per_job_default}"
...
# Keep `next-code-build-meta`'s embedded git metadata in lockstep with HEAD without
# making it watch `.git` mtimes.
#
# `next-code-build-meta/build.rs` deliberately does NOT declare `.git/HEAD`/`.git/index`
...
export_git_build_metadata() {
  ...
  if [[ -n "${NEXT_CODE_BUILD_GIT_HASH:-}" ]]; then
    git_meta_status="external:${NEXT_CODE_BUILD_GIT_HASH:-}"
    return
  fi
  ...
  export NEXT_CODE_BUILD_GIT_HASH="$hash"
  date="$(git -C "$repo_root" log -1 --format=%ci 2>/dev/null || true)"
  if [[ -n "$date" ]]; then
    export NEXT_CODE_BUILD_GIT_DATE="$date"
  fi
...
maybe_configure_low_memory_selfdev() {
  ...
  local mode="${NEXT_CODE_SELFDEV_LOW_MEMORY:-auto}"
  case "$mode" in
    1|true|yes|on|force)
      ;;
    0|false|no|off|never)
...
# builds. The next-code monoliths (next-code-base/app-core/tui) are ~80% single-threaded
...
#   next-code-base clean recompile  25.3s -> 12.7s   (-Zthreads=4)
...
dev_nightly_toolchain() {
  ...
  if [[ -n "${NEXT_CODE_DEV_TOOLCHAIN:-}" ]]; then
    printf '%s\n' "${NEXT_CODE_DEV_TOOLCHAIN:-}"
    return 0
  fi
  local tc
...
configure_parallel_frontend() {
  local requested="${NEXT_CODE_PARALLEL_FRONTEND:-auto}"
  local forced="false"
  case "$requested" in
    0|false|no|off)
  ...
  local threads="${NEXT_CODE_FRONTEND_THREADS:-4}"
...
configure_linux_linker() {
  local requested_mode="${NEXT_CODE_FAST_LINKER:-auto}"
  local mode="$requested_mode"

  case "$mode" in
    auto)
      # Prefer mold over lld: on this repo's large statically-linked binary
      # (~300 MB .text), mold links the next-code bin in ~2.0s vs lld's ~2.9s
...
print_setup() {
  if [[ -n "${NEXT_CODE_DEV_FEATURE_PROFILE:-}" && "${NEXT_CODE_DEV_FEATURE_PROFILE}" != "default" ]]; then
    feature_profile_status="${NEXT_CODE_DEV_FEATURE_PROFILE:-}"
  fi
  cat <<EOF
repo_root=$repo_root
os=$(uname -s)
...
build_git_hash=${NEXT_CODE_BUILD_GIT_HASH:-<unset>}
...
remote_connect_timeout() {
  local value="${NEXT_CODE_REMOTE_CONNECT_TIMEOUT:-5}"
...
remote_tcp_timeout() {
  ...
  local value="${NEXT_CODE_REMOTE_TCP_TIMEOUT:-1}"
...
remote_recovery_tcp_timeout() {
  ...
  local value="${NEXT_CODE_REMOTE_RECOVERY_TCP_TIMEOUT:-0.3}"
  if [[ ! "$value" =~ ^[0-9]+([.][0-9]+)?$ ]]; then
    value=0.3
  fi
  printf '%s\n' "$value"
...
remote_down_cache_ttl() {
  ...
  local value="${NEXT_CODE_REMOTE_DOWN_TTL:-300}"
  if [[ ! "$value" =~ ^[0-9]+$ ]]; then
    value=300
  fi
  printf '%s\n' "$value"
...
remote_down_cache_path() {
  local remote="${NEXT_CODE_REMOTE_HOST:-}"
  local key
  ...
  printf '%s/next-code-remote-down-%s\n' "${TMPDIR:-/tmp}" "$key"
}
...
remote_cargo_preflight() {
  ...
  local ssh_bin="${NEXT_CODE_REMOTE_SSH_BIN:-ssh}"
  ...
  local tcp_probe="${NEXT_CODE_REMOTE_TCP_PROBE:-1}"
  case "$tcp_probe" in
  ...
  local server_alive_interval="${NEXT_CODE_REMOTE_SERVER_ALIVE_INTERVAL:-10}"
  local server_alive_count="${NEXT_CODE_REMOTE_SERVER_ALIVE_COUNT_MAX:-1}"
  local output
  if ! output=$("$ssh_bin" \
    -o BatchMode=yes \
    ...
    "$remote" "printf 'next-code-remote-ok\\n'" 2>&1); then
...
remote_cargo_fallback_mode() {
  local mode="${NEXT_CODE_REMOTE_CARGO_FALLBACK:-local}"
  case "$mode" in
    local|1|true|yes|on)
      printf 'local\n'
      ;;
...
run_local_cargo() {
  if cargo_test_has_explicit_filter "${cargo_argv[@]}" && [[ "${NEXT_CODE_DEV_CARGO_ALLOW_ZERO_TESTS:-0}" != "1" ]]; then
    local output_file
