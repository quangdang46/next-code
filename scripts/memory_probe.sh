...
[[ -x "${NEXT_CODE_BIN:-}" ]] || die "next-code CLI not found at ${NEXT_CODE_BIN:-}"
...
cleanup() {
    ...
    if [[ -n "$TESTER_ID" && "$KEEP_TESTER" -ne 1 ]]; then
        "${NEXT_CODE_BIN:-}" debug "tester:${TESTER_ID}:stop" >/dev/null 2>&1 || true
...
SPAWN_OUT="$("${NEXT_CODE_BIN:-}" debug "tester:spawn {\"binary\":\"$WRAPPER\",\"cwd\":\"$TESTER_CWD\",\"cols\":120,\"rows\":40}")" \
...
TESTER_INFO="$("${NEXT_CODE_BIN:-}" debug tester:list | jq -c --arg id "$TESTER_ID" '.[] | select(.id == $id)')"
