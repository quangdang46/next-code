...
DEMO_DIR="${NEXT_CODE_AGENTCARD_DEMO_DIR:-/tmp/next-code-agentcard-discovery-demo}"
MODEL="${NEXT_CODE_AGENTCARD_DEMO_MODEL:-gpt-5.6-sol}"
LOG_FILE="${NEXT_CODE_AGENTCARD_DEMO_LOG:-/tmp/next-code-agentcard-discovery-demo.log}"
...
"${NEXT_CODE_BIN:-}" debug clients:map >"$before_file" 2>>"$LOG_FILE" || printf '{"clients":[]}' >"$before_file"
...
for _ in $(seq 1 100); do
  if "${NEXT_CODE_BIN:-}" debug clients:map >"$after_file" 2>>"$LOG_FILE"; then
...
for _ in $(seq 1 25); do
  if "${NEXT_CODE_BIN:-}" debug --session "$session_id" "client:message:$PROMPT" >>"$LOG_FILE" 2>&1; then
    exit 0
  fi
  sleep 0.2
done
