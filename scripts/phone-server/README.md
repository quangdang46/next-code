# next-code phone server (managed cloud host)

A self-managing EC2 host that runs `next-code serve` with the WebSocket gateway so
phones (the iOS app, or SSH clients like Termius) can drive next-code sessions
...
```
phone (next-code iOS app / Termius, connected through Tailscale)
  │  WebSocket :7643 (pair token auth, tailnet-only)
  ▼
EC2 next-code server ──instance role──▶ AWS Bedrock (Opus 4.6 default)
  ▲
  │ tap wake link (API Gateway → wake lambda, bearer token from URL fragment)
  │ "Pair this phone" button (lambda → SSM Run Command → `next-code pair`)
  │
AWS Budget $10 ──▶ SNS next-code-guard-stop ──▶ breaker lambda ──▶ stop instance
                         └──▶ email
```
...
| `idle-autostop.sh` | `/usr/local/bin/` | idle = no gateway/SSH clients AND next-code has no outbound :443 (not streaming) |
| `next-code-pair-service.py` | `/usr/local/bin/` | Legacy tailnet-only fallback; normal pairing now uses SSM |
| `wake-lambda.py` | Lambda `next-code-phone-wake` (behind API Gateway `8c3wp4cbag`) | wake page: starts instance, polls SSM health, generates pair codes through SSM |
| `breaker-lambda.py` | Lambda `next-code-guard-breaker` | stops the instance; subscribed to SNS `next-code-guard-stop` |
...
- `~/.next-code/config.toml`: `[provider]` default bedrock/Opus 4.6, `[gateway] enabled = true, port 7643, bind 0.0.0.0`
  (note: `~/.next-code/config.toml`, NOT `~/.config/next-code/`)
- `~/.bashrc` env: `NEXT_CODE_BEDROCK_ENABLE=1`, `AWS_REGION=us-east-1`,
  `NEXT_CODE_BEDROCK_MODEL=us.anthropic.claude-opus-4-6-v1`, `NEXT_CODE_GATEWAY_HOST=100.109.78.41`
- Helpers: `~/bin/jc` (next-code with bedrock), `~/bin/phone` (attach-or-create tmux next-code)
...
| `next-code-bedrock-tokens-warn` | >3M input tokens / 15 min | email |
| `next-code-bedrock-tokens-stop` | >10M input tokens / 15 min, 2 periods | breaker stops instance + email |
| AWS budget `next-code-dev-monthly-cost` | forecast >50% / actual >80% | email |
| AWS budget `next-code-dev-monthly-cost` | actual >100% of $10 | SNS breaker stops instance + email |
...
- The EC2 security group has no public ingress. Gateway and SSH access are tailnet-only through `next-code-phone` (`100.109.78.41`).
...
2. Install next-code, Tailscale, tmux, and git. Join the tailnet as `next-code-phone`; set the next-code gateway host to `100.109.78.41`.
...
4. Deploy `wake-lambda.py` as `waker.py`, set `WAKE_TOKEN`, `INSTANCE_ID`, and `NEXT_CODE_GATEWAY_HOST=100.109.78.41`, and grant scoped EC2/SSM permissions. API Gateway HTTP API routes to the Lambda.
5. Create the SNS topics/subscriptions and connect the $10 Budget's 100% actual notification to `next-code-guard-stop`.
