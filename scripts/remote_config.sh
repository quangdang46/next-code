...
next_code_remote_config_path() {
  if [[ -n "${NEXT_CODE_REMOTE_CONFIG:-${JCODE_REMOTE_CONFIG:-}}" ]]; then
    printf '%s\n' "${NEXT_CODE_REMOTE_CONFIG:-${JCODE_REMOTE_CONFIG:-}}"
...
next_code_load_remote_config() {
  ...
  if [[ ${NEXT_CODE_REMOTE_CARGO:-${JCODE_REMOTE_CARGO:-}}+x} ]]; then
    had_remote_cargo=1
    remote_cargo="${NEXT_CODE_REMOTE_CARGO:-${JCODE_REMOTE_CARGO:-}}"
  fi
  if [[ ${NEXT_CODE_REMOTE_HOST:-${JCODE_REMOTE_HOST:-}}+x} ]]; then
    had_remote_host=1
    remote_host="${NEXT_CODE_REMOTE_HOST:-${JCODE_REMOTE_HOST:-}}"
  fi
  if [[ ${NEXT_CODE_REMOTE_DIR:-${JCODE_REMOTE_DIR:-}}+x} ]]; then
    had_remote_dir=1
    remote_dir="${NEXT_CODE_REMOTE_DIR:-${JCODE_REMOTE_DIR:-}}"
  fi
  if [[ ${NEXT_CODE_REMOTE_SSH_BIN:-${JCODE_REMOTE_SSH_BIN:-}}+x} ]]; then
    had_remote_ssh_bin=1
    remote_ssh_bin="${NEXT_CODE_REMOTE_SSH_BIN:-${JCODE_REMOTE_SSH_BIN:-}}"
  fi
  if [[ ${NEXT_CODE_REMOTE_RSYNC_BIN:-${JCODE_REMOTE_RSYNC_BIN:-}}+x} ]]; then
    had_remote_rsync_bin=1
    remote_rsync_bin="${NEXT_CODE_REMOTE_RSYNC_BIN:-${JCODE_REMOTE_RSYNC_BIN:-}}"
  fi
