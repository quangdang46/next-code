#!/usr/bin/env python3
"""Aggregate a next-code Terminal-Bench Harbor job dir and compare against the
...
def main() -> int:
    ...
    rows = []
    next_code_resolved = next_code_trials = 0
    regressions = []
    ...
    print(f"{'task':38} {'base%':>7} {'next-code%':>7} {'pass':>6} {'flag'}")
    print("-" * 75)
    ...
    print("-" * 75)
    if next_code_trials:
        print(f"next-code micro-avg: {next_code_resolved}/{next_code_trials} = {100*next_code_resolved/next_code_trials:.1f}%")
