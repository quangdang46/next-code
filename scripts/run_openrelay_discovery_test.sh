...
MODEL="${NEXT_CODE_OPENRELAY_TEST_MODEL:-gpt-5.6-sol}"
PROVIDER="${NEXT_CODE_OPENRELAY_TEST_PROVIDER:-openai}"
...
PROMPT="${NEXT_CODE_OPENRELAY_TEST_PROMPT:-$DEFAULT_PROMPT}"
OUTPUT="${NEXT_CODE_OPENRELAY_TEST_OUTPUT:-$REPO_ROOT/target/openrelay-discovery-test/latest.ndjson}"
SERVER_LOG="${NEXT_CODE_OPENRELAY_TEST_SERVER_LOG:-$REPO_ROOT/target/openrelay-discovery-test/server.jsonl}"
...
[[ -x "${NEXT_CODE_BIN:-}" ]] || { printf 'Next Code binary is not executable: %s\n' "${NEXT_CODE_BIN:-}" >&2; exit 1; }
