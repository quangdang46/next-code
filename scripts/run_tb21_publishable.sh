...
# Publishable Terminal-Bench 2.1 campaign for next-code + Opus 4.8 (native Anthropic
...
JOBS_DIR=${NEXT_CODE_TB_JOBS_DIR:-${JCODE_TB_JOBS_DIR:-/tmp/next-code-tb21-pub}}
K=${NEXT_CODE_TB_K:-${JCODE_TB_K:-1}}
NCONC=${NEXT_CODE_TB_NCONC:-${JCODE_TB_NCONC:-3}}
AGENT_MULT=${NEXT_CODE_TB_AGENT_MULT:-${JCODE_TB_AGENT_MULT:-1000}}
JOB_NAME=${NEXT_CODE_TB_JOB_NAME:-${JCODE_TB_JOB_NAME:-tb21-opus48-uncapped-k${K}}}
TB_PATH=${NEXT_CODE_TB_PATH:-${JCODE_TB_PATH:-/tmp/terminal-bench-2.1}}
MODEL=${NEXT_CODE_TB_MODEL:-${JCODE_TB_MODEL:-anthropic-api/claude-opus-4-8}}

mkdir -p "$JOBS_DIR"

# Provenance manifest.
{
  echo "# next-code Terminal-Bench 2.1 publishable run"
  echo "timestamp_utc: $(date -u +%Y-%m-%dT%H:%M:%SZ)"
  echo "git_commit: $(git -C "$REPO_ROOT" rev-parse HEAD 2>/dev/null || echo unknown)"
  echo "git_describe: $(git -C "$REPO_ROOT" describe --tags --always --dirty 2>/dev/null || echo unknown)"
  echo "next_code_binary: ${NEXT_CODE_HARBOR_BINARY:-${JCODE_HARBOR_BINARY:-/tmp/next-code-compat-dist/next-code-linux-x86_64.bin}}"
  ...
  echo "model: $MODEL"
  echo "reasoning_effort: ${NEXT_CODE_ANTHROPIC_REASONING_EFFORT:-${JCODE_ANTHROPIC_REASONING_EFFORT:-high}}"
  echo "k_attempts: $K"
