...
use next_code_message_types::{Message, StreamEvent};
use next_code_provider_core::Provider;
use next_code_provider_grok_runtime::{GrokBuildProcess, GrokBuildProvider};
...
fn fake_process(log: &Path) -> GrokBuildProcess {
    ...
    env.insert(
        "NEXT_CODE_FAKE_GROK_ACP_LOG".to_string(),
        log.display().to_string(),
    );
    GrokBuildProcess {
        command: env!("CARGO_BIN_EXE_next-code-fake-grok-acp").into(),
        args: Vec::new(),
        env,
    }
}
...
async fn dropping_stream_cancels_prompt_and_terminates_subprocess() {
    ...
    process
        .env
        .insert("NEXT_CODE_FAKE_GROK_ACP_HANG".to_string(), "1".to_string());
