//! Vendored (near-verbatim) from `xai-org/grok-build` `xai-grok-announcements`
//! (Apache-2.0) for the next-code Grok Face migration (PR6).
//!
//! Shared announcement types, persistence, and formatting for Grok CLI apps.
//! Upstream note: this crate provides the common logic used by
//! `xai-grok-shell` and `xai-grok-pager` for handling announcements (banner
//! notifications).
//!
//! Adaptation for next-code: the `ts-rs` binding-generation feature and its
//! export test are dropped (nothing in this workspace consumes TS bindings).
//! Persistence goes through `xai_grok_config::grok_home` (the PR3 grok_home
//! shim, already `$GROK_HOME` > `$NEXT_CODE_HOME` > `~/.next-code`) instead of
//! upstream's `xai_grok_tools::util::grok_home` — the pure Rust logic below is
//! otherwise unchanged.
...
/// Read hidden announcement ids from the grok_home announcements state file.
...
/// Write hidden announcement ids to the grok_home announcements state file.
...
fn announcements_state_path() -> PathBuf {
    xai_grok_config::grok_home().join("announcements.json")
}
