...
fn runtime_cursor_api_key_reads_env() {
    ...
    next_code_base::env::set_var("CURSOR_API_KEY", "cursor-env-test");

    assert_eq!(runtime_cursor_api_key().as_deref(), Some("cursor-env-test"));

    if let Some(previous) = previous {
        next_code_base::env::set_var("CURSOR_API_KEY", previous);
    } else {
        next_code_base::env::remove_var("CURSOR_API_KEY");
    }
}
