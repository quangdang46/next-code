...
facade covering only the pager import sites:
...
- `gcs::upload_bytes` — stub, always returns `Err(...)`; **signature matches
  upstream** `upload_bytes<C: StorageConfig>(config, path, bytes, content_type)`
  (verified against pager `trace_cmd.rs`). Includes minimal `StorageConfig` +
  `UploadMethod` / `TraceExportConfig` from `upload_config.rs` so shell can
  re-export the same types upstream does. No GCS/S3 client, no network, no
  `aws-sdk-s3`/`gcloud-storage`/auth.
...
full `upload_config` skip-dir/dedup tables, `events/` — none of these are pager
import sites for PR6, and per the plan this crate intentionally does not carry
S3/GCS upload machinery or `xai-grok-auth`.
