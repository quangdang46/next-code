//! Vendored near-verbatim from upstream `xai-file-utils` `workspace_classifier.rs`
//! (pure logic: simple filesystem-existence checks + path string matching, no
//! network/auth dependency, so it ports faithfully).
