...
pub(crate) use next_code_tui_markdown::{CopyTargetKind, RawCopyTarget};
pub(crate) use next_code_tui_messages::{
...
impl CopyViewportSnapshot {
    ...
    fn inline_image_id_for_label_line(&self, abs_line: usize) -> Option<u64> {
        ...
        prepared
            ...
            .iter()
            .find(|region| {
                region.render == next_code_tui_messages::ImageRegionRender::Fit
...
pub fn draw(frame: &mut Frame, app: &dyn TuiState) {
    ...
    next_code_tui_style::adapt_buffer_for_theme(frame.buffer_mut());
}
...
fn draw_inner(frame: &mut Frame, app: &dyn TuiState) {
        ...
        && (app.swarm_panel_focused() || !super::info_widget::swarm_strip_stands_down_for_dock())
    {
        ...
        if chat_area.width >= 24 {
            ...
            let spinner_frame = (app.animation_elapsed()
                * next_code_tui_render::swarm_gallery::STRIP_SPINNER_FPS)
                as usize;
    ...
    let margins = if onboarding_welcome {
        ...
        let members = app.inline_swarm_members();
        let spinner_frame =
            (app.animation_elapsed() * next_code_tui_render::swarm_gallery::STRIP_SPINNER_FPS) as usize;
