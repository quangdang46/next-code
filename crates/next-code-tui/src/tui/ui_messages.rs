...
fn render_compact_agent_notification(
    ...
    let collapsible = next_code_tui_messages::parse_collapsible_swarm_content(content);
...
pub(crate) fn render_swarm_message(
    ...
    let collapsible = next_code_tui_messages::parse_collapsible_swarm_content(&msg.content);
    ...
    match action {
        "suggest" => {
            ...
            push_compact_discovery_kv(
                &mut content,
                "review",
                "Next Code maintainers only; not approval or availability",
                block_width,
                label_style,
                muted_style,
                MAX_DISCOVERY_DETAIL_LINES,
