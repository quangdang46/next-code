...
impl App {
    ...
    pub(crate) fn open_agents_picker(&mut self) {
        ...
        if let Some(path) = &cwd {
            let _ = registry.load_directory(path, next_code_agent_runtime::SourceKind::ProjectLocal);
        }
                ...
                entries.push(PickerEntry {
                    ...
                    action: {
                        let source_path = match &loaded.source {
                            next_code_agent_runtime::registry::AgentSource::UserGlobal { path } => {
                                path.to_string_lossy().to_string()
                            }
                            next_code_agent_runtime::registry::AgentSource::ProjectLocal { path } => {
                                path.to_string_lossy().to_string()
                            }
                            next_code_agent_runtime::registry::AgentSource::Builtin => String::new(),
                        };
...
pub(crate) fn save_last_assistant_as_agent(session: &crate::session::Session) -> String {
    ...
    let def: next_code_agent_runtime::AgentDefinition = match basic_toml::from_str(toml_content) {
        Ok(d) => d,
