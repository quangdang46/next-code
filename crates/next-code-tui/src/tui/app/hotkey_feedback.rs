...
use next_code_tui_core::keybind::format_binding;
...
impl App {
    ...
    pub(super) fn handle_hotkeys_command(&mut self, trimmed: &str) -> bool {
        ...
        self.push_display_message(next_code_tui_messages::DisplayMessage::system(listing));
        true
    }
    ...
    pub(crate) fn observe_known_hotkey(
        ...
        let macos_option_char = cfg!(target_os = "macos")
            && modifiers.is_empty()
            && next_code_tui_core::keybind::macos_option_char_to_ascii_key(code).is_some();
