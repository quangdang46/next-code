...
fn ensure_swarm_prompt_edit_path(
    ...
    let global_path = next_code_dir.join("swarm-prompt.md");
    if file_has_nonblank_content(&global_path) {
        return Ok(global_path);
    }

    std::fs::create_dir_all(next_code_dir)?;
...
pub(super) fn handle_swarm_prompt_command(app: &mut App, trimmed: &str) -> bool {
    ...
    let next_code_dir = match crate::storage::next_code_dir() {
        Ok(path) => path,
        Err(error) => {
            app.push_display_message(DisplayMessage::error(format!(
    ...
    let path = match ensure_swarm_prompt_edit_path(app.session.working_dir.as_deref(), &next_code_dir) {
        Ok(path) => path,
        Err(error) => {
            app.push_display_message(DisplayMessage::error(format!(
...
pub(super) fn handle_theme_command(app: &mut App, trimmed: &str) -> bool {
    ...
    let rest = trimmed.strip_prefix("/theme").or_else(|| trimmed.strip_prefix("/themes"));
    let rest = match rest {
        Some(r) => r.trim(),
        None => return false,
    };
