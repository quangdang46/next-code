...
pub(super) fn build_resume_command(
    ...
    match target {
        ...
        ResumeTarget::CodexSession { session_id, .. } => {
            ...
            let args = resume_invocation_args(&imported_id, socket);
            let title = format!("🧠 Codex {}", next_code_core::util::truncate_str(session_id, 8));
            (exe, args, title)
        }
        ...
        ResumeTarget::CursorSession { session_id, .. } => {
            ...
            let args = resume_invocation_args(&imported_id, socket);
            let title = format!("💾 Cursor {}", &session_id[..session_id.len().min(8)]);
            (exe, args, title)
        }
    }
}
...
pub(super) fn clipboard_image() -> Option<(String, String)> {
            ...
            && !html_output.stdout.is_empty()
        {
            ...
            if let Some(url) = extract_image_url(&html) {
                crate::logging::info(&format!(
                    ...
                    next_code_core::util::truncate_str(&url, 80)
                ));
    ...
    #[cfg(target_os = "macos")]
    {
        let temp_path = std::env::temp_dir().join("next_code_clipboard.png");
        let script = format!(
