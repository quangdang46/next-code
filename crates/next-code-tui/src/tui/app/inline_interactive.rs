...
struct RemoteModelCatalogCache {
    version: u8,
    #[serde(flatten)]
    snapshot: next_code_provider_core::ModelCatalogSnapshot,
...
fn model_picker_route_is_current(
    ...
    next_code_provider_core::model_route_provider_labels_match(&route.provider, current_provider)
}
...
fn model_picker_route_is_recommended(model_name: &str, route: &PickerOption) -> bool {
    RECOMMENDED_MODELS.contains(&model_name)
        && next_code_provider_core::model_route_metadata_is_recommended(
            model_name,
            &route.provider,
            &route.api_method,
            route.available,
...
fn model_picker_route_provider_matches_key(
    ...
    next_code_provider_core::model_route_provider_matches_key(
        route_provider_key,
...
fn model_picker_route_is_default(
    ...
    if let Some((bare_model, provider_label)) = default_model.rsplit_once('@') {
        return bare_model == model_name
            && next_code_provider_core::model_route_provider_labels_match(
                &route.provider,
                provider_label,
            );
    }
...
impl App {
    pub(super) fn remote_model_catalog_snapshot(
        &self,
    ) -> next_code_provider_core::ModelCatalogSnapshot {
        next_code_provider_core::ModelCatalogSnapshot::new(
    ...
    fn open_model_picker_with_routes(
        ...
        fn route_matches_recent_auth(route_provider: &str, login_provider: &str) -> bool {
            next_code_provider_core::model_route_provider_labels_related(route_provider, login_provider)
        }
        ...
        for target in targets {
            ...
            let name = match target {
                ...
                ResumeTarget::ClaudeCodeSession { session_id, .. } => {
                    format!(
                        "Claude Code {}",
                        next_code_core::util::truncate_str(session_id, 8)
                    )
                }
                ResumeTarget::CodexSession { session_id, .. } => {
                    format!("Codex {}", next_code_core::util::truncate_str(session_id, 8))
                }
                ...
                ResumeTarget::OpenCodeSession { session_id, .. } => {
                    format!("OpenCode {}", next_code_core::util::truncate_str(session_id, 8))
                }
...
"#,
    ...
    pub(super) fn picker_fuzzy_score(pattern: &str, text: &str) -> Option<i32> {
        next_code_fuzzy::fuzzy_score_tokens(pattern, text)
    }
...
mod tests {
    ...
    fn model_picker_current_route_allows_provider_aliases() {
        assert!(next_code_provider_core::model_route_provider_labels_match(
            "Anthropic",
            "Claude"
        ));
        assert!(next_code_provider_core::model_route_provider_labels_match(
