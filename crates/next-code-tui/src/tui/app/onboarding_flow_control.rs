...
impl App {
    ...
    pub(super) fn is_new_user_install() -> bool {
        let Ok(dir) = crate::storage::next_code_dir() else {
            return true;
        };
    ...
    fn has_established_native_session_history(next_code_dir: &std::path::Path) -> bool {
        const ESTABLISHED_SESSION_THRESHOLD: usize = 10;
        let Ok(entries) = std::fs::read_dir(next_code_dir.join("sessions")) else {
            return false;
        };
