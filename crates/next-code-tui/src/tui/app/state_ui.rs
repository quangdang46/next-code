...
impl App {
    ...
    pub(super) fn save_input_for_reload(&self, session_id: &str) {
        ...
        if let Ok(next_code_dir) = crate::storage::next_code_dir() {
            let path = next_code_dir.join(format!("client-input-{}", session_id));
    ...
    pub(super) fn restore_input_for_reload(session_id: &str) -> Option<RestoredReloadInput> {
        let next_code_dir = crate::storage::next_code_dir().ok()?;
        let path = next_code_dir.join(format!("client-input-{}", session_id));
        if !path.exists() {
            return None;
        }
...
pub(super) fn handle_info_command(app: &mut App, trimmed: &str) -> bool {
    ...
    if trimmed == "/agents save" || trimmed.starts_with("/agents save ") {
        let content =
            super::inline_interactive::openers::save_last_assistant_as_agent(&app.session);
        app.push_display_message(DisplayMessage::system(content).with_title("Agent Save"));
    ...
    if trimmed == "/permissions" || trimmed.starts_with("/permissions ") {
        ...
        content.push_str(
            "  `default`             — Legacy default; read-only + stateful-safe auto-allow\n",
        );
        content.push_str(
            "  `accept-edits`        — Allow edits (reads + writes), deny spawn/network\n",
        );
        content.push_str(
            "  `plan`                — Read-only enforcement; write effects are denied\n",
        );
        content
            .push_str("  `auto`                — LLM-classified mode; same default as `default`\n");
        content.push_str(
            "  `dont-ask`            — Restricted surface; non-allow-listed commands denied\n",
        );
        content
            .push_str("  `bypass-permissions`  — Skip all evaluation (deny rules still apply)\n\n");
        ...
        app.push_display_message(DisplayMessage::system(content).with_title("Permissions"));
        app.set_status_notice("Permissions");
        return true;
    }

    if trimmed == "/version" {
        let version = next_code_build_meta::VERSION;
    ...
    if trimmed == "/info" {
        ...
        if let Some(ref provider_id) = app.provider_session_id {
            info.push_str(&format!(
                "Provider Session: {}...\n",
                next_code_core::util::truncate_str(provider_id, 16)
            ));
        }
