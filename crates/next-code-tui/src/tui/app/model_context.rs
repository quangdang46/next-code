...
impl App {
    ...
    fn current_route_api_method(&self) -> Option<String> {
        ...
        match (provider_name.as_str(), credential) {
            ("claude", Some(next_code_provider_core::ResolvedCredential::Oauth)) => {
                Some("claude-oauth".to_string())
            }
            ("claude", Some(next_code_provider_core::ResolvedCredential::ApiKey)) => {
                Some("claude-api".to_string())
            }
            ("openai", Some(next_code_provider_core::ResolvedCredential::Oauth)) => {
                Some("openai-oauth".to_string())
            }
            ("openai", Some(next_code_provider_core::ResolvedCredential::ApiKey)) => {
