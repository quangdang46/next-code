...
fn parse_changelog() -> Vec<ChangelogEntry<'static>> {
    let changelog: &'static str = next_code_build_meta::CHANGELOG;
    parse_changelog_from_impl(changelog)
}
...
/// Reads the last-seen commit hash from ~/.next-code/last_seen_changelog,
