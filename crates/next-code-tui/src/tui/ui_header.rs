...
pub(super) fn build_persistent_header(app: &dyn TuiState, width: u16) -> Vec<Line<'static>> {
    ...
    let client_version_full = server_name
        .as_ref()
        .map(|_| next_code_build_meta::VERSION.to_string());
    ...
    let version_text = if client_version_label.is_some() {
        ...
        let tag = next_code_build_meta::GIT_TAG;
...
mod tests {
    ...
    fn persistent_header_labels_server_and_client_versions() {
        ...
        let client_version = compact_version_label(next_code_build_meta::VERSION);
        assert!(
    ...
    fn persistent_header_keeps_git_hash_when_semvers_match_but_builds_differ() {
        let mut app = create_test_app();
        let client_semver = compact_version_label(next_code_build_meta::VERSION);
        ...
        assert!(
            client_line.contains(&format!("· {}", next_code_build_meta::VERSION)),
