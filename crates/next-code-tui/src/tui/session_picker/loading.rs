...
fn session_list_disk_cache_path() -> Result<PathBuf> {
    Ok(storage::next_code_dir()?.join("cache/session-picker-list-v1.json"))
}
...
pub fn load_cached_sessions_grouped() -> Option<(Vec<ServerGroup>, Vec<SessionInfo>)> {
    let sessions_dir = storage::next_code_dir().ok()?.join("sessions");
    ...
    sessions
        ...
        .map(|session| {
            ...
            let short_name = working_dir
                ...
                .unwrap_or_else(|| {
                    format!("claude {}", next_code_core::util::truncate_str(&session_id, 8))
                });
...
fn load_codex_session_stub(path: &Path) -> Result<Option<SessionInfo>> {
    ...
    let working_dir = meta
        ...
        .and_then(|v| v.as_str())
        .map(|s| s.to_string());
    let short_name = format!("codex {}", next_code_core::util::truncate_str(&session_id, 8));
    let title = format!(
        "Codex session {}",
...
fn load_pi_session_stub(path: &Path) -> Result<Option<SessionInfo>> {
    ...
    let working_dir = header
        ...
        .and_then(|v| v.as_str())
        .map(|s| s.to_string());
    let short_name = format!("pi {}", next_code_core::util::truncate_str(&session_id, 8));
    let title = format!(
        "Pi session {}",
        next_code_core::util::truncate_str(&session_id, 8)
    );
...
fn load_cursor_session_stub(path: &Path) -> Result<Option<SessionInfo>> {
    ...
    let short_name = working_dir
        ...
        .and_then(|name| name.to_str())
        .map(|name| name.to_string())
        .unwrap_or_else(|| format!("cursor {}", next_code_core::util::truncate_str(&session_id, 8)));
