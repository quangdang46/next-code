pub use next_code_tui_markdown::{
...
) -> next_code_tui_markdown::DiagramDisplayMode {
    match mode {
        crate::config::DiagramDisplayMode::None => next_code_tui_markdown::DiagramDisplayMode::None,
        crate::config::DiagramDisplayMode::Margin => next_code_tui_markdown::DiagramDisplayMode::Margin,
        crate::config::DiagramDisplayMode::Pinned => next_code_tui_markdown::DiagramDisplayMode::Pinned,
    }
}

fn from_markdown_diagram_mode(
    mode: next_code_tui_markdown::DiagramDisplayMode,
) -> crate::config::DiagramDisplayMode {
    match mode {
        next_code_tui_markdown::DiagramDisplayMode::None => crate::config::DiagramDisplayMode::None,
        next_code_tui_markdown::DiagramDisplayMode::Margin => crate::config::DiagramDisplayMode::Margin,
        next_code_tui_markdown::DiagramDisplayMode::Pinned => crate::config::DiagramDisplayMode::Pinned,
    }
}

fn to_markdown_spacing_mode(
    mode: crate::config::MarkdownSpacingMode,
) -> next_code_tui_markdown::MarkdownSpacingMode {
    match mode {
        crate::config::MarkdownSpacingMode::Compact => {
            next_code_tui_markdown::MarkdownSpacingMode::Compact
        }
        crate::config::MarkdownSpacingMode::Document => {
            next_code_tui_markdown::MarkdownSpacingMode::Document
        }
    }
}
...
) -> next_code_tui_markdown::LatexRenderingMode {
    match mode {
        crate::config::LatexRenderingMode::None => next_code_tui_markdown::LatexRenderingMode::None,
        crate::config::LatexRenderingMode::Unicode => {
            next_code_tui_markdown::LatexRenderingMode::Unicode
        }
        crate::config::LatexRenderingMode::Image => next_code_tui_markdown::LatexRenderingMode::Image,
    }
}

pub fn install_next_code_markdown_hooks() {
    next_code_tui_markdown::set_latex_log_hook(|error| {
        crate::logging::warn(&format!(
            "LaTeX image rendering fell back to Unicode: {error}"
        ));
    });
    next_code_tui_markdown::set_config_snapshot_hook(|| {
        let cfg = crate::config::config();
        next_code_tui_markdown::MarkdownConfigSnapshot {
    ...
    next_code_tui_markdown::set_memory_snapshot_hook(|| {
        let snapshot = crate::process_memory::snapshot_with_source("client:markdown:memory");
        next_code_tui_markdown::ProcessMemorySnapshot {
...
pub fn set_diagram_mode_override(mode: Option<crate::config::DiagramDisplayMode>) {
    next_code_tui_markdown::set_diagram_mode_override(mode.map(to_markdown_diagram_mode));
}

pub fn get_diagram_mode_override() -> Option<crate::config::DiagramDisplayMode> {
    next_code_tui_markdown::get_diagram_mode_override().map(from_markdown_diagram_mode)
}
...
pub fn with_diagram_mode_scope<T>(
    ...
    next_code_tui_markdown::with_diagram_mode_scope(to_markdown_diagram_mode(mode), f)
}

pub fn with_deferred_mermaid_render_context<T>(f: impl FnOnce() -> T) -> T {
    next_code_tui_markdown::with_deferred_mermaid_render_context(f)
}
