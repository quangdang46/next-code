...
use next_code_tui_messages::{ImageRegion, ImageRegionRender, PreparedMessages};
...
impl AnchoredInlineImages {
    ...
    pub(crate) fn unplaced_items(
        &self,
        messages: &[next_code_tui_messages::DisplayMessage],
    ) -> Vec<InlineImageItem> {
...
pub(crate) fn build_section(
    ...
    let plain: Vec<String> = lines
        .iter()
        .map(next_code_tui_render::line_plain_text)
        .collect();
...
mod tests {
    ...
    fn build_section_emits_one_fit_region_per_image_with_label() {
        ...
        for region in &section.image_regions {
            ...
            assert!(
                next_code_tui_render::line_plain_text(first).trim().is_empty(),
        ...
        let label_line = next_code_tui_render::line_plain_text(&section.wrapped_lines[1]);
        assert!(
    ...
    fn visible_label_line_advertises_hide_badge() {
        ...
        let text = next_code_tui_render::line_plain_text(&line);
    ...
    fn unplaced_items_falls_back_for_missing_anchor_targets() {
        use next_code_tui_messages::DisplayMessage;
    ...
    fn anchored_image_lines_round_trip_through_region_scan() {
        ...
        for offset in 1..rows as usize {
            ...
            assert!(
                next_code_tui_render::line_plain_text(line).trim().is_empty(),
