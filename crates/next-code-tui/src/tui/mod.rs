...
pub mod permissions {
    pub use next_code_tui_permissions::*;
}
mod remote_diff;
pub mod screenshot;
...
pub use next_code_tui_workspace::workspace_map;
pub use next_code_tui_workspace::workspace_map_widget;
...
pub trait TuiState {
    ...
    fn status_line_config(&self) -> &next_code_config_types::StatusLineConfig {
        static DEFAULT: std::sync::OnceLock<next_code_config_types::StatusLineConfig> =
            std::sync::OnceLock::new();
        DEFAULT.get_or_init(next_code_config_types::StatusLineConfig::default)
    }
...
pub(crate) const REDRAW_SWARM_SPINNER: Duration =
    Duration::from_millis(next_code_tui_render::swarm_gallery::STRIP_SPINNER_FRAME_MS);
...
pub(crate) fn subscribe_metadata(
    ...
    resolve_subscribe_metadata(
        ...
        remote_working_dir,
        next_code_selfdev_types::client_selfdev_requested(),
    )
}
...
pub fn display_messages_from_session(session: &crate::session::Session) -> Vec<DisplayMessage> {
    let mut messages = next_code_tui_messages::display_messages_from_rendered_messages(
