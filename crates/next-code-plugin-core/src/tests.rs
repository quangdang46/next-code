...
mod tests {
    ...
    fn plugin_version_fields() {
        let ver = PluginVersion {
            ...
            next_code_min_version: Some(semver::Version::new(0, 9, 0)),
            next_code_max_version: None,
        };
        assert_eq!(ver.semver.to_string(), "1.2.3");
        assert_eq!(ver.next_code_min_version.unwrap().major, 0);
        assert!(ver.next_code_max_version.is_none());
    }
