fn main() {
    ...
    // NEXT_CODE_MMDR_SIZE_API_DISABLE=1 (or legacy JCODE_MMDR_SIZE_API_DISABLE=1)
    // to force the legacy SVG-retarget path (e.g. when testing against an older
    // renderer via a Cargo patch).
    println!("cargo:rerun-if-env-changed=NEXT_CODE_MMDR_SIZE_API_DISABLE");
    ...
    println!("cargo:rerun-if-env-changed=NEXT_CODE_MMDR_SIZE_API_AVAILABLE");
    println!("cargo:rerun-if-env-changed=JCODE_MMDR_SIZE_API_AVAILABLE");
    if std::env::var_os("NEXT_CODE_MMDR_SIZE_API_DISABLE").is_none()
        && std::env::var_os("JCODE_MMDR_SIZE_API_DISABLE").is_none()
    {
