...
fn layout_cache_stays_bounded_under_50_distinct_renders() {
    ...
    next_code_tui_mermaid::reset_debug_stats();
    let stats_before = next_code_tui_mermaid::debug_stats();
    ...
    for idx in 0..RENDERS {
        ...
        match next_code_tui_mermaid::render_mermaid_untracked(&content, Some(90)) {
            next_code_tui_mermaid::RenderResult::Image { .. } => {}
            next_code_tui_mermaid::RenderResult::Error(error) => {
                render_errors += 1;
    ...
    let stats = next_code_tui_mermaid::debug_stats();
    let memory = next_code_tui_mermaid::debug_memory_profile();
