...
impl<'a> IntoLineInput<'a> for Cow<'a, str> {
    fn into_line_input(self) -> LineInput<'a> {
        // crates.io ratatui 0.28: `Line` has no `From<Cow<'_, str>>` (Grok's
        // ratatui 0.29 path did). Convert explicitly.
        match self {
            Cow::Borrowed(s) => LineInput::Owned(Line::from(s)),
            Cow::Owned(s) => LineInput::Owned(Line::from(s)),
        }
    }
}
