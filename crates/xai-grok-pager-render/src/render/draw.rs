...
///
/// Uses [`SharedTermWriter`] because crates.io ratatui **0.28** has no
/// `unstable-backend-writer` feature (Grok pins ratatui 0.29 where
/// `CrosstermBackend::writer_mut` is available). Idle frames still need to
/// [`TermWriter::discard`] without flushing `BeginSynchronizedUpdate`.
pub type PagerTerminal = xai_ratatui_inline::Terminal<CrosstermBackend<SharedTermWriter>>;

/// Cloneable handle around [`TermWriter`] for backend + idle discard.
#[derive(Clone)]
pub struct SharedTermWriter {
...
impl Write for SharedTermWriter {
...
pub fn draw_frame(
    ...
    if !has_changes && !post_flush_wrote_cursor && action == CursorAction::None {
        // Prefer SharedTermWriter over CrosstermBackend::writer_mut (unavailable
        // on crates.io ratatui 0.28 without unstable-backend-writer).
        writer.discard();
        return;
    }
...
mod tests {
    ...
    fn idle_frame_emits_zero_bytes() {
        ...
        let (tx, rx) = mpsc::channel::<WriterPayload>();
        let writer =
            SharedTermWriter::new(tx, WriterSync::new()).expect("single test writer");
