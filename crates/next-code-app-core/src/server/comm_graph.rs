...
//! `next_code_plan::dag`. Each handler lifts the swarm's current `VersionedPlan` into
//! a `TaskGraph` (via `next_code_plan::bridge`), applies the engine op (which enforces
...
use next_code_plan::bridge::{apply_task_graph, parse_kind, to_task_graph};
use next_code_plan::dag::{self, HandoffArtifact, NodeSpec, NodeStatus, TaskGraph};
