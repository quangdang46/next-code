...
/// `next_code_swarm_core` so the server, tools, and prompts all agree on the one
...
pub(super) use next_code_swarm_core::MAX_SWARM_MEMBERS;
...
pub(super) async fn set_member_task_label(
    ...
    let Some(label) = next_code_swarm_core::derive_swarm_task_label(task_text) else {
        return;
    };
    let swarm_id = {
        let mut members = swarm_members.write().await;
