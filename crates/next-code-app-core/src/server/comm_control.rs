...
use next_code_agent_runtime::SoftInterruptSource;
...
fn deep_mode_assignment_content(
    ...
    if is_gate {
        ...
        let low_confidence_siblings: Vec<String> =
            next_code_plan::bridge::low_confidence_completed_ids(plan)
                .into_iter()
                .filter(|id| audited.contains(id.as_str()))
                .collect();
        next_code_swarm_core::append_deep_gate_instructions(
            content,
            item_id,
            &audited_ids,
        ...
        next_code_swarm_core::append_deep_node_instructions(content, item_id)
    }
}
...
async fn task_snapshot_for(
    ...
    let hydrated = next_code_plan::bridge::hydrate_assignment(plan, task_id, &item.content);
...
async fn handle_comm_assign_task_with_mode(
    ...
    let (selected_task_id, task_content, participant_ids, plan_item_count, blocked_reason) = {
        ...
        if found.is_some() {
            ...
            let hydrated =
                next_code_plan::bridge::hydrate_assignment(plan, &item_id, &effective_content);
            let content =
...
async fn require_plan_driver_swarm(
    ...
    let is_deep_participant = {
        ...
        plans
            .get(&swarm_id)
            .map(|plan| {
                next_code_plan::bridge::parse_mode(&plan.mode) == next_code_plan::dag::Mode::Deep
