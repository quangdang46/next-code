...
fn load_testers() -> Result<Vec<serde_json::Value>> {
    let path = crate::storage::next_code_dir()?.join("testers.json");
    if path.exists() {
...
async fn spawn_tester(opts: serde_json::Value) -> Result<String> {
    ...
    let debug_cmd = std::env::temp_dir().join(format!("next_code_debug_cmd_{}", id));
    let debug_resp = std::env::temp_dir().join(format!("next_code_debug_response_{}", id));
    let stdout_path = std::env::temp_dir().join(format!("next_code_tester_stdout_{}", id));
    let stderr_path = std::env::temp_dir().join(format!("next_code_tester_stderr_{}", id));
