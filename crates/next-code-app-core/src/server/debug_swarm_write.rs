...
fn debug_specs(specs: Vec<DebugNodeSpec>) -> Vec<next_code_plan::dag::NodeSpec> {
    specs
        .into_iter()
        .map(|s| next_code_plan::dag::NodeSpec {
            id: Some(s.id),
            content: s.content,
            kind: next_code_plan::bridge::parse_kind(s.kind.as_deref()),
...
async fn handle_debug_graph_op(arg: &str, ctx: &DebugSwarmWriteContext<'_>) -> String {
    use next_code_plan::bridge::{apply_task_graph, to_task_graph};
    use next_code_plan::dag;
