...
pub(super) async fn maybe_handle_server_state_command(
    ...
    shutdown_signals: &Arc<RwLock<HashMap<String, next_code_agent_runtime::InterruptSignal>>>,
    ...
    if cmd == "memory-judge" || cmd == "memory:judge" || cmd == "server:memory-judge" {
        ...
        return Ok(Some(
            serde_json::to_string_pretty(&next_code_base::memory_judge_metrics::snapshot())
                .unwrap_or_else(|_| "{}".to_string()),
        ));
    }
    ...
    let (summary, actions) = match decision.primary_cause {
        "runaway_live_session_population" => (
            ...
            vec![
                serde_json::json!({
                    ...
                    "commands": ["next-code debug 'swarm:list'", "next-code debug 'server:memory-incident'"]
                }),
                serde_json::json!({
                    "priority": 2,
                ...
                serde_json::json!({
                    ...
                    "commands": ["next-code debug 'server:memory-incident'", "python scripts/analyze_runtime_memory_log.py --days 1"]
                }),
                serde_json::json!({
                    "priority": 4,
        ...
        "unattributed_live_heap" => (
            ...
            vec![
                serde_json::json!({
                    ...
                    "commands": ["next-code debug 'server:memory'", "python scripts/analyze_runtime_memory_log.py --days 1"]
                }),
                serde_json::json!({
                    "priority": 2,
                    "action": "Escalate to a jemalloc-prof build and heap dump if attribution remains below 50% of live heap.",
                    "commands": ["next-code debug 'allocator:profile:on'", "next-code debug 'allocator:profile:dump /tmp/next-code-server.heap'"]
                }),
            ],
        ),
        ...
        _ => (
            ...
            vec![serde_json::json!({
                ...
                "commands": ["next-code debug 'server:memory-incident'"]
            })],
        ),
    };
