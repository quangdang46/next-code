...
use anyhow::Result;
use next_code_agent_runtime::{InterruptSignal, SoftInterruptSource};
...
pub(super) fn debug_message_timeout_secs() -> Option<u64> {
    let raw = product_env("DEBUG_MESSAGE_TIMEOUT_SECS").ok()?;
...
pub(super) async fn execute_debug_command(
    ...
    if trimmed == "mcp" || trimmed == "mcp:servers" {
        ...
        let config_path = if let Ok(next_code_dir) = crate::storage::next_code_dir() {
            let path = next_code_dir.join("mcp.json");
            if path.exists() {
    ...
    if trimmed == "reload" {
        ...
        if !target_binary.exists() {
            return Err(anyhow::anyhow!(format!(
                "No binary found at {}. Run 'next-code self-dev --build' first, or build with 'scripts/dev_cargo.sh build --profile selfdev -p next-code --bin next-code' and publish current.",
        ...
        let next_code_dir = crate::storage::next_code_dir()?;
        let info_path = next_code_dir.join("reload-info");
...
mod tests {
    ...
    async fn debug_tool_selfdev_reload_returns_promptly_for_direct_execution() {
        ...
        let _test_session = EnvGuard::set("NEXT_CODE_TEST_SESSION", "1");
        let _debug_control = EnvGuard::set("NEXT_CODE_DEBUG_CONTROL", "1");
