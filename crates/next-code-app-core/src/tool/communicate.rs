...
use next_code_swarm_core::validate_swarm_tldr;
...
impl RunPlanReporter {
    ...
    /// Record an explicit checkpoint (a NEXT_CODE_CHECKPOINT-style milestone) on
...
/// The `next-code login` invocation most likely to fix a credential wave for
...
fn credential_login_fix_hint(provider: Option<&str>) -> String {
    ...
    format!("`next-code login --provider {target}`")
}
...
fn plan_status_budget_line(summary: &PlanGraphStatus, deep_cap: usize) -> Option<String> {
    ...
    let budget_label = if budget == usize::MAX {
        format!("{} (member cap)", next_code_swarm_core::MAX_SWARM_MEMBERS)
    } else {
        budget.to_string()
    };
    ...
    let effective_budget = if budget == usize::MAX {
        next_code_swarm_core::MAX_SWARM_MEMBERS
    } else {
        budget
    };
...
fn format_swarm_model_list(
    ...
    model_routes: &[next_code_provider_core::ModelRoute],
) -> String {
...
impl CommunicateTool {
    pub fn new() -> Self {
        ...
        let description = if swarm_prompt.is_empty() {
            ...
            format!(
                "{BASE_DESCRIPTION}\n\nSwarm prompt (user-tunable via ~/.next-code/swarm-prompt.md):\n{swarm_prompt}"
            )
        };
        Self { description }
    }
