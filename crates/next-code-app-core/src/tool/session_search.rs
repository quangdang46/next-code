...
pub fn spawn_recent_index_warmup() {
    tokio::task::spawn_blocking(|| {
        ...
        let next_code_count = (|| -> Result<usize> {
            ...
            let _ = next_code_index_candidates(&collection.files, &empty_query)?;
        ...
        if !files.is_empty() {
            ...
            if !files.is_empty() {
                ...
                let mut candidates = if options.exhaustive {
                    ...
                    match next_code_index_candidates(&files, query) {
...
fn index_dir() -> Result<PathBuf> {
    Ok(storage::next_code_dir()?.join("cache"))
}
...
fn claude_index_candidates(
    sessions: &[next_code_import_core::ClaudeCodeSessionInfo],
    query: &QueryProfile,
) -> Vec<next_code_import_core::ClaudeCodeSessionInfo> {
    let index = index_dir()
                ...
                for session in chunk {
                    ...
                    records.push(ExternalSessionRecord {
                        ...
                        short_name: Some(format!(
                            "claude {}",
                            next_code_core::util::truncate_str(&session.session_id, 8)
                        )),
                        title: Some(title),
...
fn external_text_matches_query(text: &str, query: &QueryProfile) -> bool {
    next_code_session_types::normalized_session_search_text_matches(&text.to_lowercase(), query)
}
...
fn append_session_results(
    ...
    if !next_code_session_matches_filters(session, options) {
        return;
    }
