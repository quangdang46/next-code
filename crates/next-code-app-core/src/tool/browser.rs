...
async fn ensure_firefox_ready() -> Result<Option<String>> {
    ...
    let mut message = String::from(
        "Browser automation is not ready yet. Use the browser tool with action='status' to confirm current state. Only run action='setup' or `next-code browser setup` for first-time install or repair when the bridge is not already ready.\n",
    );
    if !status.binary_installed {
...
fn temp_screenshot_path() -> PathBuf {
    let ts = SystemTime::now()
        ...
        .map(|d| d.as_millis())
        .unwrap_or(0);
    std::env::temp_dir().join(format!("next-code-browser-{}.png", ts))
}
