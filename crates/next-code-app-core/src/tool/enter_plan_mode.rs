...
plan, then call ExitPlanMode for user approval before implementing."#
    ...
    async fn execute(&self, _input: Value, ctx: ToolContext) -> Result<ToolOutput> {
        ...
        let plan_path = ctx
            .working_dir
            .as_ref()
            .map(|d| d.join("plan.md"))
            .or_else(|| std::env::current_dir().ok().map(|d| d.join("plan.md")));
        let path_hint = plan_path
            .as_ref()
            .map(|p| format!(" Write the plan to `{}`.", p.display()))
            .unwrap_or_else(|| " Write the plan to `plan.md` in the working directory.".into());
        Ok(ToolOutput::new(format!(
            "Entered plan mode. Explore the codebase and design an approach.{path_hint} \
DO NOT write or edit any other files. When ready, call ExitPlanMode to present \
