...
impl SelfDevTool {
    ...
    pub(super) async fn do_find_config(&self, ctx: &ToolContext) -> Result<ToolOutput> {
        let next_code_home = storage::next_code_dir().ok();
        let config_path = next_code_home.as_ref().map(|home| home.join("config.toml"));
        ...
        let metadata = json!({
            ...
            "next_code_home": next_code_home.as_ref().map(|p| p.display().to_string()),
    ...
    pub(super) async fn do_reload_to_newer_build(&self, _ctx: &ToolContext) -> Result<ToolOutput> {
        ...
        let hash = next_code_build_meta::GIT_HASH.to_string();
