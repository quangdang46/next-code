...
use next_code_tool_types::ToolOutput;
use serde_json::json;
...
pub fn ocr(region: Option<[f64; 4]>) -> Result<ToolOutput> {
    ...
    let tmp = std::env::temp_dir().join(format!("next_code_ocr_{}.png", std::process::id()));
...
"#,
    ...
    let out = Command::new(&swift)
        .arg("-")
        .arg(image_path)
        .env("NEXT_CODE_OCR_IMG", image_path)
