...
impl Agent {
    ...
    async fn maybe_auto_best_of_n(&self, user_message: &str) -> Result<Option<String>> {
        ...
        let keyword_triggered = detections
            .iter()
            .any(|d| d.entry.workflow == next_code_keywords::WorkflowKind::BestOfN);

        // Sticky mode from a prior $bestofn in this project.
        let working_dir = self.working_dir().map(std::path::PathBuf::from);
        let sticky = next_code_keywords::load_state(working_dir.as_deref())
            .active_modes
            .iter()
    ...
    async fn maybe_auto_best_of_n_streaming(
        ...
        let sticky = next_code_keywords::load_state(Some(&working_dir))
            .active_modes
            .iter()
            .any(|m| m.workflow == next_code_keywords::WorkflowKind::BestOfN && !m.is_expired());
    ...
    pub async fn run_once_streaming_mpsc(
        ...
        // UserPromptExpansion hook (fire-and-forget, diagnostic)
        {
            ...
            if !handlers.is_empty() {
                ...
                let handlers: Vec<next_code_hooks::HookHandlerConfig> =
    ...
    pub async fn repl(&mut self) -> Result<()> {
        println!("Next Code - Coding Agent");
