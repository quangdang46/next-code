...
pub fn spawn_resume_in_new_terminal_with_context(
    ...
    let mut next_code_args: Vec<String> = Vec::new();
    if let Some(provider_arg) = resume_provider_arg(provider_key) {
        next_code_args.push("--provider".to_string());
        next_code_args.push(provider_arg.to_string());
    }
    next_code_args.push("--resume".to_string());
    next_code_args.push(session_id.to_string());

    let hook_command = crate::terminal_launch::TerminalCommand::new(exe, next_code_args.clone())
