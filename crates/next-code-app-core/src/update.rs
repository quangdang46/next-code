...
const GITHUB_REPO: &str = "quangdang46/next-code";
...
pub fn is_release_build() -> bool {
    next_code_build_meta::is_release_build()
}

fn current_update_semver() -> &'static str {
    next_code_build_meta::UPDATE_SEMVER
}
...
fn metadata_path() -> Result<PathBuf> {
    Ok(storage::next_code_dir()?.join("update_metadata.json"))
}

fn source_build_root() -> Result<PathBuf> {
    Ok(storage::next_code_dir()?.join("builds").join("source"))
}
...
pub fn should_auto_update() -> bool {
    if product_env("NO_AUTO_UPDATE").is_ok() {
        return false;
    }
...
/// Build next-code from source by cloning/pulling the repo and running cargo build
fn build_from_source() -> Result<PathBuf> {
    ...
    if repo_dir.join(".git").exists() {
        ...
        let output = std::process::Command::new("git")
            .args([
                "clone", "--depth", "1", "--branch", "main", &clone_url, "next-code",
            ])
...
pub fn download_and_install_blocking_with_progress(
    ...
    let temp_path = temp_dir.join(format!("next-code-update-{}", std::process::id()));
    ...
    if asset.name.ends_with(".tar.gz") {
        ...
        let Some(extracted_binary) = extracted_binary else {
            anyhow::bail!("Could not find next-code binary inside tar.gz archive");
        };
...
pub fn check_and_maybe_update(auto_install: bool) -> UpdateCheckResult {
    ...
    match check_for_update_blocking() {
        Ok(Some(release)) => {
            let current = next_code_build_meta::VERSION.to_string();
...
mod tests {
    use super::*;
    use next_code_update_core::parse_sha256sums;
    ...
    fn test_asset_name() {
        ...
        assert!(name.starts_with("next-code-"));
    }
    ...
    fn test_parse_sha256sums_accepts_standard_and_binary_lines() {
        ...
        let contents = format!(
            "# generated by release workflow\n{}  next-code-linux-x86_64.tar.gz\r\n{} *next-code-windows-x86_64.exe\n",
            digest_a, digest_b
        );
        let parsed = parse_sha256sums(&contents).unwrap();
        assert_eq!(
            parsed.get("next-code-linux-x86_64.tar.gz").map(String::as_str),
            Some(digest_a.as_str())
        );
        assert_eq!(
            parsed.get("next-code-windows-x86_64.exe").map(String::as_str),
    ...
    fn test_verify_asset_checksum_text_accepts_matching_digest() {
        ...
        let contents = format!("{}  next-code-linux-x86_64.tar.gz\n", digest);
        verify_asset_checksum_text(&contents, "next-code-linux-x86_64.tar.gz", bytes).unwrap();
    }
    ...
    fn test_verify_asset_checksum_text_rejects_mismatch() {
        ...
        let contents = format!("{}  next-code-linux-x86_64.tar.gz\n", wrong);
        let err = verify_asset_checksum_text(&contents, "next-code-linux-x86_64.tar.gz", b"actual")
            .unwrap_err()
            .to_string();
    ...
    fn test_parse_sha256sums_rejects_invalid_digest() {
        let err = parse_sha256sums("not-a-sha  next-code-linux-x86_64.tar.gz\n")
            .unwrap_err()
            .to_string();
    ...
    fn test_summarize_git_pull_failure_diverged() {
        ...
        assert_eq!(
            ...
            next_code_update_core::GIT_PULL_DIVERGED_SUMMARY
        );
        assert!(next_code_update_core::summary_is_divergence(
            &summarize_git_pull_failure(stderr)
        ));
    }
