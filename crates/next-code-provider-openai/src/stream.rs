...
use bytes::Bytes;
use futures::Stream;
use next_code_message_types::{StreamEvent, sanitize_tool_id};
...
fn truncated_stream_payload_context(data: &str) -> String {
    next_code_core::util::truncate_str(&data.trim().replace("\n", "\\n"), 240).to_string()
}
...
pub fn parse_openai_response_event(
    ...
    if is_websocket_fallback_notice(data) {
        next_code_logging::warn(&format!("OpenAI stream transport notice: {}", data.trim()));
        return None;
    }
...
fn handle_openai_image_generation_item(
    ...
    let metadata_path_string = match serde_json::to_vec_pretty(&metadata).ok().and_then(|bytes| {
        ...
        None => {
            next_code_logging::warn("Failed to save OpenAI generated image metadata");
            None
        }
    };
    ...
    fn parse_next_event(&mut self) -> Option<StreamEvent> {
        ...
        while let Some(pos) = self.buffer.find("\n\n") {
            ...
            for line in event_str.lines() {
                if let Some(data) = next_code_core::util::sse_data_line(line) {
