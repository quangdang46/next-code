...
fn create_hotkey_shortcut(use_alacritty: bool) -> Result<()> {
    ...
    let hotkey_dir = storage::next_code_dir()?.join("hotkey");
...
pub(super) fn reinstall_windows_launch_hotkeys() {
    ...
    match refresh_windows_launch_hotkeys() {
        Ok(()) => next_code_logging::info("Reinstalled Windows launch hotkeys after config change"),
...
"#,
    ...
    if output.status.success() {
        ...
        if stdout.contains("OK") {
            ...
            let _ = state.save();
            next_code_logging::info(&format!("Created desktop shortcut: {}", shortcut_path));
        }
    }
