...
#[cfg(windows)]
mod windows_setup;
#[cfg(any(all(test, not(windows)), target_os = "macos"))]
...
impl SetupHintsState {
    fn path() -> Result<PathBuf> {
        Ok(storage::next_code_dir()?.join("setup_hints.json"))
    }
...
#[cfg(any(test, target_os = "macos", target_os = "linux", windows))]
fn load_launch_hotkeys_config() -> next_code_config_types::LaunchHotkeysConfig {
    #[derive(serde::Deserialize, Default)]
    struct Wrapper {
        #[serde(default)]
        launch_hotkeys: next_code_config_types::LaunchHotkeysConfig,
    }
    let Ok(dir) = storage::next_code_dir() else {
...
pub fn record_launch_dirs(dir: &std::path::Path, repo_dir: Option<&std::path::Path>) {
    #[cfg(any(target_os = "macos", target_os = "linux", windows))]
    {
        if let Err(err) = record_launch_dirs_inner(dir, repo_dir) {
            next_code_logging::warn(&format!("failed to record launch dirs for hotkeys: {err}"));
        }
    }
...
pub fn launch_next_code_in_macos_terminal(extra_args: &[String]) -> Result<()> {
    ...
    let command = match macos_terminal::no_automation_launch(terminal, &shell_command) {
        ...
        macos_terminal::NoAutomationLaunch::CommandFile { app } => {
            let dir = storage::next_code_dir()?.join("launcher");
...
fn run_macos_hotkey_listener() -> Result<()> {
    ...
    use next_code_terminal_launch::{TerminalCommand, spawn_command_in_new_terminal_with};
            ...
            && state.launch_hotkey_tracking_version < LAUNCH_HOTKEY_TRACKING_VERSION
        {
            match windows_setup::refresh_windows_launch_hotkeys() {
                Ok(()) => {
                    state.launch_hotkey_tracking_version = LAUNCH_HOTKEY_TRACKING_VERSION;
                    let _ = state.save();
                    next_code_logging::info("Migrated Windows launch hotkeys to usage tracking");
                }
                Err(err) => next_code_logging::warn(&format!(
...
fn reload_compositor_config(comp: linux_env::LinuxCompositor) {
        ...
        .status()
    {
        ...
        Ok(status) => next_code_logging::warn(&format!(
            "{} exited with {status} while reloading hotkey binds",
            cmd[0]
        )),
        Err(err) => next_code_logging::warn(&format!("failed to run {} reload: {err}", cmd[0])),
    }
}
...
pub fn maybe_show_keymap_conflict_hint(
    keybindings: &next_code_config_types::KeybindingsConfig,
) -> Option<StartupHints> {
    if !io::stdin().is_terminal() || !io::stderr().is_terminal() {
        return None;
    }
...
fn create_desktop_shortcut(state: &mut SetupHintsState) -> Result<()> {
    #[cfg(any(test, target_os = "macos"))]
    {
        ...
        next_code_logging::info(&format!("Created macOS app bundle: {}", app_dir.display()));
    }
...
pub fn reinstall_launch_hotkeys_after_config_change() {
    #[cfg(target_os = "macos")]
    {
        ...
        match install_macos_hotkey_listener(preferred) {
            Ok(terminal) => next_code_logging::info(&format!(
                "Reinstalled launch hotkeys after config change for {}",
                terminal.label()
            )),
            Err(err) => next_code_logging::warn(&format!("failed to reinstall launch hotkeys: {err}")),
        }
    }
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        match install_linux_launch_hotkeys(comp) {
            Ok(true) => next_code_logging::info(&format!(
