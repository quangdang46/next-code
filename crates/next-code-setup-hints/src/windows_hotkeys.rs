...
//!
//! * [`chord_to_win32`] maps a next-code [`KeyChord`] onto `RegisterHotKey`
//!   modifier flags plus a virtual-key code. next-code's `cmd` modifier maps to
...
/// Returns `None` for keys with no stable VK code. next-code `cmd` maps to Alt
...
/// Translate a canonical next-code key token into a Win32 virtual-key code.
...
/// User-facing chord rendering for Windows notices: next-code's `cmd` modifier
...
fn ps_dir_expr(dir: &str, last_dir_file: &str, last_repo_file: &str) -> String {
    match dir {
        "$HOME" => "(Resolve-NextCodeDir $null)".to_string(),
        "$LAST_DIR" => format!("(Resolve-NextCodeDir {})", ps_quote(last_dir_file)),
        "$LAST_REPO" => format!("(Resolve-NextCodeDir {})", ps_quote(last_repo_file)),
        path => format!("(Resolve-NextCodeFixedDir {})", ps_quote(path)),
    }
}

/// Render the full hotkey-listener PowerShell script.
///
/// `launch_exe`/`launch_args` describe the terminal command that opens next-code
/// (e.g. `wt.exe` + profile args, or `alacritty -e next-code`); `{DIR}` never
...
pub(crate) fn render_windows_listener_ps1(
    ...
    for (index, entry) in entries.iter().enumerate() {
        ...
        registrations.push_str(&format!(
            "Register-NextCodeHotkey -Id 0x{id:X} -Mods 0x{mods:X} -Vk 0x{vk:X} -Label {label}\n",
    ...
    Some(format!(
        r#"# next-code global launch hotkey listener
# Auto-generated by next-code setup-hotkey from [launch_hotkeys] config. Runs at
# login via a startup shortcut. Do not edit; re-run `next-code setup-hotkey`.
...
function Register-NextCodeHotkey {{
    ...
    if ([HotKeyHelper]::RegisterHotKey([IntPtr]::Zero, $Id, $Mods -bor $MOD_NOREPEAT, $Vk)) {{
        ...
        Write-Warning "next-code: failed to register hotkey $Label (already claimed?)"
    }}
}}

# Resolve a dynamic launch dir from a tracking file, falling back to $HOME.
function Resolve-NextCodeDir {{
    param([string]$File)
    if ($File) {{
        try {{
...
function Resolve-NextCodeFixedDir {{
    param([string]$Dir)
    if ($Dir -and (Test-Path -LiteralPath $Dir -PathType Container)) {{ return $Dir }}
    return $env:USERPROFILE
}}
...
if ($script:RegisteredIds.Count -eq 0) {{
    Write-Error "next-code: no launch hotkeys could be registered"
    exit 1
}}
...
mod tests {
    ...
    fn listener_script_registers_each_entry_and_dispatches_dirs() {
        ...
        let script = render_windows_listener_ps1(
            ...
            "wt.exe",
            args_for,
            "C:\\Users\\u\\.next-code\\hotkey\\last_dir",
            "C:\\Users\\u\\.next-code\\hotkey\\last_repo",
        )
        .unwrap();

        // Three registrations with distinct ids.
        assert_eq!(script.matches("Register-NextCodeHotkey").count(), 3 + 1); // 3 calls + fn def
        ...
        assert!(script.contains("Resolve-NextCodeFixedDir 'C:\\Users\\u\\next-code'"));
        assert!(script.contains("Resolve-NextCodeDir $null"));
        assert!(script.contains("Resolve-NextCodeDir 'C:\\Users\\u\\.next-code\\hotkey\\last_repo'"));
