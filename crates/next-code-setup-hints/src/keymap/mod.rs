...
pub fn refresh_and_save() -> KeymapSnapshot {
    ...
    if let Ok(path) = snapshot_path() {
        let _ = next_code_storage::write_json(&path, &snapshot).map_err(|err| {
            next_code_logging::warn(&format!("keymap snapshot write failed: {err}"));
        });
    }
    if let Ok(path) = snapshot_path()
        ...
        && let Err(err) = next_code_storage::write_json_fast(&path, &snapshot)
    {
        next_code_logging::warn(&format!("keymap snapshot write failed: {err}"));
    }
    snapshot
