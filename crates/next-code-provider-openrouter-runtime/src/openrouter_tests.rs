...
use next_code_provider_openrouter::stream::OpenRouterStream;
...
impl SharedEnvLock {
    ...
    fn lock(&self) -> std::sync::MutexGuard<'static, ()> {
        next_code_base::storage::lock_test_env()
    }
}
...
impl EnvVarGuard {
    fn set(key: &'static str, value: impl AsRef<std::ffi::OsStr>) -> Self {
        ...
        next_code_base::env::set_var(key, value);
        Self { key, previous }
    }

    fn remove(key: &'static str) -> Self {
        let previous = std::env::var_os(key);
        next_code_base::env::remove_var(key);
        Self { key, previous }
    }
}
...
impl Drop for EnvVarGuard {
    fn drop(&mut self) {
        if let Some(previous) = &self.previous {
            next_code_base::env::set_var(self.key, previous);
        } else {
            next_code_base::env::remove_var(self.key);
        }
    }
}
...
fn named_openai_compatible_provider_exposes_static_models_as_routes() {
    ...
    let profile = next_code_base::config::NamedProviderConfig {
        ...
        models: vec![next_code_base::config::NamedProviderModelConfig {
...
fn named_openai_compatible_provider_uses_per_model_image_input_support() {
    ...
    let profile = next_code_base::config::NamedProviderConfig {
        ...
        models: vec![
            next_code_base::config::NamedProviderModelConfig {
...
fn minimax_profile_exposes_static_models_before_catalog_refresh() {
    let models = next_code_base::provider_catalog::openai_compatible_profile_static_models(
        next_code_provider_metadata::MINIMAX_PROFILE,
    );
...
fn cerebras_profile_exposes_live_chat_models_before_catalog_refresh() {
    assert_eq!(
        next_code_provider_metadata::CEREBRAS_PROFILE.default_model,
        Some("gpt-oss-120b")
    );

    let models = next_code_base::provider_catalog::openai_compatible_profile_static_models(
        next_code_provider_metadata::CEREBRAS_PROFILE,
    );
...
fn openai_compatible_profiles_with_unverified_live_catalogs_have_static_fallbacks() {
    let cases = [
        (next_code_provider_metadata::OPENCODE_PROFILE, "minimax-m2.7"),
        (next_code_provider_metadata::OPENCODE_GO_PROFILE, "kimi-k2.5"),
        (next_code_provider_metadata::ZAI_PROFILE, "glm-4.7"),
        (
            next_code_provider_metadata::AI302_PROFILE,
            "qwen3-235b-a22b-instruct-2507",
        ),
        (next_code_provider_metadata::BASETEN_PROFILE, "zai-org/GLM-4.7"),
        (next_code_provider_metadata::CORTECS_PROFILE, "kimi-k2.5"),
        (next_code_provider_metadata::KIMI_PROFILE, "kimi-for-coding"),
        (next_code_provider_metadata::FIRMWARE_PROFILE, "kimi-k2.5"),
        (
            next_code_provider_metadata::HUGGING_FACE_PROFILE,
            "Qwen/Qwen3-Coder-480B-A35B-Instruct",
        ),
        (next_code_provider_metadata::MOONSHOT_PROFILE, "kimi-k2.5"),
        (
            next_code_provider_metadata::NEBIUS_PROFILE,
            "openai/gpt-oss-120b",
        ),
        (
            next_code_provider_metadata::SCALEWAY_PROFILE,
            "qwen3-coder-30b-a3b-instruct",
        ),
        (
            next_code_provider_metadata::STACKIT_PROFILE,
            "openai/gpt-oss-120b",
        ),
        (next_code_provider_metadata::PERPLEXITY_PROFILE, "sonar"),
        (
            next_code_provider_metadata::DEEPINFRA_PROFILE,
            "moonshotai/Kimi-K2-Instruct",
        ),
        (
            next_code_provider_metadata::FIREWORKS_PROFILE,
            "accounts/fireworks/routers/kimi-k2p5-turbo",
        ),
        (next_code_provider_metadata::XIAOMI_MIMO_PROFILE, "mimo-v2.5"),
        (
            next_code_provider_metadata::ALIBABA_CODING_PLAN_PROFILE,
            "qwen3-coder-plus",
        ),
    ];

    for (profile, expected_model) in cases {
        let models = next_code_base::provider_catalog::openai_compatible_profile_static_models(profile);
        assert!(
...
fn autodetects_single_saved_openai_compatible_profile() {
    ...
    let opencode = next_code_base::provider_catalog::resolve_openai_compatible_profile(
...
fn autodetects_single_saved_local_openai_compatible_profile() {
    ...
    std::fs::write(
        ...
        format!(
            "{}=1\n",
            next_code_base::provider_catalog::OPENAI_COMPAT_LOCAL_ENABLED_ENV
        ),
    )
...
fn does_not_guess_when_multiple_saved_openai_compatible_profiles_exist() {
    ...
    let chutes = next_code_base::provider_catalog::resolve_openai_compatible_profile(
        next_code_base::provider_catalog::CHUTES_PROFILE,
    );
    write_test_api_key(
        &temp,
        &opencode.env_file,
...
fn autodetected_profile_seeds_default_model_and_cache_namespace() {
    ...
    let zai = next_code_base::provider_catalog::resolve_openai_compatible_profile(
        next_code_base::provider_catalog::ZAI_PROFILE,
    );
...
fn openai_compatible_model_catalog_refresh_calls_models_endpoint_and_updates_display() {
    ...
    assert!(
        request.to_ascii_lowercase().contains("user-agent: next-code/"),
...
fn runtime_display_name_for_profile_runtime_instance() {
    ...
    let nim = OpenRouterProvider::new_openai_compatible_profile_runtime(
        next_code_base::provider_catalog::NVIDIA_NIM_PROFILE,
    )
...
fn named_profile_construction_reads_openai_reasoning_effort_config() {
    ...
    let configured = next_code_base::config::config()
        .provider
...
fn user_named_profile_prefix_is_stripped_even_without_profile_id() {
    ...
    std::fs::create_dir_all(&next_code_home).expect("create test config dir");
    std::fs::write(
...
"#,
    ...
    next_code_base::config::invalidate_config_cache();
