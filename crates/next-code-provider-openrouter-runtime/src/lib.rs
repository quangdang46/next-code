...
use next_code_base::provider_catalog::{
...
use next_code_provider_openrouter::{
...
const OPENROUTER_TRANSPORT_STATE_ENV: &str = "NEXT_CODE_OPENROUTER_TRANSPORT_STATE";
...
-> Option<next_code_base::provider_catalog::ResolvedOpenAiCompatibleProfile> {
    ...
    let mut matches = openai_compatible_profiles()
        ...
        .filter_map(|profile| {
            ...
            if next_code_base::provider_catalog::openai_compatible_profile_is_configured(*profile) {
                Some(resolved)
            } else {
                None
            }
...
fn load_named_profile_api_key(
    env_key: &str,
    profile: &next_code_base::config::NamedProviderConfig,
) -> Option<String> {
...
fn model_catalog_enabled() -> bool {
    if let Ok(raw) = product_env("OPENROUTER_MODEL_CATALOG") {
        if let Some(value) = parse_env_bool(&raw) {
            return value;
        }
        next_code_base::logging::warn(&format!(
...
fn configured_auth_header_mode() -> AuthHeaderMode {
    let Some(raw) = product_env("OPENROUTER_AUTH_HEADER")
        .ok()
...
fn configured_auth_header_name() -> HeaderName {
    let raw = product_env("OPENROUTER_AUTH_HEADER_NAME")
        .ok()
        .map(|v| v.trim().to_string())
        .filter(|v| !v.is_empty())
...
impl OpenRouterTransportState {
    ...
    fn runtime_provider_is_direct_compatible(runtime_provider: Option<&str>) -> bool {
        matches!(runtime_provider, Some("openai-compatible" | "azure-openai"))
            || runtime_provider
                .and_then(next_code_base::provider_catalog::openai_compatible_profile_by_id)
                .is_some()
    }
...
impl ProviderAuth {
    async fn apply(&self, req: reqwest::RequestBuilder) -> Result<reqwest::RequestBuilder> {
        match self {
            ...
            Self::AzureEntra { .. } => {
                let token = next_code_base::auth::azure::get_bearer_token().await?;
...
async fn fetch_models_from_api(
    ...
    let models = parse_openai_compatible_models_response(&raw_body).with_context(|| {
            format!(
                ...
                url,
                auth.label(),
                next_code_base::util::truncate_str(&raw_body.trim().replace('\n', "\\n"), 1200)
            )
        })?;
...
pub fn maybe_schedule_openai_compatible_profile_catalog_refresh(
    ...
    let previous_fingerprint =
        next_code_provider_openrouter::load_disk_cache_entry_for_namespace(&profile_id)
    ...
    handle.spawn(async move {
        ...
        match fetch_models_from_api(
            next_code_provider_core::shared_http_client(),
            api_base,
            auth,
            models_cache,
        ...
        .await
        {
            Ok(models) => {
                ...
                if updated {
                    ...
                    next_code_base::bus::Bus::global().publish_models_updated();
                } else {
                    next_code_base::logging::info(&format!(
            ...
            Err(error) => next_code_base::logging::info(&format!(
...
pub fn maybe_schedule_standard_openrouter_catalog_refresh(context: &'static str) -> bool {
    ...
    let cache_is_fresh = current_unix_secs()
        .zip(next_code_provider_openrouter::load_disk_cache_entry_for_namespace(namespace))
        .map(|(now, cache)| {
    ...
    let previous_fingerprint =
        next_code_provider_openrouter::load_disk_cache_entry_for_namespace(namespace)
            .map(|cache| models_fingerprint(&cache.models))
            .unwrap_or_default();
    handle.spawn(async move {
        let models_cache = Arc::new(RwLock::new(ModelsCache::default()));
...
impl OpenRouterProvider {
    ...
    fn configured_max_tokens(profile_id: Option<&str>) -> Option<u32> {
        if let Ok(raw) = product_env("OPENROUTER_MAX_TOKENS") {
    ...
    fn resolve_extra_body(
        ...
        if let Some(value) = config {
            match value.as_object() {
                ...
                None => next_code_base::logging::warn(
                    "Ignoring provider `extra_body`: expected a table/object of top-level request fields",
                ),
            }
        }

        if let Some(raw) = load_env_value_from_env_or_config("NEXT_CODE_OPENAI_EXTRA_BODY", env_file) {
    ...
    ///
    /// Unlike the env-var based [`next_code_base::provider_catalog::runtime_provider_display_name`],
    ...
    pub fn runtime_display_name(&self) -> String {
        ...
        if !self.supports_provider_features {
            if let Some(profile_id) =
                next_code_base::provider_catalog::openai_compatible_profile_id_for_api_base(
                    &self.api_base,
                )
            ...
            if product_env("RUNTIME_PROVIDER")
                .ok()
    ...
    pub fn new_named_openai_compatible(
        ...
        next_code_base::env::set_var("NEXT_CODE_OPENROUTER_CACHE_NAMESPACE", profile_name);
        ...
        let auth = match profile.auth {
            next_code_base::config::NamedProviderAuth::None => ProviderAuth::None {
                label: "local endpoint (no auth)".to_string(),
            },
            next_code_base::config::NamedProviderAuth::Bearer => ProviderAuth::AuthorizationBearer {
                token: key
                    .ok_or_else(|| anyhow::anyhow!("{} not found in environment", key_label))?,
                label: key_label,
            },
            next_code_base::config::NamedProviderAuth::Header => ProviderAuth::HeaderValue {
        ...
        Ok(Self {
            ...
            supports_provider_features: matches!(
                ...
                next_code_base::config::NamedProviderType::OpenRouter
    ...
    fn is_kimi_model(model: &str) -> bool {
        next_code_provider_openrouter::is_kimi_model(model)
    }
    ...
    fn thinking_override() -> Option<bool> {
        let raw = product_env("OPENROUTER_THINKING").ok()?;
    ...
    pub fn new() -> Result<Self> {
        ...
        let profile_id = product_env("OPENROUTER_CACHE_NAMESPACE")
            .ok()
            .map(|value| value.trim().to_ascii_lowercase())
            .filter(|value| !value.is_empty())
        ...
        let static_context_limits = profile_id
            ...
            .unwrap_or_default();
        let static_models = product_env("OPENROUTER_STATIC_MODELS")
            .ok()
            .map(|raw| {
                raw.lines()
                    .map(str::trim)
        ...
        if product_env_os("OPENROUTER_CACHE_NAMESPACE").is_none()
            && let Some(profile) = autodetected_profile.as_ref()
        {
            next_code_base::env::set_var("NEXT_CODE_OPENROUTER_CACHE_NAMESPACE", &profile.id);
        }

        let model = product_env("OPENROUTER_MODEL")
            .ok()
    ...
    pub fn new_openrouter_api_key_runtime() -> Result<Self> {
        let api_key = load_api_key_from_env_or_config(DEFAULT_API_KEY_NAME, DEFAULT_ENV_FILE)
            .ok_or_else(|| {
                let path = next_code_base::storage::app_config_dir()
    ...
    pub(crate) fn filter_profile_chat_supported_models(&self, models: Vec<String>) -> Vec<String> {
        ...
        models
            .into_iter()
            .filter(|model| {
                next_code_base::provider_catalog::openai_compatible_profile_model_supports_chat(
                    profile_id, model,
                )
            })
            .collect()
    ...
    fn model_disk_cache_source_matches(
        &self,
        cache_entry: &next_code_provider_openrouter::DiskCache,
    ) -> bool {
    ...
    pub(crate) fn load_usable_model_disk_cache_entry(
        &self,
    ) -> Option<next_code_provider_openrouter::DiskCache> {
    ...
    fn maybe_schedule_model_catalog_refresh(&self, cache_age_secs: u64, context: &'static str) {
        ...
        handle.spawn(async move {
            match fetch_models_from_api(client, api_base, auth, models_cache, None).await {
                ...
                Err(e) => next_code_base::logging::info(&format!(
    ...
    fn parse_provider_routing() -> ProviderRouting {
        next_code_provider_openrouter::parse_provider_routing_from_env()
    }
    ...
    fn rank_providers_from_endpoints(endpoints: &[EndpointInfo]) -> Vec<String> {
        next_code_provider_openrouter::rank_providers_from_endpoints(endpoints)
    }
    ...
    pub fn has_credentials() -> bool {
        if matches!(
            ...
            return next_code_base::auth::azure::has_configuration();
        }
        if configured_allow_no_auth() {
            return true;
        }
