...
use super::*;
use next_code_base::provider::{ModelCatalogRefreshSummary, summarize_model_catalog_refresh};
...
impl Provider for OpenRouterProvider {
    ...
    async fn complete(
        ...
        let api_messages = next_code_provider_openrouter::request::build_chat_messages(
            &effective_messages,
            system,
            allow_reasoning,
            ...
            .map(|t| {
                serde_json::json!({
                    ...
                    "function": {
                        ...
                        "parameters": next_code_provider_openrouter::request::sanitize_tool_parameters_schema(&t.input_schema),
                    }
                })
            })
            .collect();
        ...
        if let Some(effort) = reasoning_effort.as_deref() {
            if self.supports_deepseek_reasoning_effort() {
                ...
                let effort = if next_code_base::prompt::is_swarm_effort(effort) {
                    "max"
                } else {
                    effort
                };
        ...
        // next-code-generated field with the same key (issue #341).
        ...
        next_code_base::logging::info("OpenRouter transport: HTTPS (SSE)");
    ...
    fn set_model(&self, model: &str) -> Result<()> {
        ...
        let (model_id, provider) = if self.supports_provider_features {
            ...
            let model_id = if provider.is_some() {
                next_code_base::provider::openrouter_catalog_model_id(&model_id).unwrap_or(model_id)
            } else {
                model_id
            };
            (model_id, provider)
    ...
    fn model_routes(&self) -> Vec<next_code_provider_core::ModelRoute> {
        ...
        self.available_models_display()
            .into_iter()
            .filter(|model| next_code_base::provider::is_listable_model_name(model))
            .map(|model| {
                ...
                next_code_provider_core::ModelRoute {
                    model,
    ...
    fn context_window(&self) -> usize {
        ...
        if let Some(profile_id) = self.profile_id.as_deref()
            && let Some(limit) =
                next_code_base::provider_catalog::openai_compatible_profile_context_limit(
                    profile_id, &model_id,
                )
        {
            return limit;
        }
        next_code_provider_core::context_limit_for_model_with_provider(&model_id, Some(self.name()))
