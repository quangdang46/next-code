...
use next_code_tui_mermaid as mermaid;
...
use next_code_tui_workspace::color_support::rgb;
...
fn math_inline_span(math: &str) -> Span<'static> {
    Span::styled(
        next_code_render_core::render_inline_latex(math),
...
fn math_display_lines(math: &str) -> Vec<Line<'static>> {
    ...
    for line in next_code_render_core::render_display_latex(math) {
        out.push(
            Line::from(vec![
                Span::styled("│ ", dim),
