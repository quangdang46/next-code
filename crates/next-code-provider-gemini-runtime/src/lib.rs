...
use chrono::Utc;
use next_code_base::auth::gemini as gemini_auth;
...
pub use next_code_provider_gemini::{
...
impl GeminiProvider {
    fn persisted_catalog_path() -> Result<std::path::PathBuf> {
        Ok(next_code_base::storage::app_config_dir()?.join("gemini_models_cache.json"))
    }
                ...
                if !produced_output {
                    ...
                    if abnormal {
                        ...
                        let detail = candidate
                            ...
                            .filter(|msg| !msg.trim().is_empty())
                            .map(|msg| {
                                format!(": {}", next_code_base::util::truncate_str(msg.trim(), 300))
                            })
                            .unwrap_or_default();
                        let _ = tx
                            .send(Err(anyhow::anyhow!(
...
fn gemini_http_client() -> reqwest::Client {
    reqwest::Client::builder()
        .user_agent("next-code/1.0 (gemini)")
        .http1_only()
        ...
        .build()
        .unwrap_or_else(|_| next_code_provider_core::shared_http_client())
}
...
fn is_transient_gemini_transport_error(err: &reqwest::Error) -> bool {
    ...
    err.is_connect()
        || err.is_timeout()
        || next_code_provider_core::is_transient_transport_error(&err.to_string())
}
