...
pub struct BeadsProject {
    ...
    /// Storage wrapped in RefCell so flush() can borrow mutably from &self.
    storage: RefCell<SqliteStorage>,
}

impl BeadsProject {
    /// Discover and open an existing beads project under `working_dir`.
    ...
    /// Try to open a beads project; if none exists, initialize one.
    ...
    /// Initialize a new beads project under `working_dir/.beads/`.
    ...
    /// Flush dirty issues to JSONL (no git operations).
