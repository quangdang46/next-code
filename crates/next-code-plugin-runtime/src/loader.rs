...
use next_code_plugin_core::config::{
...
impl PluginLoader {
    ...
    pub(crate) async fn load_one(
        ...
        let manifest_caps = next_code_plugin_core::manifest::PluginCapabilities::default();
    ...
    async fn resolve_npm_entry(
        ...
        let manifest = next_code_plugin_core::manifest::PluginManifest::from_package_json(&json)?;
        let entry = manifest
            .entry
            .server
            .as_ref()
...
mod tests {
    ...
    use next_code_plugin_core::config::DiscoveryPaths;
