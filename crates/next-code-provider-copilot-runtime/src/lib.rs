...
use chrono::Utc;
use next_code_base::auth::copilot as copilot_auth;
use next_code_message_types::{
    ContentBlock, Message as ChatMessage, Role, StreamEvent, ToolDefinition,
};
#[cfg(test)]
use next_code_provider_copilot::max_token_parameter_for_model as copilot_max_token_parameter_for_model;
use next_code_provider_copilot::{
...
use next_code_provider_copilot::{DEFAULT_MODEL, FALLBACK_MODELS};
pub use next_code_provider_core::PremiumMode;
...
impl CopilotApiProvider {
    ...
    fn persisted_catalog_path() -> Result<std::path::PathBuf> {
        Ok(next_code_base::storage::app_config_dir()?.join("copilot_models_cache.json"))
    }
    ...
    fn get_or_create_machine_id() -> String {
        ...
        let path = home.join(".next-code").join("machine_id");
    ...
    pub fn set_premium_mode(&self, mode: PremiumMode) {
        ...
        if mode != PremiumMode::Normal {
            next_code_base::logging::info(&format!("Copilot premium mode set to {:?}", mode));
        }
    }
    ...
    async fn stream_request(
        ...
        for attempt in 0..MAX_RETRIES {
            if attempt > 0 {
                let delay = next_code_provider_core::attempt_tracker::retry_backoff_delay(
                    attempt,
                    RETRY_BASE_DELAY_MS,
                );
                next_code_base::logging::info(&format!(
            ...
            if Self::is_auth_error(status) && !attempted_auth_refresh {
                ...
                *self.bearer_token.write().await = None;
                next_code_base::logging::info("Copilot bearer token expired, refreshing...");
                last_error = Some(anyhow::anyhow!("Copilot auth error (HTTP {})", status));
                continue;
            }

            if !status.is_success() {
                let body_text = next_code_base::util::http_error_body(resp, "HTTP error").await;
                let error_str =
    ...
    async fn process_sse_stream(
        ...
        loop {
            ...
            while let Some(line_end) = buffer.find('\n') {
                ...
                if let Some(data) = next_code_base::util::sse_data_line(&line) {
                    if data.trim() == "[DONE]" {
                        ...
                        next_code_base::copilot_usage::record_request(
                            input_tokens,
                            output_tokens,
                            true,
                        );
...
impl Provider for CopilotApiProvider {
    ...
    fn context_window(&self) -> usize {
        next_code_provider_core::context_limit_for_model_with_provider(&self.model(), Some(self.name()))
            .unwrap_or(128_000)
    }
