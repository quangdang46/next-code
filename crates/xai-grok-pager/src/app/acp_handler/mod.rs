...
fn handle_next_code_provider_name(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    let Ok(parsed) = serde_json::from_str::<serde_json::Value>(notif.params.get()) else {
        tracing::warn!("Failed to parse next-code/provider_name");
        return false;
    };
    let Some(session_id) = parsed.get("sessionId").and_then(|v| v.as_str()) else {
        return false;
    ...
    match matched {
        SessionMatch::Root(_) => {
            agent.info_float_provider = Some(provider.to_string());
        }
        SessionMatch::Child(_) => {
            let child_key: &str = sid.0.as_ref();
            if let Some(child) = agent.subagent_views.get_mut(child_key) {
                child.info_float_provider = Some(provider.to_string());
            }
        }
    }
    is_active
...
/// next-code daemon TokenUsage bridge — updates Face context_state + KV float data.
fn handle_next_code_token_usage(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    let Ok(parsed) = serde_json::from_str::<serde_json::Value>(notif.params.get()) else {
        tracing::warn!("Failed to parse next-code/token_usage");
        return false;
    };
    let Some(session_id) = parsed.get("sessionId").and_then(|v| v.as_str()) else {
        return false;
    };
    let input = parsed.get("input").and_then(|v| v.as_u64()).unwrap_or(0);
    let output = parsed.get("output").and_then(|v| v.as_u64()).unwrap_or(0);
    let cache_read = parsed.get("cacheReadInput").and_then(|v| v.as_u64());
    ...
    match matched {
        SessionMatch::Root(_) => {
            agent.apply_token_usage_sample(input, output, cache_read, cache_creation);
        }
        SessionMatch::Child(_) => {
            let child_key: &str = sid.0.as_ref();
            if let Some(child) = agent.subagent_views.get_mut(child_key) {
                child.apply_token_usage_sample(input, output, cache_read, cache_creation);
            }
        }
    }
    is_active
