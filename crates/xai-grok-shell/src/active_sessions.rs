//! Tracks open Face sessions in `<grok_home>/active_sessions.json` for crash
...
//!
//! Copied from upstream `xai-org/grok-build` `xai-grok-shell::active_sessions`
//! (pure FS helper). Home root is next-code's `grok_home()` (`~/.next-code`).
...
pub fn list_in(root: &Path) -> io::Result<Vec<ActiveSession>> {
    read_data_file(&root.join(DATA_FILENAME))
}
