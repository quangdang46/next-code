...
fn catalog_error_is_auth_rejection(err: &anyhow::Error) -> bool {
    err.downcast_ref::<next_code_base::provider::ModelCatalogHttpStatus>()
...
impl Provider for OpenAIProvider {
    ...
    fn set_credential_mode(&self, mode: next_code_provider_core::CredentialMode) -> anyhow::Result<()> {
    ...
    async fn complete(
        ...
        let prompt_cache_key_hash = request
            ...
            .map(next_code_provider_core::fingerprint::stable_hash_json);
        ...
        let usage_snapshot = next_code_base::usage::get_openai_usage_sync();
        log_openai_stream_lifecycle(
            next_code_base::logging::LogLevel::Info,
            "request_start",
            vec![
        ...
        tokio::spawn(async move {
                ...
                if use_websocket_transport {
                    ...
                    match continuation_result {
                        ...
                        PersistentWsResult::Failed(err) => {
                            log_openai_stream_lifecycle(
                                next_code_base::logging::LogLevel::Warn,
                ...
                for attempt in 0..MAX_RETRIES {
                    if attempt > 0 {
                        emit_connection_phase(
                            &tx,
                            next_code_message_types::ConnectionPhase::Retrying {
                                attempt: attempt + 1,
                                max: MAX_RETRIES,
                            },
                        )
                    ...
                    match result {
                        ...
                        Err(OpenAIStreamFailure::Other(error)) => {
                            ...
                            if is_retryable_error(&error_str) && attempt + 1 < MAX_RETRIES {
                                ...
                                next_retry_delay =
                                    next_code_provider_core::retry_after::retry_after_from_error(
                                        &error,
                                    );
                                last_error = Some(error);
                                continue;
                            ...
                            log_openai_stream_lifecycle(
                                next_code_base::logging::LogLevel::Error,
                                "attempt_failed",
                                vec![
                                    ("model", model_for_transport.clone()),
                                    ("attempt", (attempt + 1).to_string()),
            ...
            if let Err(panic_payload) = result {
                ...
                next_code_base::logging::error(&format!(
    ...
    fn set_model(&self, model: &str) -> Result<()> {
        ...
        let availability = next_code_base::provider::model_availability_for_account(model);
        if availability.state == next_code_base::provider::AccountModelAvailabilityState::Unavailable {
            let detail =
                next_code_base::provider::format_account_model_availability_detail(&availability)
        ...
        if let Ok(mut current) = self.model.try_write() {
            ...
            *current = model.to_string();
            next_code_base::provider::clear_model_unavailable_for_account(model);
            drop(current);
            if changed {
    ...
    fn available_models(&self) -> Vec<&'static str> {
        next_code_provider_core::ALL_OPENAI_MODELS.to_vec()
    }

    fn available_models_for_switching(&self) -> Vec<String> {
        next_code_base::provider::cached_openai_model_ids().unwrap_or_else(|| vec![self.model()])
    }
    ...
    async fn prefetch_models(&self) -> Result<()> {
        ...
        let catalog = if is_chatgpt_mode {
            ...
            match next_code_base::provider::fetch_openai_model_catalog(&access_token).await {
                Ok(catalog) => catalog,
                ...
                Err(err) if catalog_error_is_auth_rejection(&err) => {
                    ...
                    next_code_base::provider::fetch_openai_model_catalog(&refreshed).await?
                }
                Err(err) => return Err(err),
            }
        } else {
            next_code_base::provider::fetch_openai_api_key_model_catalog(&access_token).await?
        };
        next_code_base::provider::persist_openai_model_catalog(&catalog);
        ...
        if !catalog.available_models.is_empty() {
            next_code_base::provider::populate_account_models(catalog.available_models);
        }
        Ok(())
    }
    ...
    async fn native_compact(
    ...
    ) -> Result<next_code_provider_core::NativeCompactionResult> {
        ...
        if let Some(encrypted_content) = existing_openai_encrypted_content {
            if !next_code_base::provider::openai_request::openai_encrypted_content_is_sendable(
                encrypted_content,
            ) {
                anyhow::bail!(
                    "OpenAI native compaction payload is too large to replay ({} chars > safe limit {} chars)",
                    encrypted_content.len(),
                    next_code_base::provider::openai_request::OPENAI_ENCRYPTED_CONTENT_SAFE_MAX_CHARS,
                );
            }
        ...
        Ok(next_code_provider_core::NativeCompactionResult {
            summary_text: None,
    ...
    fn context_window(&self) -> usize {
        ...
        next_code_provider_core::context_limit_for_model_with_provider(&model, Some(self.name()))
            .unwrap_or(next_code_provider_core::DEFAULT_CONTEXT_LIMIT)
    }
