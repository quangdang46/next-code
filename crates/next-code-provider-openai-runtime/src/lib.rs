...
//!
//! Model-catalog/account-availability state stays in `next_code_base::provider`
...
use next_code_base::auth::codex::CodexCredentials;
use next_code_base::auth::oauth;
use next_code_base::provider::openai_request::{build_responses_input, build_tools};
#[cfg(test)]
use next_code_message_types::TOOL_OUTPUT_MISSING_TEXT;
use next_code_message_types::{Message as ChatMessage, StreamEvent, ToolDefinition};
...
const DEFAULT_MODEL: &str = next_code_provider_core::DEFAULT_OPENAI_MODEL;
...
/// Shared dual-auth credential pin (see `next_code_provider_core::CredentialMode`).
/// The OpenAI-specific alias is kept so existing call sites read naturally.
pub(crate) use next_code_provider_core::CredentialMode as OpenAICredentialMode;
...
pub(crate) fn load_credentials_for_mode(mode: OpenAICredentialMode) -> Result<CodexCredentials> {
    match mode {
        OpenAICredentialMode::Auto => next_code_base::auth::codex::load_credentials(),
        OpenAICredentialMode::OAuth => next_code_base::auth::codex::load_oauth_credentials(),
        OpenAICredentialMode::ApiKey => next_code_base::auth::codex::load_api_key_credentials(),
    }
}
...
fn log_openai_stream_lifecycle(
    level: next_code_base::logging::LogLevel,
    phase: &str,
    ...
    next_code_base::logging::event(level, "PROVIDER_STREAM_LIFECYCLE", owned);
}
...
async fn emit_connection_phase(
    ...
    phase: next_code_message_types::ConnectionPhase,
) {
...
impl OpenAIProvider {
    pub(crate) fn supports_extended_prompt_cache_retention(model_id: &str) -> bool {
        next_code_base::provider::openai::supports_extended_prompt_cache_retention(model_id)
    }
    ...
    pub fn new(credentials: CodexCredentials) -> Self {
        ...
        let native_compaction_threshold_tokens = next_code_base::config::config()
            .provider
    ...
    async fn clear_persistent_ws(&self, reason: &str) {
        ...
        if persistent_ws.is_some() {
            next_code_base::logging::info(&format!("Clearing persistent OpenAI WS state: {}", reason));
        }
        *persistent_ws = None;
    }
    ...
    fn api_reasoning_effort(effort: Option<&str>) -> Option<String> {
        match effort {
            Some(e) if next_code_base::prompt::is_swarm_effort(e) => Some("xhigh".to_string()),
    ...
    fn load_max_output_tokens() -> Option<u32> {
        ...
        if raw.is_some() {
            match parsed {
                Some(value) => next_code_base::logging::info(&format!(
    ...
    pub(crate) fn resolve_api_base() -> String {
        next_code_base::provider::openai::resolve_api_base()
    }
    ...
    async fn model_id(&self) -> String {
        ...
        let availability = next_code_base::provider::model_availability_for_account(&current);

        match availability.state {
            next_code_base::provider::AccountModelAvailabilityState::Unavailable => {
            ...
            next_code_base::provider::AccountModelAvailabilityState::Unknown => {
                if next_code_base::provider::should_refresh_openai_model_catalog()
                    && next_code_base::provider::begin_openai_model_catalog_refresh()
                {
                    let is_chatgpt_mode = {
                    ...
                    next_code_base::provider::refresh_openai_model_catalog_in_background(
                        token,
                        is_chatgpt_mode,
            ...
            next_code_base::provider::AccountModelAvailabilityState::Available => {}
        }
