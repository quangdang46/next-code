...
impl LiveOpenAITestEnv {
    fn new() -> Result<Option<Self>> {
        let lock = next_code_base::storage::lock_test_env();
...
async fn live_openai_catalog() -> Result<Option<next_code_base::provider::OpenAIModelCatalog>> {
    let Some(_env) = LiveOpenAITestEnv::new()? else {
        return Ok(None);
    };
    let creds = next_code_base::auth::codex::load_credentials()?;
    ...
    Ok(Some(
        next_code_base::provider::fetch_openai_model_catalog(&token).await?,
    ))
}
