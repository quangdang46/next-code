...
fn test_openai_switching_models_include_dynamic_catalog_entries() {
    ...
    next_code_base::auth::codex::set_active_account_override(Some("switching-test".to_string()));
    next_code_base::provider::populate_account_models(vec![
        "gpt-5.4".to_string(),
...
async fn test_set_model_clears_persistent_ws_state() {
    let _guard = next_code_base::storage::lock_test_env();
    next_code_base::auth::codex::set_active_account_override(Some("openai-set-model-clears-ws".to_string()));
    next_code_base::provider::populate_account_models(vec!["gpt-5.3-codex".to_string()]);
