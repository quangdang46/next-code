...
fn test_build_responses_input_replaces_oversized_native_compaction_with_text() {
    let oversized =
        "x".repeat(next_code_base::provider::openai_request::OPENAI_ENCRYPTED_CONTENT_SAFE_MAX_CHARS + 1);
...
fn test_openai_cache_ttl_is_model_aware() {
    assert_eq!(
        next_code_base::provider::cache_ttl_for_provider_model("openai", Some("gpt-5.5")),
        Some(24 * 60 * 60)
    );
    assert_eq!(
        next_code_base::provider::cache_ttl_for_provider_model("openai", Some("gpt-4o")),
        Some(300)
    );
}
