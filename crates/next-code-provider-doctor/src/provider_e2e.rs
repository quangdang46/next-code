...
use next_code_base::protocol::{AuthChanged, CatalogNamespace, RuntimeProviderKey};
...
fn tool_stage_detail(stage: &next_code_base::live_tests::LiveVerificationStage) -> String {
...
/// doctor report.
fn reasoning_stage_detail(stage: &next_code_base::live_tests::LiveVerificationStage) -> String {
    match stage
        .evidence
...
///
/// The predicate itself lives in `next_code_base::auth::doctor` so base-internal
...
pub use next_code_base::auth::doctor::native_doctor_supports_provider;
...
pub async fn run_claude_native_e2e(
    ...
    use next_code_base::provider::Provider;
    use next_code_provider_anthropic_runtime::AnthropicProvider;

    let normalized = next_code_base::auth::lifecycle::normalized_auth_provider_id(Some(provider_id))
        .unwrap_or("claude");
    let provider_label = next_code_base::auth::lifecycle::provider_display_label(Some(normalized))
    ...
    let catalog_models: Vec<String> = if tier.requires_api_key() {
        match provider_runtime.fetch_live_model_ids_for_doctor().await {
            ...
            Ok(_) => {
                ...
                let fallback = next_code_base::provider::known_anthropic_model_ids();
                checks.push(DoctorCheck::passed(
                    checkpoints::MODEL_CATALOG_LIVE_ENDPOINT,
                    label_for(checkpoints::MODEL_CATALOG_LIVE_ENDPOINT),
                    format!(
...
pub async fn run_antigravity_native_e2e(
    ...
    use next_code_base::provider::Provider;
    use next_code_provider_antigravity_runtime::AntigravityProvider;
    ...
    let _ = next_code_base::auth::lifecycle::normalized_auth_provider_id(Some(provider_id));
    let provider_label = next_code_base::auth::lifecycle::provider_display_label(Some("antigravity"))
...
impl NativeProviderKind {
    ...
    fn build_runtime(self) -> anyhow::Result<std::sync::Arc<dyn next_code_base::provider::Provider>> {
        use anyhow::Context as _;
        use next_code_base::provider::Provider;
        let runtime: std::sync::Arc<dyn Provider> = match self {
            Self::OpenAi => {
                let credentials =
                    next_code_base::auth::codex::load_credentials().unwrap_or_else(|_| {
                        next_code_base::auth::codex::CodexCredentials {
                ...
                std::sync::Arc::new(next_code_provider_openai_runtime::OpenAIProvider::new(
                    credentials,
                ))
            }
            Self::Gemini => {
                std::sync::Arc::new(next_code_provider_gemini_runtime::GeminiProvider::new())
            }
            Self::Cursor => {
                std::sync::Arc::new(next_code_provider_cursor_runtime::CursorCliProvider::new())
            }
            Self::Copilot => {
            ...
            Self::Azure => {
                ...
                next_code_base::auth::azure::apply_runtime_env()
                    .context("apply Azure OpenAI runtime env")?;
                let runtime = next_code_provider_openrouter_runtime::OpenRouterProvider::new()
                ...
                if let Some(model) = next_code_base::auth::azure::load_model() {
    ...
    async fn resolve_credential(self) -> anyhow::Result<String> {
        ...
        match self {
            ...
            Self::Gemini => {
                let tokens = next_code_base::auth::gemini::load_or_refresh_tokens()
                    .await
            ...
            Self::Bedrock => {
                if !next_code_base::provider::bedrock::BedrockProvider::has_credentials() {
                    anyhow::bail!(
            ...
            Self::Azure => {
                ...
                Ok(format!(
                    ...
                    next_code_base::auth::azure::method_detail()
                ))
            }
        }
    }
...
async fn run_generic_native_api_checks(
    provider: &dyn next_code_base::provider::Provider,
    selected: &str,
    label: &str,
    checks: &mut Vec<DoctorCheck>,
    spend: &mut DoctorSpend,
...
fn wiring_contract(profile: OpenAiCompatibleProfile) -> WiringContract {
    match next_code_base::auth::lifecycle::normalized_auth_provider_id(Some(profile.id)) {
...
fn run_wiring_checks_for_contract(
    ...
    let auth = AuthChanged {
        provider: next_code_base::protocol::AuthProviderId::new(auth_provider_id),
...
mod tests {
    ...
    /// `native_doctor_supports_provider` lives in `next_code_base::auth::doctor`
