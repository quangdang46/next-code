...
use next_code_base::message::{ContentBlock, Message, Role, StreamEvent, ToolDefinition};
use next_code_base::provider::Provider;
use next_code_base::provider_catalog::{OpenAiCompatibleProfile, ResolvedOpenAiCompatibleProfile};
...
pub async fn fetch_live_openai_compatible_models(
    ...
    let request = next_code_base::provider::shared_http_client().get(&url);
...
pub async fn run_live_openai_compatible_smoke(
    ...
    let request = next_code_base::provider::shared_http_client()
        .post(&url)
        .json(&body);
    let request = apply_provider_auth(request, &resolved, api_key);
    ...
    let mut stage = next_code_base::live_tests::LiveVerificationStage::passed(
...
mod tests {
    use super::*;
    use next_code_base::provider_catalog::resolve_openai_compatible_profile;
...
pub async fn run_live_openai_compatible_tool_smoke(
    ...
    let tool_calls = parsed
        ...
        resolved.display_name,
        next_code_base::util::truncate_str(text.trim(), 1200)
    );
    ...
    let parsed_arguments = next_code_base::message::ToolCall::parse_streamed_input_to_object(arguments);
    ensure!(
...
pub async fn run_live_claude_native_smoke(
    ...
    .await?;
        ...
        next_code_base::util::truncate_str(outcome.text.trim(), 200),
        outcome.diagnostics()
    );
...
pub async fn run_live_claude_native_tool_smoke(
    ...
    .await?;
        ...
        first.stop_reason,
        next_code_base::util::truncate_str(first.text.trim(), 200)
    );
    ...
    let parsed_arguments = next_code_base::message::ToolCall::parse_streamed_input_to_object(
    ...
    .await?;
        ...
        next_code_base::util::truncate_str(second.text.trim(), 200)
    );
...
pub async fn run_live_native_provider_reasoning_smoke(
    ...
    let mut stage = next_code_base::live_tests::LiveVerificationStage::passed(
        next_code_base::live_tests::checkpoints::REASONING_CAPABILITY,
    )
    .with_duration_ms(started.elapsed().as_millis() as u64)
    .with_evidence("model", serde_json::json!(model))
...
fn parse_tool_arguments(input_json: &str) -> serde_json::Value {
    next_code_base::message::ToolCall::parse_streamed_input_to_object(if input_json.trim().is_empty() {
        "{}"
    } else {
        input_json.trim()
    })
