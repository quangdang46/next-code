    ...
    CLIENT
        .get_or_init(|| {
            reqwest::Client::builder()
                .user_agent(NEXT_CODE_USER_AGENT)
                ...
                .build()
                .unwrap_or_else(|err| {
                    eprintln!("next-code: failed to build shared provider HTTP client: {err}");
                        ...
                        .build()
                    {
                        ...
                        Err(fallback_err) => {
                            eprintln!(
                                "next-code: failed to build fallback provider HTTP client: {fallback_err}"
                            );
                            reqwest::Client::new()
                        }
                    }
...
/// through. Mirrors `next_code_base::provider::openrouter_catalog_model_id` but
...
mod tests {
    ...
    #[test]
    fn canonical_user_agent_identifies_next_code() {
        assert!(NEXT_CODE_USER_AGENT.starts_with("next-code/"));
    }
