...
use next_code_base::message::ContentBlock;
use next_code_base::session::Session;
use next_code_import_core::imported_claude_code_session_id;
...
impl Drop for EnvVarGuard {
    fn drop(&mut self) {
        if let Some(prev) = self.prev.take() {
            next_code_base::env::set_var(self.key, prev);
        } else {
...
fn import_array_tool_result_with_image_uses_placeholder_not_base64() {
    ...
    let saved = std::fs::read_to_string(next_code_session_file(temp.path(), "img-session")).unwrap();
    assert!(
        !saved.contains(&big),
