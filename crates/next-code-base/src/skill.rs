...
impl SkillRegistry {
    ...
    fn import_from_external() {
        ...
        {
            let count = Self::copy_skills_dir(&claude_skills, &next_code_skills);
            if count > 0 {
                sources.push(format!("{} from Claude Code", count));
                copied.extend(Self::list_skill_names(&next_code_skills));
            }
        }
        ...
        if let Ok(codex_skills) = crate::storage::user_home_path(".codex/skills")
            && codex_skills.is_dir()
        {
            let count = Self::copy_skills_dir(&codex_skills, &next_code_skills);
            if count > 0 {
    ...
    pub fn load_global() -> Result<Self> {
        ...
        if let Ok(next_code_dir) = crate::storage::next_code_dir() {
            ...
            if next_code_skills.exists() {
                registry.load_from_dir(&next_code_skills)?;
            }
        }
    ...
    pub fn reload_global(&mut self) -> Result<usize> {
        ...
        if let Ok(next_code_dir) = crate::storage::next_code_dir() {
            let next_code_skills = next_code_dir.join("skills");
            if next_code_skills.exists() {
                count += self.load_from_dir_count(&next_code_skills)?;
            }
        }
