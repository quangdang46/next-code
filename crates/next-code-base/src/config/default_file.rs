...
# minimal keeps core coding tools only: bash, read, write, edit, ffs_grep, ffs_glob, ls.
profile = "full"
...
# endpoint = ""
	"#;
        ...
        let p = next_code_config_types::KeybindingPlatform::current();
        let effort_increase =
            next_code_config_types::default_binding("effort_increase", p).unwrap_or("alt+right");
        let effort_decrease =
            next_code_config_types::default_binding("effort_decrease", p).unwrap_or("alt+left");
