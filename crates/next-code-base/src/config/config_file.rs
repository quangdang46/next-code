use super::*;
use crate::storage::next_code_dir;
use std::path::PathBuf;

impl Config {
    /// Get the config file path
    pub fn path() -> Option<PathBuf> {
        next_code_dir().ok().map(|d| d.join("config.toml"))
    }
    ...
    ///
    /// Scans `~/.next-code/sessions` for the directories the user works in most,
    ...
    pub fn bake_launch_hotkeys_once() -> bool {
        use next_code_import_core::repo_ranking;

        let cfg = Self::load();
        if cfg.launch_hotkeys.imported {
            return false;
        }
        let Ok(next_code_dir) = next_code_dir() else {
            return false;
        };
        let sessions_dir = next_code_dir.join("sessions");
        ...
        let entries: Vec<next_code_config_types::LaunchHotkeyEntry> = plan
            .into_iter()
            .map(|p| next_code_config_types::LaunchHotkeyEntry {
                chord: p.chord,
    ...
    pub fn external_auth_source_allowed_for_path_cached(
        ...
        // a new path-bound trust decision, or before tests switch NEXT_CODE_HOME. Fall
