...
const CLAUDE_CODE_NEXT_CODE_NOTICE: &str = "You are next_code, powered by Claude Code. You are a third-party CLI, not the official Claude Code CLI.";
...
fn anthropic_sidecar_prefers_api_key() -> bool {
    match next_code_provider_core::runtime_env_pinned_mode(
        next_code_provider_core::DualAuthProvider::Anthropic,
    ) {
        Some(next_code_provider_core::AuthMode::ApiKey) => true,
        Some(next_code_provider_core::AuthMode::Oauth) => false,
    ...
    impl crate::provider::Provider for StubProvider {
        async fn complete(
            ...
            let stream = futures::stream::once(async move {
                Ok(next_code_message_types::StreamEvent::TextDelta(reply))
            });
            Ok(Box::pin(stream))
        }
