...
fn auth_status_check_fast_includes_bedrock_probe() {
    ...
    let prev_bedrock_enable = product_env_os("BEDROCK_ENABLE");

    crate::env::set_var("NEXT_CODE_BEDROCK_ENABLE", "1");
    AuthStatus::invalidate_cache();

    let status = AuthStatus::check_fast();
    assert_eq!(status.bedrock, AuthState::Available);

    restore_env_var("NEXT_CODE_BEDROCK_ENABLE", prev_bedrock_enable);
    AuthStatus::invalidate_cache();
}
...
fn full_and_fast_auth_status_match_for_shared_probe_fields() {
    ...
    let saved = [
        ...
        "NEXT_CODE_CURSOR_CLI_PATH",
    ]
    .into_iter()
    .map(|key| (key, std::env::var_os(key)))
    .collect::<Vec<_>>();

    crate::env::set_var("NEXT_CODE_HOME", temp.path().join("next-code-home"));
...
fn full_and_fast_auth_status_document_cursor_cli_exception() {
    ...
    crate::env::remove_var("CURSOR_ACCESS_TOKEN");
    crate::env::remove_var("CURSOR_REFRESH_TOKEN");
    crate::env::set_var("NEXT_CODE_CURSOR_CLI_PATH", &mock_cli);
    AuthStatus::invalidate_cache();
...
fn cursor_status_is_available_when_api_key_exists_without_cli() {
    ...
    let prev_cli_path = product_env_os("CURSOR_CLI_PATH");
    let temp = tempfile::TempDir::new().expect("create temp dir");
    ...
    restore_env_var("NEXT_CODE_CURSOR_CLI_PATH", prev_cli_path);
    AuthStatus::invalidate_cache();
}
...
fn configured_api_key_source_uses_valid_overrides() {
    let _lock = crate::storage::lock_test_env();
    let key_var = "NEXT_CODE_OPENAI_COMPAT_API_KEY_NAME";
    let file_var = "NEXT_CODE_OPENAI_COMPAT_ENV_FILE";
...
fn anthropic_api_provider_reports_api_key_independently_of_oauth() {
    ...
    std::fs::create_dir_all(&home).expect("create temp home");
    std::fs::create_dir_all(&xdg).expect("create temp xdg config");
    let saved = ["NEXT_CODE_HOME", "XDG_CONFIG_HOME", "HOME", "ANTHROPIC_API_KEY"]
        .into_iter()
        .map(|key| (key, std::env::var_os(key)))
        .collect::<Vec<_>>();
...
/// it even without --no-browser or NO_BROWSER/NEXT_CODE_NO_BROWSER.
#[test]
