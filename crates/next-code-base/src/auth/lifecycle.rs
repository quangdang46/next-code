...
use next_code_provider_core::ActiveProvider;
...
fn globally_preferred_model_rank(model: &str) -> (u8, usize) {
    ...
    let openai_default = normalize_model_for_preference(next_code_provider_core::DEFAULT_OPENAI_MODEL);
    let claude_default = normalize_model_for_preference(next_code_provider_core::DEFAULT_CLAUDE_MODEL);
...
fn normalize_model_for_preference(model: &str) -> String {
    let mut id = next_code_provider_core::model_id::canonical(model);
    ...
    id = next_code_provider_core::model_id::strip_date_suffix(&id).to_string();
...
/// `next_code_provider_core::auto_default_provider`: Claude wins when both families
...
fn direct_provider_activation(provider_id: &str) -> Option<ProviderActivation> {
    match normalized_login_provider_id(provider_id)? {
        ...
        "next-code" | "jcode" => Some(ProviderActivation::locked(
...
pub fn model_switch_request_for_provider_id(
    ...
    match normalized_auth_provider_id(provider_id) {
        ...
        Some("openrouter") | Some("next-code") | Some("jcode") => {
...
mod tests {
    ...
    fn direct_login_provider_activation_sets_runtime_identity_and_active_provider() {
        ...
        // (input, expected_normalized_id, runtime, active, force_locked)
        for (provider, expected_id, runtime, active, force_locked) in [
            ("claude", "claude", "claude", "claude", true),
            ("claude-api", "claude-api", "claude-api", "claude", true),
            ...
            ("jcode", "next-code", "next-code", "openrouter", true),
