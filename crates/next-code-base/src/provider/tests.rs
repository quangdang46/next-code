...
fn save_test_openrouter_model_cache(namespace: &str, source_api_base: &str, model_ids: &[&str]) {
    ...
    let cache = next_code_provider_openrouter::DiskCache {
        cached_at: next_code_provider_openrouter::current_unix_secs().expect("current unix time"),
        source_api_base: Some(source_api_base.to_string()),
        models: model_ids
            .iter()
            .map(|id| next_code_provider_openrouter::ModelInfo {
                id: (*id).to_string(),
                name: String::new(),
                context_length: None,
                pricing: next_code_provider_openrouter::ModelPricing::default(),
                created: None,
            })
            .collect(),
    };
...
fn standard_openrouter_catalog_refresh_fires_when_named_profile_owns_slot() {
    with_clean_provider_test_env(|| {
        ...
        runtime.block_on(async {
            ...
            next_code_provider_openrouter_runtime::reset_profile_catalog_refresh_tracker_for_tests();
...
struct StubExternalRuntime {
    ...
    credential_mode: std::sync::RwLock<next_code_provider_core::CredentialMode>,
}
...
impl StubExternalRuntime {
    fn new(
        ...
        Self {
            ...
            credential_mode: std::sync::RwLock::new(next_code_provider_core::CredentialMode::Auto),
        }
    }
    ...
    with_clean_provider_test_env(|| {
        ...
        runtime.block_on(async {
            ...
            assert_eq!(
                ...
                Some(next_code_provider_core::ResolvedCredential::ApiKey)
            );
