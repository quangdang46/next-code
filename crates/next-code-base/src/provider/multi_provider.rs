    ...
    pub(super) async fn try_same_provider_account_failover(
        ...
        for alternative_label in &alternatives {
            ...
            match attempt {
                ...
                .push(format!(
                        "⚡ Auto-switched {} account: {} → {}. To turn this off, set `[provider].same_provider_account_failover = false` in `~/.next-code/config.toml` or export `NEXT_CODE_SAME_PROVIDER_ACCOUNT_FAILOVER=false`.",
