...
pub fn remote_model_routes_fallback(
    ...
    for model in remote_available_entries {
        ...
        if next_code_provider_core::model_id::matches_known_model(model, ALL_OPENAI_MODELS) {
            let availability = model_availability_for_account(model);
...
fn remote_openai_compatible_profile_models(
    ...
    if let Some(cache) =
        next_code_provider_openrouter::load_disk_cache_entry_for_namespace(&resolved.id)
    {
    ...
    impl EnvGuard {
        ...
        fn save_opencode_cache(&self, source_api_base: &str, model_ids: &[&str]) {
            ...
            let cache = next_code_provider_openrouter::DiskCache {
                cached_at: next_code_provider_openrouter::current_unix_secs()
