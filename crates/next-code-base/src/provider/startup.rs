...
impl MultiProvider {
    ...
    pub(super) fn new_with_auth_status(auth_status: auth::AuthStatus) -> Self {
        ...
        let use_claude_cli = product_env("USE_CLAUDE_CLI")
            .map(|v| v == "1" || v.eq_ignore_ascii_case("true"))
            .unwrap_or(false);
        if use_claude_cli {
            crate::logging::warn(
                "NEXT_CODE_USE_CLAUDE_CLI is deprecated and will be removed. Direct Anthropic API transport is the default.",
            );
        }

        let claude = if has_claude_creds && use_claude_cli {
            crate::logging::info(
                "Using deprecated Claude CLI provider (forced by NEXT_CODE_USE_CLAUDE_CLI=1)",
            );
        ...
        let openrouter = if has_openrouter_creds {
            let named_profile = product_env("NAMED_PROVIDER_PROFILE")
                .ok()
        ...
        let copilot_premium_zero = matches!(
            product_env("COPILOT_PREMIUM").ok().as_deref(),
            Some("0")
        );
