...
#[cfg(test)]
use next_code_provider_core::FailoverDecision;
...
pub use next_code_provider_core::attempt_tracker;
pub use next_code_provider_core::cli_provider_arg_for_session_key;
pub use next_code_provider_core::{
    ...
    NEXT_CODE_USER_AGENT, ModelCapabilities, ModelCatalogRefreshSummary, ModelRoute,
...
pub use next_code_provider_core::{ProviderFailoverPrompt, parse_failover_prompt_message};
...
fn configured_standard_openrouter_profile_routes() -> Vec<ModelRoute> {
    let Some(cache) = next_code_provider_openrouter::load_disk_cache_entry_for_namespace("openrouter")
    else {
        return Vec::new();
    };
...
impl MultiProvider {
    ...
    pub(super) fn set_config_default_model(
            ...
            Self::resolve_config_provider_selection(pref, crate::config::config())
        {
            ...
            let pinned = next_code_provider_core::AuthRoute::parse_explicit_credential_prefix(pref);
            let anthropic_credential_mode = pinned.and_then(|route| {
                ...
                .then(|| match route.mode {
                    ...
                    next_code_provider_core::AuthMode::Oauth => {
            ...
            let openai_credential_mode = pinned.and_then(|route| {
                ...
                .then(|| match route.mode {
                    next_code_provider_core::AuthMode::ApiKey => openai::OpenAICredentialMode::ApiKey,
                    next_code_provider_core::AuthMode::Oauth => openai::OpenAICredentialMode::OAuth,
                })
            });
        ...
        let prefix = match active {
            ActiveProvider::Claude => {
                if let Some(anthropic) = self.anthropic_provider() {
                    ...
                    anthropic
                        .credential_mode()
                        .auth_route(next_code_provider_core::DualAuthProvider::Anthropic)
            ...
            ActiveProvider::OpenAI => {
                if let Some(openai) = self.openai_provider() {
                    openai
                        .credential_mode()
                        .auth_route(next_code_provider_core::DualAuthProvider::OpenAI)
                        .map(|route| route.model_prefix())
                        .unwrap_or("openai")
                } else {
                    "openai"
...
impl Provider for MultiProvider {
        ...
        match self.active_provider() {
            ActiveProvider::Claude => {
                ...
                if let Some(anthropic) = self.anthropic_provider() {
                    ...
                    next_code_provider_core::DEFAULT_CLAUDE_MODEL.to_string()
                }
            }
            ActiveProvider::OpenAI => self
                .openai_provider()
                .map(|o| o.model())
                .unwrap_or_else(|| next_code_provider_core::DEFAULT_OPENAI_MODEL.to_string()),
    ...
    fn active_resolved_credential(&self) -> Option<next_code_provider_core::ResolvedCredential> {
        use next_code_provider_core::ResolvedCredential;
        match self.active_provider() {
            ActiveProvider::Claude => {
    ...
    fn active_explicit_credential(&self) -> Option<next_code_provider_core::ResolvedCredential> {
        use next_code_provider_core::ResolvedCredential;
    ...
    fn set_model(&self, model: &str) -> Result<()> {
            ...
            explicit_model_provider_prefix(requested_model)
        {
            ...
            let pinned = next_code_provider_core::AuthRoute::parse_explicit_credential_prefix(prefix);
            let openai_credential_mode = pinned.and_then(|route| {
                matches!(
                    route.provider,
                    next_code_provider_core::DualAuthProvider::OpenAI
