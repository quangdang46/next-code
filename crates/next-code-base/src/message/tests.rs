...
fn generated_image_rendered_image_uses_inline_tool_anchor() {
    ...
    assert_eq!(
        image.anchor,
        Some(next_code_session_types::RenderedImageAnchor::ToolCall {
...
fn redact_secrets_also_covers_shared_sanitizer_formats() {
    ...
    // the layered next-code-redact pass (AWS access key id, Bearer header).
...
fn format_background_task_notification_markdown_highlights_failure_reason() -> Result<()> {
    let rendered = format_background_task_notification_markdown(&BackgroundTaskCompleted {
        ...
        display_name: Some("Build next-code".to_string()),
        session_id: "session".to_string(),
        status: BackgroundTaskStatus::Failed,
        exit_code: Some(101),
        output_preview: "[stderr]    Compiling next-code\nsccache: Compile terminated by signal 15\n[stderr] error: could not compile `next-code` (lib)".to_string(),
        output_file: std::path::PathBuf::from("/tmp/output.log"),
        duration_secs: 62.5,
        notify: true,
        wake: false,
