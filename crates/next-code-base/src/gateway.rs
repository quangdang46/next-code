...
//! This lets iOS/web clients interact with next-code sessions identically
//! to TUI clients.
//!
//! Architecture:
...
pub use next_code_gateway_types::{PairedDevice, PairingCode};
    ...
    let response = match (method, path_base) {
        ("GET", "/health") => {
            let body = serde_json::json!({
                "status": "ok",
                "version": next_code_build_meta::VERSION,
                "gateway": true,
            });
...
async fn handle_pair_request(
    ...
    // Reload from disk - pairing codes are generated by `next-code pair` CLI
    *reg = DeviceRegistry::load();
    ...
    let body = serde_json::json!({
        "token": token,
        "server_name": "next-code",
        "server_version": next_code_build_meta::VERSION,
    });
    http_response(200, "OK", &body.to_string())
}
