...
use next_code_background_types::{
...
pub fn redact_secrets(text: &str) -> String {
        ...
        && !lower.contains("token")
    {
        ...
        return next_code_redact::redact_secrets(text);
    }
    ...
    next_code_redact::redact_secrets(&redacted)
}
...
pub fn generated_image_rendered_image(
    ...
    path: &str,
    output_format: &str,
) -> Option<next_code_session_types::RenderedImage> {
    let (media_type, data) = generated_image_payload(path, output_format)?;
    Some(next_code_session_types::RenderedImage {
        media_type,
        data,
        ...
        anchor: Some(next_code_session_types::RenderedImageAnchor::ToolCall { id: id.to_string() }),
    })
}
