...
fn truncate_import_text(text: String, max_bytes: usize) -> String {
    ...
    format!(
        "{}{}",
        next_code_core::util::truncate_str(&text, prefix_budget),
        suffix
    )
}
...
pub fn imported_session_id_for_target(
    ...
    match target {
        ...
        next_code_session_types::ResumeTarget::CodexSession { session_id, .. } => {
            Some(imported_codex_session_id(session_id))
        }
        next_code_session_types::ResumeTarget::PiSession { session_path } => {
            Some(imported_pi_session_id(session_path))
        }
        next_code_session_types::ResumeTarget::OpenCodeSession { session_id, .. } => {
            Some(imported_opencode_session_id(session_id))
        }
        next_code_session_types::ResumeTarget::CursorSession { session_id, .. } => {
...
pub fn import_cursor_session_from_path(
    ...
    let created_at =
        next_code_import_core::file_modified_datetime(session_path).unwrap_or_else(Utc::now);
