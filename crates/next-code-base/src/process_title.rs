...
#[cfg(target_os = "linux")]
const KILLALL_PROCESS_NAME: &str = "next-code";
...
/// identifies the session/connection, so do not repeat `next-code` or the memorable
...
pub fn set_server_title(server_name: &str) {
    set_title(compact_process_title("nc:s:", Some(server_name)));
}
...
pub fn set_client_display_title(session_name: &str, is_selfdev: bool) {
    let prefix = if is_selfdev { "nc:d:" } else { "nc:c:" };
    ...
    fn terminal_window_title_omits_product_and_animal_names() {
        assert_eq!(
            terminal_window_title(
                ...
                Some("next-code Octopus"),
                false
            ),
            "🐙 resume window title"
        );
        assert_eq!(
            terminal_window_title("🐙", None, Some("next-code Octopus"), false),
            "🐙 next-code Octopus"
        );
        assert_eq!(
            terminal_window_title(
