...
impl GeneratedImagePanelInfo {
    fn from_inputs(
        ...
        Self {
            path: path.to_string(),
            file_label: next_code_terminal_image::metadata::compact_path_label(path),
    ...
    fn title(&self) -> String {
        ...
        if let Some((width, height)) = self.dimensions {
            title.push_str(" · ");
            title.push_str(&next_code_terminal_image::metadata::format_dimensions(
                width, height,
            ));
        }
        title
    ...
    fn summary_parts(&self) -> Vec<String> {
        ...
        if let Some((width, height)) = self.dimensions {
            parts.push(next_code_terminal_image::metadata::format_dimensions(
                width, height,
            ));
        }
        parts.push(next_code_terminal_image::metadata::compact_image_format(
            &self.output_format,
        ));
        if let Some(byte_count) = self.byte_count {
            parts.push(next_code_terminal_image::metadata::format_byte_count(
                byte_count,
            ));
        }
    ...
    fn markdown(&self) -> String {
        ...
        if let Some((width, height)) = self.dimensions {
            let mut dimensions = next_code_terminal_image::metadata::format_dimensions(width, height);
            if let Some(ratio) = next_code_terminal_image::metadata::aspect_ratio(width, height) {
        ...
        markdown.push_str(&format!(
            "- Format: {}\n",
            markdown_code(&next_code_terminal_image::metadata::compact_image_format(
                &self.output_format,
            ))
        ));
        if let Some(byte_count) = self.byte_count {
            markdown.push_str(&format!(
                "- Bytes: {}\n",
                markdown_code(&next_code_terminal_image::metadata::format_byte_count(
                    byte_count
                ))
            ));
        }
