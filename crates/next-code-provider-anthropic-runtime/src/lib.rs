...
//! loading, cache-TTL toggles) stay in `next_code_base::provider::anthropic`;
...
fn oauth_beta_headers(model: &str) -> &'static str {
    next_code_provider_core::anthropic_oauth_beta_headers(model)
}

use anyhow::{Context, Result};
use async_trait::async_trait;
use futures::StreamExt;
use next_code_base::provider::anthropic::{
...
#[cfg(test)]
use next_code_base::provider::anthropic::{OAUTH_BETA_HEADERS, effectively_1m};
#[cfg(test)]
use next_code_message_types::{ContentBlock, Role};
use next_code_message_types::{Message, StreamEvent, ToolDefinition};
#[cfg(test)]
use next_code_provider_anthropic::{ApiContentBlock, ToolResultContent, ToolResultContentBlock};
use next_code_provider_anthropic::{
...
#[cfg(test)]
pub(crate) const OAUTH_BETA_HEADERS_1M: &str = next_code_provider_core::ANTHROPIC_OAUTH_BETA_HEADERS_1M;
...
fn load_official_claude_client_metadata() -> OAuthClientMetadata {
    let path = match next_code_base::storage::user_home_path(".claude.json") {
        Ok(path) => path,
...
async fn oauth_preflight_get(
    ...
    if !resp.status().is_success() {
        ...
        let body = next_code_base::util::http_error_body(resp, "HTTP error").await;
...
async fn ensure_oauth_preflight(
    ...
    let eval = OAuthEvalRequest {
        attributes: OAuthEvalAttributes {
            ...
            subscription_type: next_code_base::auth::claude::get_subscription_type()
    ...
    if all_ok {
        next_code_base::logging::info("Claude OAuth preflight completed successfully");
    }
    Ok(())
}

/// Quality-first default shared with model routing and first-run selection.
const DEFAULT_MODEL: &str = next_code_provider_core::DEFAULT_CLAUDE_MODEL;
...
impl AnthropicProvider {
    fn is_usage_exhausted() -> bool {
        let usage = next_code_base::usage::get_sync();
    ...
    pub async fn fetch_live_model_ids_for_doctor(&self) -> Result<Vec<String>> {
        ...
        let catalog = if is_oauth {
            next_code_base::provider::fetch_anthropic_model_catalog_oauth(&token).await?
        } else {
            next_code_base::provider::fetch_anthropic_model_catalog(&token).await?
        };
        // Persist so the rest of the process benefits from the warm catalog,
        // exactly like the runtime's own prefetch.
        next_code_base::provider::persist_anthropic_model_catalog(&catalog);
        ...
        if !catalog.available_models.is_empty() {
            next_code_base::provider::populate_anthropic_models(catalog.available_models.clone());
        }
    ...
    pub fn new() -> Self {
        ...
        let _ = tokio::runtime::Handle::try_current().map(|_| {
            tokio::spawn(async {
                let _ = next_code_base::usage::get().await;
            })
        });
        ...
        let reasoning_effort = next_code_base::config::config()
            .provider
    ...
    fn model_supports_output_effort(model: &str) -> bool {
        ...
        // generations); see `next_code_provider_core::anthropic_reasoning_caps`.
        // Fable 5 verified live 2026-07-01; Sonnet 5 verified live 2026-07-07.
        next_code_provider_core::anthropic_reasoning_caps(model).output_effort
    }

    fn model_supports_adaptive_thinking(model: &str) -> bool {
        next_code_provider_core::anthropic_reasoning_caps(model).adaptive_thinking
    }

    fn model_supports_manual_thinking(model: &str) -> bool {
        next_code_provider_core::anthropic_reasoning_caps(model).manual_thinking
    }

    fn model_supports_xhigh_effort(model: &str) -> bool {
        next_code_provider_core::anthropic_reasoning_caps(model).xhigh_effort
    }
    ...
    fn model_supports_max_effort(model: &str) -> bool {
        next_code_provider_core::anthropic_reasoning_caps(model).max_effort
    }

    fn model_supports_reasoning_effort(model: &str) -> bool {
        next_code_provider_core::anthropic_reasoning_caps(model).supports_reasoning_effort()
    }
    ...
    fn actual_effort_for_model(model: &str, effort: &str) -> String {
        if next_code_base::prompt::is_swarm_effort(effort) {
    ...
    fn store_effort_for_model(model: &str, effort: &str) -> String {
        if next_code_base::prompt::is_deep_swarm_effort(effort) {
            next_code_base::prompt::SWARM_DEEP_EFFORT.to_string()
        } else if next_code_base::prompt::is_swarm_effort(effort) {
            next_code_base::prompt::SWARM_EFFORT.to_string()
        } else {
    ...
    fn manual_thinking_budget(effort: &str, max_tokens: u32) -> Option<u32> {
        let desired = match effort {
            ...
            "high" => 8_192,
            "xhigh" | "max" => 16_384,
            e if next_code_base::prompt::is_swarm_effort(e) => 16_384,
            _ => return None,
        };
    ...
    fn build_reasoning_request_parts(
        ...
        let show_thinking = next_code_base::config::config().display.show_thinking;
    ...
    async fn get_oauth_access_token(&self) -> Result<(String, bool)> {
        ...
        if fresh_creds.expires_at < now + 300_000 && !fresh_creds.refresh_token.is_empty() {
            ...
            {
                Ok(refreshed) => {
                    next_code_base::logging::info("OAuth token refreshed successfully");
                ...
                Err(e) => {
                    next_code_base::logging::error(&format!("OAuth token refresh failed: {}", e));
    ...
    fn format_messages(&self, messages: &[Message], is_oauth: bool) -> Vec<ApiMessage> {
        next_code_provider_anthropic::format_messages(messages, is_oauth)
    }
    ...
    fn format_content_blocks(
        ...
        next_code_provider_anthropic::format_content_blocks(blocks, is_oauth)
    }
    ...
    fn format_tools(&self, tools: &[ToolDefinition], is_oauth: bool) -> Vec<ApiTool> {
        next_code_provider_anthropic::format_tools(tools, is_oauth, is_cache_ttl_1h())
    }
}
...
impl Provider for AnthropicProvider {
    ...
    fn credential_mode(&self) -> next_code_provider_core::CredentialMode {
        self.credential_mode_snapshot().into()
    }

    fn set_credential_mode(&self, mode: next_code_provider_core::CredentialMode) -> Result<()> {
    ...
    fn set_model(&self, model: &str) -> Result<()> {
        ...
        let model: &str = if is_1m_model(model)
            && matches!(
                next_code_provider_core::anthropic_context_mode(model),
                next_code_provider_core::AnthropicContextMode::Native1M
            ) {
            strip_1m_suffix(model)
        } else {
            model
        };
        if !next_code_base::provider::known_anthropic_model_ids()
            .iter()
    ...
    fn available_models_for_switching(&self) -> Vec<String> {
        next_code_base::provider::cached_anthropic_model_ids()
            .unwrap_or_else(next_code_base::provider::known_anthropic_model_ids)
    }
    ...
    async fn prefetch_models(&self) -> Result<()> {
        ...
        let catalog = if is_oauth {
            match next_code_base::provider::fetch_anthropic_model_catalog_oauth(&token).await {
                ...
                Err(err) if is_oauth_catalog_auth_error(&err.to_string()) => {
                    ...
                    next_code_base::provider::fetch_anthropic_model_catalog_oauth(&refreshed_token)
                        .await
                }
                Err(err) => Err(err),
            }
        ...
        next_code_base::provider::persist_anthropic_model_catalog(&catalog);
        if !catalog.context_limits.is_empty() {
            next_code_base::provider::populate_context_limits(catalog.context_limits);
        }
        if !catalog.available_models.is_empty() {
            next_code_base::provider::populate_anthropic_models(catalog.available_models);
        }
        Ok(())
    }
...
async fn run_stream_with_retries(
    ...
    for attempt in 0..MAX_RETRIES {
        if attempt > 0 {
            ...
            let delay = next_code_provider_core::retry_after::retry_delay(
                attempt,
                RETRY_BASE_DELAY_MS,
            ...
            let _ = tx
                .send(Ok(StreamEvent::ConnectionPhase {
                    phase: next_code_message_types::ConnectionPhase::Retrying {
                        attempt: attempt + 1,
                        max: MAX_RETRIES,
                    },
                }))
        ...
        let (attempt_tx, attempt_guard) =
            next_code_provider_core::attempt_tracker::track_attempt_output(tx.clone());
        ...
        let attempt_client = if attempt == 0 {
            ...
            next_code_provider_core::fresh_transport_client()
        };
        ...
        .await
        {
            ...
            Err(e) => {
                ...
                if is_oauth && is_oauth_auth_error(&error_str) && !attempted_forced_refresh {
                    ...
                    let _ = tx
                        .send(Ok(StreamEvent::ConnectionPhase {
                            phase: next_code_message_types::ConnectionPhase::Authenticating,
                        }))
                        .await;
                ...
                if is_retryable_error(&error_str) && attempt + 1 < MAX_RETRIES {
                    if saw_output {
                        ...
                        next_code_base::logging::info(&format!("Transient error, will retry: {}", e));
                    }
                    next_retry_delay = next_code_provider_core::retry_after::retry_after_from_error(&e);
                    last_error = Some(e);
                    continue;
                }
...
async fn stream_response(
    ...
    {
        next_code_base::logging::info(&format!("Anthropic request payload:\n{}", json));
    }
    ...
    if !response.status().is_success() {
        ...
        let retry_after = next_code_provider_core::retry_after::retry_after(response.headers());
        let error_text = next_code_base::util::http_error_body(response, "HTTP error").await;
        return Err(next_code_provider_core::retry_after::error_with_retry_after(
...
fn is_retryable_error(error_str: &str) -> bool {
    next_code_provider_core::is_transient_transport_error(error_str)
        // Server errors (5xx)
...
fn anthropic_model_quality_rank(model: &str) -> usize {
    ...
    let normalized = next_code_provider_core::model_id::strip_date_suffix(
        &next_code_provider_core::model_id::canonical(model),
    )
    .to_string();
    next_code_provider_core::ALL_CLAUDE_MODELS
        .iter()
        .position(|candidate| {
            next_code_provider_core::model_id::strip_date_suffix(
                &next_code_provider_core::model_id::canonical(candidate),
            ) == normalized
        })
        ...
        // available.
        .unwrap_or(next_code_provider_core::ALL_CLAUDE_MODELS.len())
}
...
fn anthropic_recommended_model_from_error(error_str: &str) -> Option<String> {
    ...
    next_code_base::provider::known_anthropic_model_ids()
        .into_iter()
...
fn parse_sse_event(buffer: &mut String) -> Option<SseEvent> {
    ...
    for line in event_str.lines() {
        ...
        } else if let Some(rest) = next_code_base::util::sse_data_line(line) {
            data = rest.to_string();
        }
    }
    ...
    match event.event_type.as_str() {
        ...
        "ping" => {
            ...
            events.push(StreamEvent::ConnectionPhase {
                phase: next_code_message_types::ConnectionPhase::Streaming,
            });
        }
        "error" => {
            next_code_base::logging::error(&format!("Anthropic stream error: {}", event.data));
...
fn build_system_param(system: &str, is_oauth: bool) -> Option<ApiSystem> {
    next_code_provider_anthropic::build_system_param(system, is_oauth, is_cache_ttl_1h())
}
...
fn build_system_param_split(
    ...
    next_code_provider_anthropic::build_system_param_split(
        static_part,
        dynamic_part,
        is_oauth,
        is_cache_ttl_1h(),
...
fn format_messages_with_identity(messages: Vec<ApiMessage>, is_oauth: bool) -> Vec<ApiMessage> {
    next_code_provider_anthropic::format_messages_with_identity(messages, is_oauth, is_cache_ttl_1h())
}

#[cfg(test)]
fn add_message_cache_breakpoint(messages: &mut [ApiMessage]) {
    next_code_provider_anthropic::add_message_cache_breakpoint(messages, is_cache_ttl_1h())
}
