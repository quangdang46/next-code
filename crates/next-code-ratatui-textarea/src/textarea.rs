...
impl TextArea {
    ...
    fn is_scrollbar_thumb_at(&self, row: u16, area: Rect, tw: u16) -> bool {
        ...
        // Render scrollbar — inline version (avoids tui-scrollbar dep conflict)
        let sb_x = area.right().saturating_sub(1);
        let mut scratch = ratatui::buffer::Buffer::empty(ratatui::layout::Rect {
            x: sb_x,
            y: area.y,
            width: 1,
            height: area.height,
        ...
        if total > area.height as usize {
            let ratio = current_scroll as f64 / total as f64;
            let thumb_y = (ratio * area.height as f64).round() as u16;
            if thumb_y < area.height {
                scratch.set_string(sb_x, area.y + thumb_y, "\u{258c}", Style::default());
            }
        }
...
impl TextArea {
    ...
    fn render_scrollbar(
        ...
        // Render scrollbar — inline version (avoids tui-scrollbar dep conflict)
        let mut scratch = ratatui::buffer::Buffer::empty(sb_area);
        if total_lines as usize > viewport_lines as usize {
            let ratio = offset as f64 / total_lines.max(1) as f64;
            let thumb_y = (ratio * viewport_lines as f64) as u16;
            if thumb_y < sb_area.height {
                scratch.set_string(sb_area.x, sb_area.y + thumb_y, "\u{258c}", Style::default());
            }
        }
