//! Tracking of active session process IDs under `~/.next-code/active_pids`.
//!
...
//! lives in the storage crate because it only needs [`next_code_dir`] and is a
...
use crate::next_code_dir;
use std::path::PathBuf;

/// Directory holding one file per active session ID (`~/.next-code/active_pids`).
pub fn active_pids_dir() -> Option<PathBuf> {
    next_code_dir().ok().map(|d| d.join("active_pids"))
}
...
pub fn streaming_pids_dir() -> Option<std::path::PathBuf> {
    next_code_dir().ok().map(|d| d.join("streaming_pids"))
}
...
/// List active session IDs currently tracked in `~/.next-code/active_pids`.
...
/// Live snapshot of how many next-code sessions are running, and how many of
/// those are actively streaming a model response right now. Used by the menu
/// bar indicator and any other presence UI.
