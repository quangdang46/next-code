...
impl jcode_memory_types::MemoryProvider for MemoryManager {
    async fn remember(
        &self,
        ...
        match scope {
            jcode_memory_types::MemoryScope::Project => self.remember_project(entry),
            jcode_memory_types::MemoryScope::Global => self.remember_global(entry),
            jcode_memory_types::MemoryScope::All => self.remember_project(entry),
        }
    }
    ...
    async fn recall(
        ...
        match mode {
            ...
            _ => self.find_similar_scoped(query, 0.5, limit, scope),
        }
    }
