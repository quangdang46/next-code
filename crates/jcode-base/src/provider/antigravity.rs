...
/// Free-function wrapper for load_persisted_catalog.
pub fn load_persisted_catalog() -> Option<PersistedCatalog> {
    AntigravityProvider::load_persisted_catalog()
}

/// Free-function wrapper for persist_catalog.
/// Delegates to the associated function on AntigravityProvider.
pub fn persist_catalog(snapshot: &CatalogSnapshot) {
    AntigravityProvider::persist_catalog(snapshot);
}

/// Free-function wrapper for fetch_catalog_snapshot.
pub async fn fetch_catalog_snapshot(
    client: &reqwest::Client,
) -> anyhow::Result<CatalogSnapshot> {
    use anyhow::Context;
    let url = format!("{FETCH_MODELS_API_URL}");
    let resp = client
        .get(&url)
        .header("X-Goog-Api-Client", X_GOOG_API_CLIENT)
    ...
    let response: FetchAvailableModelsResponse =
        serde_json::from_str(&text).context("failed to parse antigravity model catalog")?;
    let snapshot = parse_fetch_available_models_response(&response);
    Ok(snapshot)
}
