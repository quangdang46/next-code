...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum OpenAICredentialMode {
    Auto,
    OAuth,
    ApiKey,
}

impl From<OpenAICredentialMode> for jcode_provider_core::CredentialMode {
    fn from(m: OpenAICredentialMode) -> Self {
        match m {
            OpenAICredentialMode::Auto => Self::Auto,
            OpenAICredentialMode::OAuth => Self::OAuth,
            OpenAICredentialMode::ApiKey => Self::ApiKey,
        }
    }
}

impl From<jcode_provider_core::CredentialMode> for OpenAICredentialMode {
    fn from(m: jcode_provider_core::CredentialMode) -> Self {
        match m {
            jcode_provider_core::CredentialMode::Auto => Self::Auto,
            jcode_provider_core::CredentialMode::OAuth => Self::OAuth,
            jcode_provider_core::CredentialMode::ApiKey => Self::ApiKey,
        }
    }
}

impl OpenAICredentialMode {
    pub fn from_runtime_env(_provider: impl std::fmt::Debug) -> Self {
    ...
    pub fn auth_route(self, _provider: impl std::fmt::Debug) -> Option<jcode_provider_core::AuthRoute> {
...
/// Free-function wrapper for OpenAIProvider::resolve_api_base.
/// This is the canonical place for other modules (models_catalog, etc.) to get
/// the active OpenAI API base URL without constructing the full provider.
pub fn resolve_api_base() -> String {
    OpenAIProvider::resolve_api_base()
}

/// Free-function wrapper for OpenAIProvider::supports_extended_prompt_cache_retention.
pub fn supports_extended_prompt_cache_retention(model_id: &str) -> bool {
    OpenAIProvider::supports_extended_prompt_cache_retention(model_id)
}
