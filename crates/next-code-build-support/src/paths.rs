...
///
/// Release archives install a tiny `next-code` wrapper script alongside the real
/// `next-code-<platform>.bin` payload (the wrapper sets `LD_LIBRARY_PATH` and execs
...
pub fn launcher_dir() -> Result<PathBuf> {
    ...
    #[cfg(windows)]
    {
        if let Ok(local) = std::env::var("LOCALAPPDATA") {
            return Ok(PathBuf::from(local).join("next-code").join("bin"));
        }
        Ok(home_dir()?
            .join("AppData")
            .join("Local")
...
/// Path to the launcher binary (`~/.local/bin/next-code` by default).
...
mod tests {
    ...
    fn repo_fixture(git_file: bool, package_name: &str) -> tempfile::TempDir {
        let temp = tempfile::TempDir::new().expect("temp repo");
        if git_file {
            std::fs::write(temp.path().join(".git"), "gitdir: /tmp/next-code-test-git\n")
                .expect("git file");
        } else {
    ...
    fn find_repo_in_ancestors_finds_workspace_from_crate_dir() {
        let repo = repo_fixture(false, "next-code");
        let crate_dir = repo.path().join("crates").join("next-code-build-support");
