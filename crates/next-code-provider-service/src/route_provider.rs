...
//!
//! [`RouteProvider`] implements the legacy [`next_code_provider_core::Provider`]
...
use next_code_message_types::{Message, ToolDefinition};
...
impl RouteProvider {
    ...
    fn uses_sse_framing(&self) -> bool {
        matches!(self.route.framing, next_code_llm_core::framing::Framing::Sse)
    }
}
...
mod tests {
    ...
    use next_code_llm_core::framing::Framing;
    use next_code_llm_core::route::Route;
    use next_code_llm_core::schema::ModelRef;
    use next_code_llm_core::transport::Transport;
    ...
    fn route_provider_active_resolved_credential_detects_oauth_via_protocol() {
        ...
        assert_eq!(
            rp.active_resolved_credential(),
            Some(next_code_provider_core::ResolvedCredential::Oauth)
        );
    }
