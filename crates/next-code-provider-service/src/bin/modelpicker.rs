...
use next_code_keyring_store::{DefaultKeyringStore, MockKeyringStore};
use next_code_provider_service::catalog::CatalogService;
use next_code_provider_service::integration::IntegrationService;
use next_code_provider_service::service::ProviderService;
use next_code_provider_service::store::DefaultProviderService;
use next_code_provider_service::tui_picker::{Filter, PickerState, RowOrigin};
...
async fn build_service() -> Result<DefaultProviderService> {
    ...
    let svc = if std::env::var("MOCK_KEYRING").is_ok() {
        ...
        let credentials: Arc<dyn next_code_provider_service::credential::CredentialService> = Arc::new(
            next_code_provider_service::store::KeyringCredentialStore::new(keyring),
        );
        let integration: Arc<dyn IntegrationService> =
            Arc::new(next_code_provider_service::store::PersistentIntegration::<
                MockKeyringStore,
            >::new(credentials.clone()));
        let catalog: Arc<dyn CatalogService> =
            Arc::new(next_code_provider_service::catalog::InMemoryCatalog::new());
        next_code_provider_service::boot::register_builtins::<MockKeyringStore>(
            catalog.as_ref(),
            integration.as_ref(),
        )
        .await?;
        ...
        let catalog: Arc<dyn CatalogService> =
            Arc::new(next_code_provider_service::catalog::InMemoryCatalog::new());
        next_code_provider_service::boot::register_builtins::<DefaultKeyringStore>(
            catalog.as_ref(),
            integration.as_ref(),
        )
        .await?;
...
async fn run<B: ratatui::backend::Backend>(
    ...
    let (initial_favorites, initial_recents): (
        ...
        next_code_provider_service::model_prefs::ModelPrefs::load(&path)
            .map(|p| {
                (
                    p.favorites_set(),
                    p.recents
    ...
    let mut selected: Option<(ProviderId, next_code_provider_service::types::ModelId)> = None;
                        ...
                        KeyCode::Char('f') => {
                            ...
                            if let Some(path) = next_code_provider_service::model_prefs::default_path()
                            {
                                ...
                                prefs.favorites = state
                                    ...
                                    .iter()
                                    .map(|(p, m)| {
                                        next_code_provider_service::model_prefs::FavoriteEntry {
                                            provider: p.clone(),
                                            model: m.clone(),
                                        }
                                    })
...
mod tests {
    ...
    use next_code_provider_service::boot::{BUILTIN_PROVIDERS, register_builtins};
    use next_code_provider_service::catalog::{InMemoryCatalog, ModelTier};
