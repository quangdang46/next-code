...
use next_code_keyring_store::DefaultKeyringStore;

use next_code_provider_service::integration::AuthMethod;
use next_code_provider_service::service::ProviderService;
use next_code_provider_service::store::DefaultProviderService;
use next_code_provider_service::types::ProviderId;
...
async fn main() -> Result<()> {
    ...
    let result = match cmd {
        ...
        "aliases" => {
            ...
            for a in next_code_provider_service::aliases::AliasTable::with_builtins().patterns() {
                println!("{a}");
            }
            Ok(())
        }
...
async fn build_service() -> Result<DefaultProviderService> {
    ...
    next_code_provider_service::boot::boot_default::<DefaultKeyringStore>()
        .await
...
async fn cmd_prefs_show() -> Result<()> {
    let path = next_code_provider_service::model_prefs::default_path().context("HOME not set")?;
    let prefs = next_code_provider_service::model_prefs::ModelPrefs::load(&path)
        .map_err(|e| anyhow::anyhow!(e.to_string()))?;
...
async fn cmd_prefs_favorite(provider: &str, model: &str) -> Result<()> {
    let path = next_code_provider_service::model_prefs::default_path().context("HOME not set")?;
    let mut prefs = next_code_provider_service::model_prefs::ModelPrefs::load(&path)
        .map_err(|e| anyhow::anyhow!(e.to_string()))?;
...
async fn cmd_session_start(provider: Option<&str>, model: Option<&str>) -> Result<()> {
    ...
    use next_code_provider_service::types::{ModelId, ProviderProfile};
...
async fn cmd_resolve(
    ...
    let model_id = if let Some(m) = model {
        next_code_provider_service::types::ModelId::from(m)
    } else {
...
async fn cmd_model_default(
    ...
    use next_code_provider_service::defaults::ProviderDefaults;
    let id = ProviderId::from(provider);
    let _ = svc
        .catalog()
    ...
    let path = next_code_provider_service::defaults::default_path()
...
async fn cmd_model_show(
    ...
    let model_id = match model {
        Some(m) => next_code_provider_service::types::ModelId::from(m),
        None => {
    ...
    for provider in svc.integration().list().await? {
        ...
        for c in creds {
            ...
            let type_str = match &c.credential {
                next_code_provider_service::credential::CredentialType::ApiKey { .. } => "api-key",
                next_code_provider_service::credential::CredentialType::OAuth { .. } => "oauth",
                next_code_provider_service::credential::CredentialType::ExternalCommand { .. } => {
                    "command"
                }
            };
            println!(
...
fn cmd_legacy(flag: &str) -> Result<()> {
    use next_code_provider_service::retrofit::parse_legacy_provider_flag;
...
async fn cmd_metadata_list() -> Result<()> {
    ...
    {
        for r in next_code_provider_service::metadata_profiles::all_metadata_records() {
...
async fn cmd_metadata_register(svc: &DefaultProviderService) -> Result<()> {
    #[cfg(feature = "metadata")]
    {
        let registry = next_code_provider_service::metadata_profiles::metadata_registry();
        let n = registry
...
#[cfg(test)]
mod tests {
    use next_code_provider_service::boot::BUILTIN_PROVIDERS;
...
mod login_unified_tests {
    ...
    use next_code_provider_service::integration::{AuthMethod, InMemoryIntegration, LoginProvider};
    use next_code_provider_service::service::ProviderService;
    ...
    async fn fixture_with_provider(auth_methods: Vec<AuthMethod>) -> DefaultProviderService {
        let creds: Arc<dyn next_code_provider_service::credential::CredentialService> =
            Arc::new(InMemoryCredentialStore::new());
        let integration: Arc<dyn next_code_provider_service::integration::IntegrationService> =
        ...
        integration
            ...
            .await
            .unwrap();
        let catalog: Arc<dyn next_code_provider_service::catalog::CatalogService> =
    ...
    async fn login_with_key_persists_api_key() {
        ...
        assert!(matches!(
            creds[0].credential,
            next_code_provider_service::credential::CredentialType::ApiKey { .. }
        ));
    }
}
