...
impl Tool for BeadsListTool {
    ...
    async fn execute(&self, input: Value, ctx: ToolContext) -> Result<ToolOutput> {
        ...
        match params.status.as_deref() {
            Some("open") => { filters.statuses = Some(vec![crate::beads::Status::Open]); }
            Some("in_progress") => { filters.statuses = Some(vec![crate::beads::Status::InProgress]); }
            Some("blocked") => { filters.statuses = Some(vec![crate::beads::Status::Blocked]); }
            Some("closed") => { filters.include_closed = true; filters.statuses = Some(vec![crate::beads::Status::Closed]); }
            Some("all") => { filters.include_closed = true; filters.include_deferred = true; }
            _ => {
                filters.statuses = Some(vec![
                    crate::beads::Status::Open, crate::beads::Status::InProgress, crate::beads::Status::Blocked,
                ]);
            }
        }
        if let Some(label) = &params.label { filters.labels = Some(vec![label.clone()]); }
        if let Some(assignee) = &params.assignee { filters.assignee = Some(assignee.clone()); }
        if let Some(sort) = &params.sort { filters.sort = Some(sort.clone()); }
        filters.limit = params.limit.or(Some(50));

        let issues = project.storage().list_issues(&filters)?;
        let items: Vec<Value> = issues.into_iter().map(|i| json!({
        ...
        Ok(ToolOutput::new(serde_json::to_string_pretty(&json!({"issues": items}))?)
...
struct BeadsCreateInput {
    ...
    #[serde(default)] issue_type: Option<String>,
}

#[async_trait]
impl Tool for BeadsCreateTool {
    fn name(&self) -> &str { "beads_create" }
    fn description(&self) -> &str { "Create a new beads issue/task." }
    ...
    async fn execute(&self, input: Value, ctx: ToolContext) -> Result<ToolOutput> {
        ...
        let issue = crate::beads::Issue {
            ...
            priority, issue_type, labels: params.labels,
            assignee: params.assignee, status: crate::beads::Status::Open,
            created_at: now, updated_at: now,
            ..default_issue()
        };
        project.storage_mut().create_issue(&issue, &ctx.session_id)
...
fn default_ready_limit() -> usize { 10 }
...
impl Tool for BeadsReadyTool {
    ...
    async fn execute(&self, input: Value, ctx: ToolContext) -> Result<ToolOutput> {
        ...
        let items: Vec<Value> = ready.into_iter().map(|i| json!({
            "id": i.id, "title": i.title, "priority": i.priority.to_string(),
            "assignee": i.assignee, "labels": i.labels,
            "created_at": i.created_at.to_rfc3339(),
        })).collect();

        Ok(ToolOutput::new(serde_json::to_string_pretty(&json!({"ready": items}))?)
...
#[derive(Deserialize)]
struct BeadsClaimInput { id: String }

#[async_trait]
impl Tool for BeadsClaimTool {
    fn name(&self) -> &str { "beads_claim" }
    fn description(&self) -> &str {
    ...
    async fn execute(&self, input: Value, ctx: ToolContext) -> Result<ToolOutput> {
        ...
        let manager = crate::beads::BeadsTaskManager::new(&project);
        let issue = manager.set_status(&params.id, crate::beads::Status::InProgress, &ctx.session_id)
...
impl Tool for BeadsCloseTool {
    ...
    fn description(&self) -> &str { "Close a beads issue with an optional reason." }
    ...
    async fn execute(&self, input: Value, ctx: ToolContext) -> Result<ToolOutput> {
        ...
        match params.action.as_str() {
            "add" => {
                ...
                Ok(ToolOutput::new(format!("Added dep: {} blocks on {}", params.issue, params.depends_on)))
            }
            "remove" => {
...
fn default_issue() -> crate::beads::Issue {
    let now = chrono::Utc::now();
    crate::beads::Issue {
        id: String::new(), content_hash: None, title: String::new(),
        description: None, design: None, acceptance_criteria: None, notes: None,
        status: crate::beads::Status::Open,
        priority: crate::beads::Priority(2),
        issue_type: crate::beads::IssueType::Task,
        assignee: None, owner: None, estimated_minutes: None,
        created_at: now, created_by: None, updated_at: now,
        closed_at: None, close_reason: None, closed_by_session: None,
        due_at: None, defer_until: None, external_ref: None,
        source_system: Some("jcode".to_string()),
        source_repo: None, source_repo_path: None, agent_context: None,
        labels: Vec::new(),
        deleted_at: None, deleted_by: None, delete_reason: None,
        original_type: None, compaction_level: None, compacted_at: None,
        compacted_at_commit: None, original_size: None,
        sender: None, ephemeral: false, pinned: false, is_template: false,
        dependencies: Vec::new(), comments: Vec::new(),
    }
}
