...
#[cfg(feature = "aws-sdk")]
use next_code_message_types::{ContentBlock as JContentBlock, Role as JRole, StreamEvent};
use next_code_message_types::{Message as JMessage, ToolDefinition};
#[cfg(feature = "aws-sdk")]
use next_code_provider_core::summarize_model_catalog_refresh;
...
impl BedrockProvider {
    pub fn new() -> Self {
        let model =
            std::env::var("NEXT_CODE_BEDROCK_MODEL").unwrap_or_else(|_| DEFAULT_MODEL.to_string());
        let provider = Self {
    ...
    pub fn has_credentials() -> bool {
        let explicitly_enabled = product_env("BEDROCK_ENABLE")
            .ok()
        ...
        let has_credential_hint = Self::configured_bearer_token().is_some()
            ...
            || product_env_os("BEDROCK_PROFILE").is_some()
    ...
    async fn sdk_config() -> aws_types::SdkConfig {
        ...
        if let Some(token) = Self::configured_bearer_token() {
            next_code_core::env::set_var(API_KEY_ENV, token);
        }
        if let Some(region) = Self::configured_region() {
            loader = loader.region(aws_types::region::Region::new(region));
        }
        if let Ok(profile) =
            std::env::var("NEXT_CODE_BEDROCK_PROFILE").or_else(|_| std::env::var("AWS_PROFILE"))
        {
    ...
    async fn validate_credentials_if_requested() -> Result<()> {
        let validate = product_env("BEDROCK_VALIDATE_STS")
            .ok()
    ...
    pub fn configured_bearer_token() -> Option<String> {
        next_code_provider_env::load_api_key_from_env_or_config(API_KEY_ENV, ENV_FILE)
    }
    ...
    fn env_or_config(name: &str) -> Option<String> {
        std::env::var(name)
            ...
            .filter(|v| !v.is_empty())
            .or_else(|| next_code_provider_env::load_env_value_from_env_or_config(name, ENV_FILE))
    }

    fn persisted_catalog_path() -> Result<std::path::PathBuf> {
        Ok(next_code_storage::app_config_dir()?.join("bedrock_models_cache.json"))
    }

    fn load_persisted_catalog() -> Option<PersistedCatalog> {
        let path = Self::persisted_catalog_path().ok()?;
        next_code_storage::read_json(&path).ok()
    }
    ...
    fn persist_catalog(
        ...
        if let Err(err) = next_code_storage::write_json(&path, &payload) {
    ...
    fn inference_config() -> Option<InferenceConfiguration> {
        let max_tokens = product_env("BEDROCK_MAX_TOKENS")
            .ok()
            .and_then(|v| v.trim().parse::<i32>().ok())
            .filter(|v| *v > 0);
        let temperature = product_env("BEDROCK_TEMPERATURE")
            .ok()
            .and_then(|v| v.trim().parse::<f32>().ok())
            .filter(|v| (0.0..=1.0).contains(v));
        let top_p = product_env("BEDROCK_TOP_P")
            .ok()
            .and_then(|v| v.trim().parse::<f32>().ok())
            .filter(|v| (0.0..=1.0).contains(v));
        let stop_sequences = product_env("BEDROCK_STOP_SEQUENCES")
            .ok()
            .map(|v| {
                v.split(',')
                    .map(str::trim)
...
impl Provider for BedrockProvider {
    ...
    async fn complete(
        ...
        next_code_provider_core::log_provider_canonical_input(
            "bedrock",
            &model,
...
mod tests {
    ...
    impl EnvVarGuard {
        fn set(key: &'static str, value: impl AsRef<OsStr>) -> Self {
            ...
            next_code_core::env::set_var(key, value);
            Self { key, previous }
        }
    ...
    impl Drop for EnvVarGuard {
        fn drop(&mut self) {
            if let Some(value) = self.previous.as_ref() {
                next_code_core::env::set_var(self.key, value);
            } else {
    ...
    fn detects_env_credentials_requires_region_and_credential_hint() {
        ...
        let _removed = [
            "NEXT_CODE_BEDROCK_ENABLE",
            API_KEY_ENV,
            REGION_ENV,
            "AWS_REGION",
            "AWS_DEFAULT_REGION",
            "AWS_PROFILE",
            "NEXT_CODE_BEDROCK_PROFILE",
            "AWS_ACCESS_KEY_ID",
        ...
        next_code_core::env::set_var(REGION_ENV, "us-east-1");
        assert!(!BedrockProvider::has_credentials());
        next_code_core::env::set_var("AWS_PROFILE", "test");
        assert!(BedrockProvider::has_credentials());
    }
    ...
    fn explicit_enable_marks_configured_for_instance_metadata_credentials() {
        let _guard = lock_test_env();
        next_code_core::env::set_var("NEXT_CODE_BEDROCK_ENABLE", "1");
        assert!(BedrockProvider::has_credentials());
        next_code_core::env::remove_var("NEXT_CODE_BEDROCK_ENABLE");
    }
    ...
    fn detects_bedrock_login_env_file_credentials() {
        ...
        assert!(!BedrockProvider::has_credentials());
        next_code_provider_env::save_env_value_to_env_file(API_KEY_ENV, ENV_FILE, Some("test-key"))
            .unwrap();
        next_code_core::env::remove_var(API_KEY_ENV);
        assert!(!BedrockProvider::has_credentials());

        next_code_provider_env::save_env_value_to_env_file(REGION_ENV, ENV_FILE, Some("us-east-2"))
            .unwrap();
        next_code_core::env::remove_var(REGION_ENV);
    ...
    async fn bedrock_live_smoke_test() {
        if product_env("BEDROCK_LIVE_TEST").ok().as_deref() != Some("1") {
            return;
        }
