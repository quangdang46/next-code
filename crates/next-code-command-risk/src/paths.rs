...
pub fn classify_target(
    ...
    if raw.contains('*') || raw.contains('?') {
            ...
            && expanded.file_name().is_some_and(|n| n == "*")
        {
            return Some(RiskFinding {
                ...
                justification: "would destroy the entire contents of a protected directory"
                    .to_string(),
            ...
            && !parent.to_string_lossy().contains(['*', '?'])
        {
            ...
            if inside_cwd || is_temp_path(parent) {
                return Some(RiskFinding {
                    level: RiskLevel::Low,
                    justification: "glob is bounded to the working or temporary directory"
                        .to_string(),
                    target: Some(raw.to_string()),
                });
            }
        ...
        return Some(RiskFinding {
            ...
            justification: "target contains a glob, so the exact set of affected files \
    ...
    if is_catastrophic_target(expanded, ctx) {
        return Some(RiskFinding {
            level: RiskLevel::Critical,
            justification: "targets a protected system or home path that must never be \
                     destroyed"
                .to_string(),
    ...
    if raw.contains('$') || raw.contains('`') {
        return Some(RiskFinding {
            level: RiskLevel::Medium,
            justification: "target is computed at runtime (variable or command \
        ...
        && !expanded.starts_with("/dev/stderr")
    {
        return Some(RiskFinding {
            level: RiskLevel::Critical,
            justification: "writes directly to a device node, which can destroy a \
                     filesystem or disk"
                .to_string(),
            target: Some(expanded.display().to_string()),
        });
    ...
    if inside_cwd {
        ...
        return recursive.then(|| RiskFinding {
            level: RiskLevel::Low,
            justification: "recursive delete inside the working directory".to_string(),
            target: Some(expanded.display().to_string()),
        });
    }
