...
//! # Why this exists
//!
//! next-code executes `bash` tool calls with no gate of its own: the only check in
...
//! (Ported from jcode-command-risk, where a user lost their home directory.)
//!
//! # Design
//!
...
//! [`RiskLevel::Medium`] is a reflection prompt rather than a hard block, and
...
///
/// `Safe` / `Low` run immediately; `Medium` requires the model to re-justify
/// (jcode's `Confirm`); `Critical` never runs (jcode's `Catastrophic`).
#[derive(
...
pub enum RiskLevel {
    ...
    Medium,
    /// Destructive but recoverable or scope-limited; elevated scrutiny.
    High,
...
pub struct RiskFinding {
    pub level: RiskLevel,
    /// Human-readable justification, shown to the model verbatim.
...
impl RiskFinding {
    /// The justification line, with the offending target when present.
    pub fn description(&self) -> String {
        match &self.target {
            Some(target) => format!("{} (target: {target})", self.justification),
            None => self.justification.clone(),
        }
    }
}
...
impl RiskAssessment {
    ...
    pub fn explanation(&self) -> String {
        ...
        for finding in &self.findings {
            out.push_str("- ");
            out.push_str(&finding.justification);
