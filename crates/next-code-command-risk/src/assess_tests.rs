...
fn root_destruction_is_blocked_outright() {
    for command in [
        ...
        assert_eq!(level(command), RiskLevel::Critical, "{command:?}");
    }
}
...
fn chaining_does_not_bypass_the_gate() {
    ...
    assert_eq!(level("echo starting && rm -rf ~"), RiskLevel::Critical);
    assert_eq!(level("cd /tmp; rm -rf $HOME"), RiskLevel::Critical);
    assert_eq!(level("true || rm -rf /"), RiskLevel::Critical);
}
...
fn non_rm_destructive_tools_are_covered() {
    ...
    assert_eq!(level("find /home/u -delete"), RiskLevel::Critical);
...
fn runtime_computed_targets_require_justification() {
    ...
    assert!(level("rm -rf $TARGET") >= RiskLevel::Medium);
    assert!(level("rm -rf $(cat list.txt)") >= RiskLevel::Medium);
}

#[test]
fn destructive_command_without_a_visible_target_escalates() {
    // Unknown blast radius is a reason to ask, not to proceed.
    assert!(level("rm -rf") >= RiskLevel::Medium);
}
...
fn catastrophic_cannot_be_unlocked_but_confirm_can() {
    assert!(RiskLevel::Critical.is_absolute_deny());
    assert!(!RiskLevel::Medium.is_absolute_deny());
    assert!(!RiskLevel::Medium.runs_immediately());
    ...
    for verb in verbs {
        for flag in flags {
            for home in homes {
                ...
                if assess(&command, &ctx).level != RiskLevel::Critical {
...
fn nested_wrappers_are_unwrapped_all_the_way_down() {
    assert_eq!(level("sudo env nice -n 5 rm -rf ~"), RiskLevel::Critical);
    assert_eq!(level("timeout 5 sudo rm -rf /"), RiskLevel::Critical);
}
...
fn a_wrapper_hiding_an_unparseable_payload_escalates() {
    ...
    let ctx = ctx();
    assert!(assess("sudo", &ctx).level >= RiskLevel::Medium);
}
...
fn files_inside_system_directories_are_protected_too() {
    ...
    for path in [
        ...
        assert_eq!(level(path), RiskLevel::Critical, "{path:?}");
    }
}
...
fn user_directories_under_home_root_stay_workable() {
    ...
    assert_eq!(level("rm -rf /home"), RiskLevel::Critical);
}
...
fn read_only_find_with_stderr_redirect_is_safe() {
    let command = "find ~/.next-code -maxdepth 3 -type f \\
...
fn harmless_command_operands_are_not_redirect_targets() {
    for command in [
        "grep needle /home/u/.next-code/config 2>/dev/null",
...
fn safe_redirect_sink_does_not_make_explicit_deletion_safe() {
    assert_eq!(level("rm /dev/null"), RiskLevel::Critical);
}
...
fn shell_constructs_do_not_hide_catastrophic_commands() {
    ...
    assert!(
        level("rm -rf $(echo ~)") >= RiskLevel::Medium,
...
fn wrapper_value_flags_and_unbounded_globs_remain_guarded() {
    for command in [
        ...
        assert!(
            level(command) >= RiskLevel::Medium,
