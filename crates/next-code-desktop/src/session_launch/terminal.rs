...
pub(super) fn terminal_candidates(title: &str, next_code_args: &[&str]) -> Vec<Command> {
...
pub(super) fn terminal_candidates_in_dir(
    ...
    terminal_candidates_with_working_dir(title, next_code_args, Some(working_dir))
}
...
fn terminal_env_command(raw_terminal: &str, next_code_args: &[&str]) -> Result<Command> {
