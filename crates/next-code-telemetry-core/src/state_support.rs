...
pub(super) fn telemetry_id_path() -> Option<PathBuf> {
    storage::next_code_dir().ok().map(|d| d.join("telemetry_id"))
}
...
pub(super) fn telemetry_state_path(name: &str) -> Option<PathBuf> {
    storage::next_code_dir().ok().map(|d| d.join(name))
}
...
pub(super) fn version() -> String {
    next_code_build_meta::PKG_VERSION.to_string()
}
...
pub(super) fn is_jcode_repo_dir(dir: &Path) -> bool {
    ...
    std::fs::read_to_string(cargo_toml)
        .map(|content| {
            content.contains("name = \"next-code\"") || content.contains("name = \"next-code\"")
        })
        .unwrap_or(false)
}
...
pub(super) fn build_channel() -> String {
    if std::env::var(next_code_selfdev_types::CLIENT_SELFDEV_ENV).is_ok() {
