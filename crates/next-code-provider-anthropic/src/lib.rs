...
use next_code_llm_core::protocol::{Protocol, StepOutput};
use next_code_llm_core::route::PreparedRoute;
use next_code_llm_core::schema::{ContentPart, GenerationParams, LlmRequest, ModelRef};
use next_code_llm_protocols::anthropic_messages::{
...
pub fn add_message_cache_breakpoint(messages: &mut [ApiMessage], cache_ttl_1h: bool) {
    ...
    if messages.len() < 3 {
        ...
        next_code_logging::info("Conversation caching: too few messages, skipping");
        return;
    }
    ...
    if assistant_indices.is_empty() {
        next_code_logging::info("Conversation caching: no assistant message found");
        return;
    }
...
fn message_to_part(msg: &Message) -> next_code_llm_core::schema::Message {
    ...
    next_code_llm_core::schema::Message {
        role: role.to_string(),
        content,
    }
}
...
fn anthropic_event_to_stream_events(
    ...
    match event {
        ...
        AnthropicEvent::ContentBlockStart {
            ...
            next_code_llm_protocols::anthropic_messages::AnthropicContentBlockStart::Text {
                ..
            } => {
                // No event needed for text start
            }
            next_code_llm_protocols::anthropic_messages::AnthropicContentBlockStart::ToolUse {
                id,
                name,
                ..
            } => {
            ...
            next_code_llm_protocols::anthropic_messages::AnthropicContentBlockStart::Thinking {
                ..
            } => {
        ...
        AnthropicEvent::ContentBlockDelta { index: _, delta } => match delta {
            next_code_llm_protocols::anthropic_messages::AnthropicContentDelta::TextDelta { text } => {
                if !text.is_empty() {
                    events.push(StreamEvent::TextDelta(text.clone()));
                }
            }
            next_code_llm_protocols::anthropic_messages::AnthropicContentDelta::InputJsonDelta {
                partial_json,
            } => {
            ...
            next_code_llm_protocols::anthropic_messages::AnthropicContentDelta::ThinkingDelta {
                thinking,
            } => {
...
impl AnthropicMessagesProvider {
    ...
    fn build_url(&self) -> String {
        ...
        let path = match &self.route.endpoint.path {
            next_code_llm_core::endpoint::PathSpec::Static(p) => p.trim_start_matches('/').to_string(),
            next_code_llm_core::endpoint::PathSpec::Dynamic(p) => p.trim_start_matches('/').to_string(),
        };
        format!("{base}/{path}")
    }
                    ...
                    StepOutput::Events(protocol_events) => {
                        for proto_event in &protocol_events {
                            ...
                            if matches!(
                                proto_event,
                                AnthropicEvent::ContentBlockStart {
                                    content_block: next_code_llm_protocols::anthropic_messages::AnthropicContentBlockStart::ToolUse { .. },
                                    ..
                                }
                            ) {
...
impl Provider for AnthropicMessagesProvider {
    async fn complete(
        ...
        // Convert messages
        let core_messages: Vec<next_code_llm_core::schema::Message> =
            messages.iter().map(message_to_part).collect();

        // Convert tools
        let core_tools: Vec<next_code_llm_core::schema::ToolDefinition> = if tools.is_empty() {
            vec![]
        } else {
...
mod cache_prefix_invariant_tests {
    ...
    use super::*;
    use next_code_message_types::{ContentBlock, Message, Role};
