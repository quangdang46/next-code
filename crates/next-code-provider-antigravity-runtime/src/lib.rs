...
//! Gemini/Claude/gpt-oss upstreams), moved out of `next-code-base` so provider
...
//! [`AntigravityProvider`] with `next_code_base::provider::external` at startup.

use anyhow::{Context, Result};
use async_trait::async_trait;
use next_code_base::auth::antigravity as antigravity_auth;
use next_code_message_types::{ConnectionPhase, Message, StreamEvent, ToolDefinition};
use next_code_provider_antigravity::{
...
impl AntigravityProvider {
    fn load_persisted_catalog() -> Option<PersistedCatalog> {
        next_code_base::provider::antigravity::load_persisted_catalog()
    }

    fn persist_catalog(snapshot: &CatalogSnapshot) {
        next_code_base::provider::antigravity::persist_catalog(snapshot);
    }
    ...
    /// by `next-code login --provider antigravity`; there is no API-key path. This
    ...
    fn resolve_model_for_request(&self, model: &str) -> String {
        ...
        // Claude models on the Cloud Code backend currently reject next-code's tool
    ...
    async fn fetch_available_models(&self) -> Result<CatalogSnapshot> {
        next_code_base::provider::antigravity::fetch_catalog_snapshot(&self.client).await
    }
    ...
    async fn generate_content(
        ...
        let mut tools = next_code_provider_gemini::build_tools(tools);
        ...
        // construct. Gemini-native accepts everything next-code emits, so Gemini
        ...
        let request = CodeAssistGenerateRequest {
            ...
            request: VertexGenerateContentRequest {
                contents: next_code_provider_gemini::build_contents(messages),
                system_instruction: next_code_provider_gemini::build_system_instruction_with_tool_guard(
                    system,
                    !tools_is_empty,
                ),
                tools,
    ...
    async fn complete(
        ...
        tokio::spawn(async move {
            ...
            if let Some(content) = candidate.content {
                ...
                // calls through the standard ToolUseStart/End path so next-code
                ...
                for part in content.parts {
                    ...
                    if let Some(function_call) = part.function_call {
                        ...
                        let call_id = next_code_message_types::sanitize_tool_id(&raw_call_id);
                        let _ = tx
            ...
            if !produced_output {
                ...
                if abnormal {
                    ...
                    let detail = candidate
                        ...
                        .map(|msg| format!(": {}", next_code_base::util::truncate_str(msg.trim(), 300)))
                        .unwrap_or_default();
                    let _ = tx
                        .send(Err(anyhow::anyhow!(
