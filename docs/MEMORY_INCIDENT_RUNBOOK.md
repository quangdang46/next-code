# next-code Server Memory Incident Runbook
...
1. What is using the next-code server's memory?
...
```bash
next-code debug 'server:memory-incident'
```
...
```bash
python scripts/analyze_runtime_memory_log.py --days 1 --json > /tmp/next-code-memory-analysis.json
```
...
2. Run `next-code debug 'swarm:list'` and inspect the largest live swarm.
...
```bash
next-code debug 'server:memory-incident' > /tmp/before.json
next-code debug 'allocator:purge'
next-code debug 'server:memory-incident' > /tmp/after.json
```
...
1. Run `next-code debug 'server:memory'` for the full attribution walk.
