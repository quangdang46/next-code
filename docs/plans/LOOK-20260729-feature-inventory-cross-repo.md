# LOOK — Cross-repo TUI/agent UX feature inventory (Face gap analysis)
...
**Anti-pattern this doc corrects:** Prior `LOOK-20260729-face-best-of-repos-matrix.md` stamped most clusters “Done” after shallow P0 shipping. Merged PRs #112/#124–#135 are **real but incomplete** vs reference depth. Prefer dozens of honest gaps over rubber stamps.
...
Prior PR numbers in Gap column mean “related work landed” — **not** “cluster closed.”
...
- `BuiltinStatusLine` + `StatusLine` + `StatusNotices` — rate-limit countdown, notices (`components/BuiltinStatusLine.tsx`, `StatusLine.tsx`)
...
**Permissions & plan**
- Mode cycle Shift+Tab / Alt+M: `default` ↔ `acceptEdits` ↔ `plan` ↔ `bypassPermissions` (`types/permissions.ts`, PromptInput cycle)
...
**Edit / diff / review**
- `/diff` → DiffDialog / file list (`commands/diff/`, `components/diff/`, `StructuredDiff`)
...
**Explicit non-portable for Face:** buddy sprites, stickers, desktop/mobile QR, Slack/GitHub app installers, Claude Desktop bridge product, “poor mode” budget flags.
...
**Connect / auth / model**
- Provider dialog (`dialog-provider.tsx`) — **best-in-class connect UX reference**
...
- **Which-key overlay** (`feature-plugins/system/which-key.tsx`) — pending-key preview, dock/overlay layouts, group tabs
...
**Roots:** `.tmp/codex/codex-rs/tui/src/{bottom_pane,status,notifications,chatwidget,…}`

**Statusline / footer**
- Rich `StatusLineItem` enum: model, model+reasoning, reasoning, cwd, project-root, git-branch, PR#, branch-changes, run-state, permissions, approval-mode, context-used/remaining, thread-title, … (`bottom_pane/status_line_setup.rs`)
...
- Sandbox crates (OS policy — not Face UI clone)
...
**Notifications**
- `notifications/{bel,osc9,mod}.rs` — terminal bell / OSC9

**Session**
- Resume picker; session archive commands; thread transcript; onboarding

**Slash**
- `slash_command.rs` — large enum (model, compact, diff, …)
...
- Review screen (`review-screen.tsx`) ≠ BoN but related
...
### 1.6 grok-build (Rust — Face ancestor)
...
**Caution:** soft YOLO defaults / brand chrome — do not reintroduce.
...
- TUI primitives (`packages/tui/src/components`: editor, select-list, settings-list, markdown, …)
...
- Slash/commands catalog (acp, agents, mcp, session-pin, stats-dashboard, todo, usage-report, worktree, …)
- TUI components (editor, tab-bar, scroll-view, …)
...
**Roots:** `.tmp/oh-my-openagent/packages/{hashline-core,lsp-*,team-core,tmux-core,claude-code-compat-core,delegate-core,…}`

**Notable (mostly library, not full TUI)**
- hashline-core edit ops
- lsp-core / lsp-daemon / lsp-tools-mcp — diagnostics freshness, tools surface
...
**Unique for Face portability:** LSP daemon patterns; tmux team pane ops; hashline-core; Claude-compat mode mapping — not pixel TUI.
...
- Permission cards + plan gate + AcceptEdits cycle — #112/#125
- Diff hunks / hashline in scrollback — #124
- Session `/diff` DiffReview — #134
- Statusline segments config (`StatusLineSegment`: mode/model/context/cwd/git) + `/statusline` — #133 adjacent
- Footer **mouse** pills (tasks / `@agent`) — #133
- Multitask / teams / BoN cards / mermaid sidebar / memory edit / connect — #126–#132
- Alt+T effort toggle — #135
- Sticky scrollback headers — `scrollback/sticky.rs`, `scrollback_pane.rs`
- Ctrl+R history search — `views/history_search`, `agent_view/prompt.rs`
- `!` bash mode — `ActionId::BashMode`, prompt.rs
- Remappable `~/.next-code/keybindings.json` + `/keybindings` — `actions/user_bindings.rs`
- ShortcutsHelp modal (Ctrl+./Ctrl+X) — **not** which-key pending overlay
- `/vim-mode` — **scrollback** vim letters only (`slash/commands/vim_mode.rs`)
- Notifications package (progress/title/tmux/hooks) — not OpenCode toast parity
- Large slash set under `slash/commands/*` (see §3)

---

## 2. Cross-repo comparison (cluster → strengths → Face winner)
...
| Footer keyboard snag | **Claude** Footer context | — | Claude | **Partial** (mouse only) |
...
| BoN cards | **Codebuff** implementor grid | — | Codebuff | **Partial** (#128) |
...
| LSP-in-chat | **Crush** diagnostics/symbols/… | oh-my lsp-daemon | Crush UX + oh-my daemon | **Missing** |
...
| Session tags / timeline fork | **OpenCode** tag + timeline | — | OpenCode | **Missing**/thin |
| Workspace file changes | **OpenCode** dialog | — | OpenCode | **Missing** |
...
| Output styles | **Claude** `/output-style` | — | Claude | **Missing** |
| Doctor / health | **Claude** `/doctor` | — | Claude | **Missing** |
| Conversation tree | **pi_agent_rust** tree_ui | — | pi_agent_rust | **Missing** |
...
|-----|------|-----|-----------|
| Footer **keyboard** snag (←/→/↑/↓/Enter on pills: tasks, agents, diff, loop) | Partial (#133 mouse) | **P0** | Claude `Footer` bindings |
...
| BoN masonry density / ± progress polish | Partial | P2 | Codebuff blocks |
...
Face already has a large set: `always_approve`, `announcements`, `auto`, `btw`, `cd`, `compact`, `connect`, `context`, `copy`, `dashboard`, `diff`, `effort`, `experimental`, `fork`, `goal`, `history`, `keybindings`, `memory`, `model`, `multitask`, `plan`, `plugin`, `queue`, `recap`, `resume`, `rewind`, `share`, `statusline`, `tasks`, `theme`, `timeline`, `usage`, `vim_mode`, `voice`, … (`slash/commands/`).
...
Do **not** claim “P0s done” until the P0 row below is honestly **Has** or consciously deferred with reason.

1. **P0 — Footer keyboard snag** (Claude) — complete #133  
2. **P0 — Which-key overlay** (OpenCode) — distinct from ShortcutsHelp  
3. **P0 — Prompt stash multi-entry dialog** (OpenCode)  
4. **P0 — Statusline segment expansion** (Codex reasoning / run-state / context-remaining)  
5. **P0 — DiffReview split|unified** (Crush)  
6. **P0 — Prompt vim Normal/Insert** (gajae/Claude) — larger; own PR  
7. **P0 — LSP diagnostics-in-chat** (Crush) — needs daemon story; may span PRs  
8. P1 — `/doctor`, workspace file-changes, session tags, toast stack, external-editor-for-prompt, EffortPanel, memdir depth, `@Agent` chips, MCP elicitation, …

**Skip re-doing unless inventory proves incomplete:** solid cores of #124–#135 (diff hunks, AcceptEdits chrome, BoN cards, multitask spawn, Alt+T binding, `/diff` entrypoint, memory black-screen fix). Prefer **deepening** those surfaces (e.g. split diff on top of #134) over greenfield duplicates.
...
- Pixel-clone Ink/Solid — Face stays ratatui pager chrome  
...
- Prior shallow matrix: `docs/plans/LOOK-20260729-face-best-of-repos-matrix.md` — treat as superseded for “done” claims.  
- Earlier Claude gap look (2026-07-24): `LOOK-20260724-claude-code-ux-gaps-for-face.md` — many items shipped as Partial; this doc re-scores vs all repos.
