...
**Date:** 2026-07-29  
**Branch:** `feat/face-alt-t-thinking`  
**Base:** `review` @ f78a40d7f (PRs #112/#124–#134 merged)  
**Scope:** Alt+T reasoning-effort toggle (Claude thinking-toggle muscle memory).

**Sibling skip (do not duplicate):**
- Statusline Claude interactivity — **#133 merged**
- Session `/diff` DiffReview — **#134 merged**
...
| #134 | Session `/diff` DiffReview (Claude DiffDialog) | **Done** — do not re-implement |
...
| Edit/diff / hashline | **oh-my-pi** hashline + Claude DiffDialog + grok edit blocks | **Has** — #134 `/diff` DiffReview + hashline Diff | — | Done |
...
| Thinking / effort toggle | **Claude** Alt+T ThinkingToggle | **Has** (this PR) — Alt+T / † toggles effort (`none`↔stash or cycle menu) | — | Done |
...
| **claude-code** | AcceptEdits cycle, DiffDialog, permission cards, sticky, statusline, Alt+T | Buddy sprites, remote/mobile, Claude Desktop |
...
1. ~~**P0 — Session `/diff` review**~~ — **#134 merged**.
2. ~~**P0 — AcceptEdits / BoN / hashline / permission cards / statusline**~~ — on `review` (#112/#124–#133).
3. ~~**P2 — Alt+T thinking toggle**~~ — **this PR** (`ActionId::ToggleThinkingEffort`).
4. **P2 — Memory delete via AskUserQuestion** — Claude uses AskUser; Face uses double-press confirm (intentional simpler UX unless product wants parity).
5. **P2 — `@Agent` mentions** — Codebuff; optional.
...
- Do not re-implement BoN/hashline/permission-cards/statusline/`/diff` while siblings or #133/#134 own those surfaces.
- Alt+T is effort toggle (generation), not Ctrl+E expand-all-thinking (display fold).
