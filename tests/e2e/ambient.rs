...
fn test_ambient_state_lifecycle() {
    use next_code::ambient::{AmbientCycleResult, AmbientState, AmbientStatus, CycleStatus};
...
fn test_ambient_scheduled_queue() {
    use next_code::ambient::{Priority, ScheduledItem, ScheduledQueue};

    let tmp = std::env::temp_dir().join("next-code-test-queue.json");
    ...
    queue.push(ScheduledItem {
        ...
        target: next_code::ambient::ScheduleTarget::Ambient,
...
fn test_adaptive_scheduler_intervals() {
    use next_code::ambient_scheduler::{AdaptiveScheduler, AmbientSchedulerConfig};
...
async fn test_ambient_end_cycle_tool() -> Result<()> {
    ...
    let result = next_code::tool::ambient::take_cycle_result();
...
async fn test_ambient_request_permission_tool() -> Result<()> {
    ...
    next_code::tool::ambient::register_ambient_session(ambient_session_id.clone());

    let response = agent.run_once_capture("Request permission").await?;
    next_code::tool::ambient::unregister_ambient_session(&ambient_session_id);
...
async fn test_ambient_runner_status() {
    use next_code::ambient_runner::AmbientRunnerHandle;
...
async fn test_ambient_runner_trigger_and_stop() {
    use next_code::ambient::AmbientStatus;
    use next_code::ambient_runner::AmbientRunnerHandle;
    use next_code::safety::SafetySystem;
...
fn test_memory_reinforcement_provenance() {
    use next_code::memory::{MemoryCategory, MemoryEntry};
...
fn test_ambient_config_defaults() {
    use next_code::config::AmbientConfig;
...
fn test_ambient_lock() {
    use next_code::ambient::AmbientLock;
    let _env = setup_test_env().expect("failed to setup isolated NEXT_CODE_HOME");
...
async fn test_full_ambient_cycle_simulation() -> Result<()> {
    ...
    agent.set_system_prompt("You are the next-code ambient maintenance agent.");
