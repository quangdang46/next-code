...
fn test_safety_classification() {
    use next_code::safety::SafetySystem;

    let safety = SafetySystem::new();

    // Tier 1: auto-allowed
    assert!(safety.classify("read") == next_code::safety::ActionTier::AutoAllowed);
    assert!(safety.classify("glob") == next_code::safety::ActionTier::AutoAllowed);
    assert!(safety.classify("grep") == next_code::safety::ActionTier::AutoAllowed);
    assert!(safety.classify("memory") == next_code::safety::ActionTier::AutoAllowed);
    assert!(safety.classify("todoread") == next_code::safety::ActionTier::AutoAllowed);
    assert!(safety.classify("todowrite") == next_code::safety::ActionTier::AutoAllowed);

    // Tier 2: requires permission
    assert!(safety.classify("bash") == next_code::safety::ActionTier::RequiresPermission);
    assert!(safety.classify("edit") == next_code::safety::ActionTier::RequiresPermission);
    assert!(safety.classify("write") == next_code::safety::ActionTier::RequiresPermission);
    assert!(
        safety.classify("create_pull_request") == next_code::safety::ActionTier::RequiresPermission
    );
    assert!(safety.classify("send_email") == next_code::safety::ActionTier::RequiresPermission);
...
fn test_safety_permission_flow() {
    use next_code::safety::{PermissionRequest, PermissionResult, SafetySystem, Urgency};
...
fn test_safety_transcript() {
    use next_code::safety::{AmbientTranscript, SafetySystem, TranscriptStatus};
...
fn test_safety_summary_generation() {
    use next_code::safety::{ActionLog, ActionTier, SafetySystem};
