...
A deterministic, no-LLM harness for developing and validating the next-code iOS
client (`NextCodeMobile`) end-to-end. It replaces the role of the old Rust
...
- **`mock_gateway.py`** - a self-contained (stdlib-only) mock of the next-code
  server gateway. Speaks the exact wire protocol from
  `crates/next-code-base/src/gateway.rs` on one TCP port:
...
```bash
# Full pipeline, screenshot lands in $TMPDIR/next-code-ios-e2e/chat.png
...
python3 TestHarness/mock_gateway.py &        # or run a real `next-code` gateway
...
`Library/Application Support/next-code-servers.json` when the Keychain is
...
`NextCodeKit` (the platform-free client core) is fully unit-tested with `swift
