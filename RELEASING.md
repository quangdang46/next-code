...
Release asset names use the `next-code-<os>-<arch>` prefix (for example `next-code-linux-x86_64.tar.gz`). Installers also create a one-release compat symlink: `next-code` → `next-code`.
