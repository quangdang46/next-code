...
pub trait Mutation: Send + Sync {
    ...
    fn mutate(&self, source: &str, rng: &mut SimpleRng) -> Option<(String, MutationInfo)> {
        ...
        let mutated_snippet = edits.first().map(|e| e.replacement.clone()).unwrap_or_default();
...
pub fn apply_source_edits(source: &str, edits: &[SourceEdit]) -> Option<String> {
    ...
    for edit in &sorted {
        ...
        let mut new = String::with_capacity(
            out.len() - (edit.end - edit.start) + edit.replacement.len(),
        );
...
impl Mutation for SwapComparisonMutation {
    fn name(&self) -> &str { "swap-comparison" }
    fn category(&self) -> &str { "operator" }
    fn description(&self) -> &str { "A comparison operator is subtly wrong." }
    fn fix_hint(&self) -> &str { "Swap the comparison operator to the correct variant." }
    ...
    fn collect_candidates<'a>(&self, root: Node<'a>, source: &'a str) -> Vec<Candidate<'a>> {
        ...
        for node in find_nodes_recursive(root, "binary_expression") {
            if let Some(op) = node.child_by_field_name("operator") {
                ...
                if matches!(txt, "<" | "<=" | ">" | ">=") {
                    candidates.push(Candidate { node: op, meta: None });
                }
            }
        }
        candidates
    ...
    fn apply(&self, candidate: &Candidate, source: &str, _rng: &mut SimpleRng) -> Vec<SourceEdit> {
        ...
        vec![SourceEdit { start, end, replacement: replacement.to_string() }]
    }
}
...
impl Mutation for SwapEqualityMutation {
    fn name(&self) -> &str { "swap-equality" }
    fn category(&self) -> &str { "operator" }
    fn description(&self) -> &str { "An equality operator is inverted." }
    fn fix_hint(&self) -> &str { "Fix the equality comparison operator." }
...
impl Mutation for SwapLogicalMutation {
    fn name(&self) -> &str { "swap-logical" }
    fn category(&self) -> &str { "operator" }
    fn description(&self) -> &str { "A boolean operator is incorrect." }
    fn fix_hint(&self) -> &str { "Use the intended boolean operator." }
...
impl Mutation for RemoveNegationMutation {
    fn name(&self) -> &str { "remove-negation" }
    fn category(&self) -> &str { "operator" }
    fn description(&self) -> &str { "A logical negation (!) was accidentally removed." }
    fn fix_hint(&self) -> &str { "Add back the missing logical negation (!)." }
    ...
    fn apply(&self, candidate: &Candidate, _source: &str, _rng: &mut SimpleRng) -> Vec<SourceEdit> {
        ...
        if let Some(arg) = candidate.node.child_by_field_name("argument") {
            ...
            let (start, end) = node_range(candidate.node);
            vec![SourceEdit { start, end, replacement: arg_text.to_string() }]
        } else {
            vec![]
        }
    }
...
impl Mutation for SwapAssignOpMutation {
    fn name(&self) -> &str { "swap-assign-op" }
    fn category(&self) -> &str { "operator" }
    fn description(&self) -> &str { "A compound assignment operator is wrong." }
    fn fix_hint(&self) -> &str { "Correct the compound assignment operator." }
...
impl Mutation for SwapArithmeticMutation {
    fn name(&self) -> &str { "swap-arithmetic" }
    fn category(&self) -> &str { "operator" }
    fn description(&self) -> &str { "An arithmetic operator was swapped." }
    fn fix_hint(&self) -> &str { "Correct the arithmetic operator." }
...
impl Mutation for BooleanLiteralFlipMutation {
    fn name(&self) -> &str { "flip-boolean" }
    fn category(&self) -> &str { "literal" }
    fn description(&self) -> &str { "A boolean literal is inverted." }
    fn fix_hint(&self) -> &str { "Flip the boolean literal to the intended value." }
...
impl Mutation for OffByOneMutation {
    fn name(&self) -> &str { "off-by-one" }
    fn category(&self) -> &str { "literal" }
    fn description(&self) -> &str { "A numeric boundary has an off-by-one error." }
    fn fix_hint(&self) -> &str { "Fix the off-by-one error in the numeric literal or comparison." }
        ...
        for node in find_nodes_recursive(root, "integer_literal") {
            ...
            if txt == "0" || txt == "1" {
                ...
                while let Some(p) = parent {
                    let kind = p.kind();
                    if matches!(kind, "for_expression" | "while_expression" | "if_expression") {
                        candidates.push(Candidate { node, meta: None });
                        break;
                    }
...
impl Mutation for SwapCallArgsMutation {
    fn name(&self) -> &str { "swap-call-args" }
    fn category(&self) -> &str { "call" }
    fn description(&self) -> &str { "Two arguments in a call are swapped." }
    fn fix_hint(&self) -> &str { "Swap the two arguments to their original order." }
    ...
    fn collect_candidates<'a>(&self, root: Node<'a>, source: &'a str) -> Vec<Candidate<'a>> {
        ...
        for node in find_nodes_recursive(root, "call_expression") {
            ...
            if let Some(args_node) = args {
                ...
                if arg_list.len() >= 2 {
                    candidates.push(Candidate { node: args_node, meta: None });
                }
            }
        }
        candidates
    ...
    fn apply(&self, candidate: &Candidate, source: &str, _rng: &mut SimpleRng) -> Vec<SourceEdit> {
        ...
        vec![
            SourceEdit { start: f_start, end: f_end, replacement: second_text },
            SourceEdit { start: s_start, end: s_end, replacement: first_text },
        ]
    }
}
...
impl Mutation for RemoveQuestionMarkMutation {
    fn name(&self) -> &str { "remove-question-mark" }
    fn category(&self) -> &str { "access" }
    fn description(&self) -> &str { "The ? operator was removed from a Result expression." }
    fn fix_hint(&self) -> &str { "Add back the missing ? operator." }
    ...
    fn apply(&self, candidate: &Candidate, source: &str, _rng: &mut SimpleRng) -> Vec<SourceEdit> {
        ...
        if let Some(inner) = candidate.node.child_by_field_name("value") {
            ...
            vec![SourceEdit { start, end, replacement: inner_text.to_string() }]
        } else {
            vec![]
        }
    }
...
impl Mutation for IdentifierMultiEditMutation {
    fn name(&self) -> &str { "identifier-multi-edit" }
    fn category(&self) -> &str { "identifier" }
    fn description(&self) -> &str { "An identifier is misspelled in multiple locations." }
    fn fix_hint(&self) -> &str { "Restore the identifier to its original spelling in all affected locations." }
    ...
    fn collect_candidates<'a>(&self, root: Node<'a>, source: &'a str) -> Vec<Candidate<'a>> {
        ...
        for node in find_nodes_recursive(root, "identifier") {
            ...
            if txt.starts_with('_') { continue; }
            counts.entry(txt).or_default().push(node);
        }
        for (_name, nodes) in &counts {
            if nodes.len() >= 3 {
                results.push(Candidate { node: nodes[0], meta: Some(nodes.len().to_string()) });
                break;
            }
        }
        results
    ...
    fn apply(&self, candidate: &Candidate, source: &str, _rng: &mut SimpleRng) -> Vec<SourceEdit> {
        ...
        let (start, end) = node_range(candidate.node);
        vec![SourceEdit { start, end, replacement: mutated }]
    }
}
...
impl Mutation for DuplicateExprMutation {
    fn name(&self) -> &str { "duplicate-expr" }
    fn category(&self) -> &str { "duplicate" }
    fn description(&self) -> &str { "A duplicated expression contains a subtle change." }
    fn fix_hint(&self) -> &str { "Fix the literal or operator on the duplicated expression." }
    ...
    fn collect_candidates<'a>(&self, root: Node<'a>, source: &'a str) -> Vec<Candidate<'a>> {
        ...
        for (_txt, nodes) in &counts {
            if nodes.len() >= 2 {
                ...
                candidates.push(Candidate { node: nodes[0], meta: None });
                break;
            }
        }
        candidates
    ...
    fn apply(&self, candidate: &Candidate, source: &str, _rng: &mut SimpleRng) -> Vec<SourceEdit> {
        ...
        for node in find_nodes_recursive(candidate.node, "boolean_literal") {
            ...
            return vec![SourceEdit { start, end, replacement: replacement.to_string() }];
        }
...
impl Mutation for SwapNamedImportsMutation {
    fn name(&self) -> &str { "swap-named-imports" }
    fn category(&self) -> &str { "import" }
    fn description(&self) -> &str { "Two imported names are swapped." }
    fn fix_hint(&self) -> &str { "Swap the two imported names back to their original order." }
    ...
    fn collect_candidates<'a>(&self, root: Node<'a>, source: &'a str) -> Vec<Candidate<'a>> {
        ...
        for node in find_nodes_recursive(root, "use_list") {
            ...
            if args.len() >= 2 {
                candidates.push(Candidate { node, meta: Some(format!("{}", args.len())) });
            }
        }
        candidates
    }
    ...
    fn apply(&self, candidate: &Candidate, source: &str, _rng: &mut SimpleRng) -> Vec<SourceEdit> {
        ...
        vec![
            SourceEdit { start: s1, end: e1, replacement: f2 },
            SourceEdit { start: s2, end: e2, replacement: f1 },
        ]
    }
}
...
impl Mutation for SwapAdjacentLinesMutation {
    fn name(&self) -> &str { "swap-adjacent-lines" }
    fn category(&self) -> &str { "structural" }
    fn description(&self) -> &str { "Two adjacent statements are in the wrong order." }
    fn fix_hint(&self) -> &str { "Swap the two adjacent lines back to their original order." }
    ...
    fn collect_candidates<'a>(&self, root: Node<'a>, source: &'a str) -> Vec<Candidate<'a>> {
        ...
        for node in find_nodes_recursive(root, "block") {
            ...
            for child in node.children(&mut cursor) {
                ...
                if txt == "{" || txt == "}" || child.is_extra() { continue; }
                stmts.push(child);
            }
    ...
    fn apply(&self, candidate: &Candidate, source: &str, _rng: &mut SimpleRng) -> Vec<SourceEdit> {
        ...
        vec![
            SourceEdit { start: s1, end: e1, replacement: b_text.clone() },
            SourceEdit { start: s2, end: e2, replacement: a_text.clone() },
        ]
    }
}
...
impl Mutation for SwapIfElseBranchesMutation {
    fn name(&self) -> &str { "swap-if-else" }
    fn category(&self) -> &str { "structural" }
    fn description(&self) -> &str { "The if and else branches are swapped." }
    fn fix_hint(&self) -> &str { "Swap the if and else branch bodies back to their original positions." }

    fn collect_candidates<'a>(&self, root: Node<'a>, source: &'a str) -> Vec<Candidate<'a>> {
        let mut candidates = Vec::new();
        for node in find_nodes_recursive(root, "if_expression") {
            // Must have an else clause
            let has_else = node.children(&mut node.walk()).any(|c| node_text(c, source) == "else");
            if has_else {
                candidates.push(Candidate { node, meta: None });
            }
        }
    ...
    fn apply(&self, candidate: &Candidate, source: &str, _rng: &mut SimpleRng) -> Vec<SourceEdit> {
        ...
        vec![
            SourceEdit { start: s1, end: e1, replacement: alt_text },
            SourceEdit { start: s2, end: e2, replacement: cons_text },
        ]
    }
}
...
impl Mutation for RemoveEarlyReturnMutation {
    fn name(&self) -> &str { "remove-early-return" }
    fn category(&self) -> &str { "structural" }
    fn description(&self) -> &str { "A guard clause (early return) was removed." }
    fn fix_hint(&self) -> &str { "Restore the missing guard clause (if expression with early return)." }
    ...
    fn apply(&self, candidate: &Candidate, source: &str, _rng: &mut SimpleRng) -> Vec<SourceEdit> {
        ...
        let (start, end) = node_range(candidate.node);
        vec![SourceEdit { start, end, replacement: String::new() }]
    }
}
...
impl Mutation for DeleteStatementMutation {
    fn name(&self) -> &str { "delete-statement" }
    fn category(&self) -> &str { "structural" }
    fn description(&self) -> &str { "A critical statement was deleted from the code." }
    fn fix_hint(&self) -> &str { "Restore the deleted statement." }
...
impl Mutation for SwapBorrowMutation {
    fn name(&self) -> &str { "swap-borrow" }
    fn category(&self) -> &str { "rust-specific" }
    fn description(&self) -> &str { "A borrow expression uses the wrong mutability." }
    fn fix_hint(&self) -> &str { "Fix the borrow mutability (& vs &mut)." }
...
impl Mutation for SwapUnwrapExpectMutation {
    fn name(&self) -> &str { "swap-unwrap-expect" }
    fn category(&self) -> &str { "rust-specific" }
    fn description(&self) -> &str { "An unwrap/expect call uses the wrong method." }
    fn fix_hint(&self) -> &str { "Replace unwrap() with expect(\"...\") or vice versa." }
            ...
            if let Some(func) = node.child_by_field_name("function") {
                ...
                if txt == "unwrap" || txt.ends_with(".unwrap") {
                    ...
                    if let Some(dot_unwrap) = find_field_expression(func, "unwrap", source) {
                        candidates.push(Candidate { node: dot_unwrap, meta: None });
                    }
                }
                if txt == "expect" || txt.ends_with(".expect") {
                    if let Some(dot_expect) = find_field_expression(func, "expect", source) {
                        candidates.push(Candidate { node: dot_expect, meta: None });
                    }
                }
            }
        }
...
impl Mutation for SwapOkErrMutation {
    fn name(&self) -> &str { "swap-ok-err" }
    fn category(&self) -> &str { "rust-specific" }
    fn description(&self) -> &str { "Ok and Err variants are swapped." }
    fn fix_hint(&self) -> &str { "Swap Ok(..) and Err(..) to the correct variant." }
    ...
    fn collect_candidates<'a>(&self, root: Node<'a>, source: &'a str) -> Vec<Candidate<'a>> {
        ...
        for node in find_nodes_recursive(root, "call_expression") {
            if let Some(func) = node.child_by_field_name("function") {
                let txt = node_text(func, source);
                if txt == "Ok" || txt == "Err" {
                    candidates.push(Candidate { node: func, meta: None });
                }
            }
        }
        candidates
...
impl Mutation for RemoveUnsafeMutation {
    fn name(&self) -> &str { "remove-unsafe" }
    fn category(&self) -> &str { "rust-specific" }
    fn description(&self) -> &str { "The unsafe block was removed." }
    fn fix_hint(&self) -> &str { "Add back the unsafe { ... } wrapper." }
    ...
    fn apply(&self, candidate: &Candidate, source: &str, _rng: &mut SimpleRng) -> Vec<SourceEdit> {
        ...
        if let Some(block) = candidate.node.child(1) {
            ...
            let (start, end) = node_range(candidate.node);
            vec![SourceEdit { start, end, replacement: block_text.to_string() }]
        } else {
            vec![]
        }
    }
...
impl Mutation for SwapMatchArmsMutation {
    fn name(&self) -> &str { "swap-match-arms" }
    fn category(&self) -> &str { "rust-specific" }
    fn description(&self) -> &str { "Two consecutive match arms are swapped." }
    fn fix_hint(&self) -> &str { "Swap the two match arms back to their original order." }
    ...
    fn apply(&self, candidate: &Candidate, source: &str, _rng: &mut SimpleRng) -> Vec<SourceEdit> {
        ...
        let (s2, e2) = node_range(b);
        vec![
            SourceEdit { start: s1, end: e1, replacement: b_text },
            SourceEdit { start: s2, end: e2, replacement: a_text },
        ]
    }
}
...
impl Mutation for FlipFmtArgMutation {
    fn name(&self) -> &str { "flip-fmt-arg" }
    fn category(&self) -> &str { "rust-specific" }
    fn description(&self) -> &str { "A format specifier is wrong ({} vs {:?})." }
    fn fix_hint(&self) -> &str { "Fix the format specifier in the format string." }
    ...
    fn apply(&self, candidate: &Candidate, source: &str, _rng: &mut SimpleRng) -> Vec<SourceEdit> {
        ...
        let (start, end) = node_range(candidate.node);
        vec![SourceEdit { start, end, replacement }]
    }
}
...
impl Mutation for SwapDeriveAttrMutation {
    fn name(&self) -> &str { "swap-derive-attr" }
    fn category(&self) -> &str { "rust-specific" }
    fn description(&self) -> &str { "A derive attribute has the wrong trait." }
    fn fix_hint(&self) -> &str { "Swap Debug/Clone in the derive attribute." }
...
impl Mutation for RenameFunctionMutation {
    ...
    fn category(&self) -> &str { "rust-specific" }
    fn description(&self) -> &str { "A function name has words in the wrong order." }
    fn fix_hint(&self) -> &str { "Swap the word order in the function name." }
        ...
        for node in find_nodes_recursive(root, "function_item") {
            if let Some(name_node) = node.child_by_field_name("name") {
                ...
                if name.contains('_') {
                    ...
                    if parts.len() >= 2 {
                        candidates.push(Candidate { node: name_node, meta: None });
                    }
                }
            }
        }
    ...
    fn apply(&self, candidate: &Candidate, source: &str, _rng: &mut SimpleRng) -> Vec<SourceEdit> {
        ...
        if replacement == name { return vec![]; }
        let (start, end) = node_range(candidate.node);
        vec![SourceEdit { start, end, replacement }]
    }
}
...
mod tests {
    ...
    fn test_all_mutations_have_names() {
        let names: Vec<String> = all_mutations().iter().map(|m| m.name().to_string()).collect();
        assert_eq!(names.len(), 25, "Expected 25 mutations, got {}", names.len());
    ...
    fn test_apply_source_edits_reverse_order() {
        ...
        let edits = vec![
            SourceEdit { start: 0, end: 5, replacement: "world".into() },
            SourceEdit { start: 14, end: 19, replacement: "hello".into() },
        ];
    ...
    fn test_apply_source_edits_overlapping_returns_none() {
        let edits = vec![
            SourceEdit { start: 0, end: 10, replacement: "short".into() },
            SourceEdit { start: 5, end: 15, replacement: "nope".into() },
        ];
